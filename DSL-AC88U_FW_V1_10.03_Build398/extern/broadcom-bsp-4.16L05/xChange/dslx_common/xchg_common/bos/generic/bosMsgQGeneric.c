...
BOS_STATUS bosMsgQSend( BOS_MSGQ *qid, const BOS_MSGQ_MSG *qMsg)
{
   ...
   {
      msgQ->noFreeMsgCount++;
      blogStrErr(( gBosBlog, "Free message buffer pool empty - queue id %d", (unsigned int)*qid ));
   }
...
)
{
   ...
   */
    blogStrAlways(( gBosBlog, "Created message queue \"%s\", depth %d, id %u",
...
BOS_STATUS bosMsgQDisplayStats( BOS_MSGQ *qid )
{
   ...
   blogStrAlwaysNoFields(( gBosBlog, "No Free Buffer Count: %d", msgQ->noFreeMsgCount ));
