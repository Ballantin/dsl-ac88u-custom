...
#define BOS_EVENTCREATE( errcode, eventid, bInitialized, taskid )                \
   ...
   {                                                                             \
      blogStrErr(( gBosBlog, "Not creating event: " #eventid "" ));            \
   }
...
)
{
   ...
   blogStrFnc(( gBosBlog, "Enter TaskCreate %s", name ));
   ...
   {
      blogStrErr(( gBosBlog, "Can't create task (%s) during a reset!", name ));
   ...
   ** Spawn the task.
   */
   blogStrAlways(( gBosBlog, "TaskCreate - spawn new task %s", name ));
   ...
   */
   ...
   if ( retval != BOS_STATUS_OK )
   {
      blogStrErr(( gBosBlog, "TaskCreate: Initialization error, cleaning up task state" ));
   ...
   blogStrFnc(( gBosBlog, "Exit TaskCreate " ));
   return ( retval );
}
...
PUBLIC BOS_STATUS bosTaskDestroyR( BOS_TASK_ID *taskId )
{
   ...
   blogStrFnc(( gBosBlog, "Enter bosTaskDestoryR, taskId = %08x", *taskId ));
...
PUBLIC BOS_STATUS bosTaskResumeR( BOS_TASK_ID *taskId )
{
   ...
   {
      blogStrErr(( gBosBlog, "TaskResume() attempting to resume a task not in SUSPENDED state!" ));
      bosMutexRelease( &boszcb.tcbMutexId );
      return ( BOS_STATUS_ERR );
   }
...
PUBLIC BOS_STATUS bosTaskResetNotify( BOS_TASK_ID *taskid )
{
   ...
   blogStrFnc(( gBosBlog, "BOS: Enter TaskResetNotify " ));
   ...
   if ( t->inuse )
   {
      switch ( t->state )
      {
         case BOS_TASKSTATE_NORMAL:
         {
            ...
            blogStrInfo(( gBosBlog, "TaskResetNotify informed task %s",
         ...
         case BOS_TASKSTATE_SUSPENDED:
         {
            ...
            {
               blogStrInfo(( gBosBlog, "BOS: TaskResetNotify informed and resumed task %s",
                            GetTaskName( &t->taskid ) ));
         ...
         {
            /* Do not interrupt */
            blogStrErr(( gBosBlog, "TaskResetNotify can't reset an uninterruptible task!" ));
   ...
   */
   blogStrAlways(( gBosBlog, "Task 0x%x notified of reset, waiting for ack... ", *taskid ));
   retval = WaitForEvent( BOS_RESET_ACK_EVENT );

   if ( retval != BOS_STATUS_OK )
   {
      blogStrErr(( gBosBlog, "TaskResetNotify(): BIG TROUBLE in little china! "
                             "Can't wait on event while waiting for reset ack!" ));
   }
   else
   {
      blogStrAlways(( gBosBlog, "BOS: Task 0x%x acknowledged reset. ", *taskid ));
   }

   blogStrFnc(( gBosBlog, "Exit TaskResetNotify " ));
   return ( BOS_STATUS_OK );
}
...
PUBLIC BOS_STATUS bosTaskResetAck( void )
{
   ...
   blogStrFnc(( gBosBlog, "Enter TaskResetAck (task=%s) ", GetTaskNameSelf() ));
   ...
   */
   ...
   blogStrFnc(( gBosBlog, "Exit TaskResetAck (task=%s) ", GetTaskNameSelf() ));
...
PUBLIC BOS_STATUS bosTaskProceedToShutdown( BOS_TASK_ID *taskid )
{
   ...
   blogStrFnc(( gBosBlog, "Enter TaskProceedToShutdown 0x%x (%s)", *taskid, GetTaskName( taskid ) ));
   ...
   switch ( state )
   {
      ...
      case BOS_TASKSTATE_RESET_NTFY:
      {
         ...
         blogStrInfo(( gBosBlog, "TaskProceedToShutdown() signaling task (%s)", GetTaskName( taskid ) ));
      ...
      case BOS_TASKSTATE_DONE:
      {
         ...
         blogStrWarn(( gBosBlog, "TaskProceedToShutdown() not signaling task (%s) (task already DONE)", GetTaskName( taskid ) ));
      ...
      default:
      {
         ...
         blogStrWarn(( gBosBlog, "TaskProceedToShutdown() not signaling task (%s) (not in RESET_ACKED or NTFY)", GetTaskName( taskid ) ));
         bSignalTask = BOS_FALSE;
      }
      break;
   }
   ...
   blogStrFnc(( gBosBlog, "BOS: Exit TaskProceedToShutdown (%s)", GetTaskName( taskid ) ));
...
PUBLIC BOS_STATUS bosTaskWaitForDone( BOS_TASK_ID *taskid )
{
   ...
   blogStrFnc(( gBosBlog, "Enter TaskWaitForDone - %s waiting on task 0x%x (%s)",
   ...
   blogStrFnc(( gBosBlog, "Exit TaskWaitForDone - %s done waiting on task 0x%x (%s)",
         GetTaskNameSelf(), *taskid, GetTaskName( taskid ) ));
...
PUBLIC BOS_STATUS bosAppInit( void )
{
   ...
   blogStrFnc(( gBosBlog, "Enter bosAppInit " ));
   ...
   */
   ...
   if ( retval != BOS_STATUS_OK )
   {
      blogStrErr(( gBosBlog, "bosAppInit: Initialization error, cleaning up semaphores/mutexes" ));
   ...
   blogStrFnc(( gBosBlog, "Exit bosAppInit " ));
   return( retval );
}
...
{
   blogStrFnc(( gBosBlog, "Enter bosStartApp" ));
   ...
   blogStrFnc(( gBosBlog, "Exit bosStartApp" ));
...
PUBLIC void bosAppReset( void )
{
   ...
   switch ( boszcb.resetDetectMode )
   {
      ...
      {
         blogStrAlways(( gBosBlog, "AppReset() - Signalling root task of reset." ));
         ...
         ** reset signal, and then notify the aoRootTask of the reset */
         ...
         blogStrAlways(( gBosBlog, "AppReset() - Wait for root task to complete reset." ));
      ...
      case BOS_RESETDETECT_DISABLED:
      {
         ...
         blogStrWarn(( gBosBlog, "AppReset() - Ignoring reset, reset detection is disabled." ));
      }
      break;
      ...
      case BOS_RESETDETECT_INHIBITED:
      {
         ...
         blogStrWarn(( gBosBlog, "AppReset() - Postponing reset until detection is enabled, reset detection is currently inhibited." ));
...
PUBLIC void bosAppResetDetectionEnable( void )
{
   ...
   switch ( boszcb.resetDetectMode )
   {
      case BOS_RESETDETECT_ENABLED:
      {
         blogStrWarn(( gBosBlog, "AppResetDetectionEnable() - Doing nothing, detection already enabled." ));
      }
      break;

      case BOS_RESETDETECT_DISABLED:
      {
         blogStrInfo(( gBosBlog, "AppResetDetectionEnable() - Enabled reset detection." ));
      ...
      case BOS_RESETDETECT_INHIBITED:
      {
         ...
         {
            blogStrInfo(( gBosBlog, "AppResetDetectionEnable() - Enabled reset detection, no resets were postponed." ));
         }
         else
         {
            /* A reset was postponed, signal it now */
            blogStrInfo(( gBosBlog, "AppResetDetectionEnable() - Enabled reset detection, continuing a postponed reset..." ));
            boszcb.resetState = BOS_RESETSTATE_NORESET;
            bosAppReset();
         }
      }
...
PUBLIC BOS_STATUS bosAppResetDetectionInhibit( void )
{
   ...
   if ( boszcb.resetState == BOS_RESETSTATE_RESETTING )
   {
      blogStrWarn(( gBosBlog, "AppResetDetectionInhibit() - Can't inhibit reset detection, reset already in progress." ));
      retval = BOS_STATUS_RESET;
   }
   else
   {
      boszcb.resetDetectMode = BOS_RESETDETECT_INHIBITED;
      blogStrInfo(( gBosBlog, "AppResetDetectionInhibit() - Reset detection has been inhibited." ));
   }
...
PRIVATE BOS_BOOL IsResetPendingCheck( BOS_BOOL isResource )
{
   ...
   else if ( isResource && ( state == BOS_TASKSTATE_UNINTERRUPTIBLE ) )
   {
      ...
      blogStrErr(( gBosBlog, "Uninterruptible task trying to use an interruptible resource!" ));
      BOS_ASSERT( 0 );
   }
...
PRIVATE BOS_STATUS TaskResetNotifyAll( void )
{
   ...
   blogStrFnc(( gBosBlog, "Enter TaskResetNotifyAll " ));
      ...
      if ( t->inuse )
      {
         switch ( t->state )
         {
            case BOS_TASKSTATE_NORMAL:
            {
               ...
               {
                  SetTaskState( &t->taskid, BOS_TASKSTATE_RESET_NTFY );
                  blogStrInfo(( gBosBlog, "TaskResetNotifyAll informed task %s", GetTaskName( &t->taskid ) ));
                  nResetTasks++;
               }
            }
            break;
            ...
            case BOS_TASKSTATE_SUSPENDED:
            {
               ...
               {
                  blogStrInfo(( gBosBlog, "TaskResetNotifyAll informed and resumed task %s", GetTaskName( &t->taskid ) ));
   ...
   */
   blogStrInfo(( gBosBlog, "%d tasks notified of reset, waiting for acks...", nResetTasks ));
   for ( i = 0; i < nResetTasks; i++ )
   {
      ...
      if ( retval != BOS_STATUS_OK )
      {
         blogStrErr(( gBosBlog, "TaskResetNotifyAll(): Failed to take %p with %d", &boszcb.taskResetSemId, (int)retval ));
      }
   }

   blogStrInfo(( gBosBlog, ": All %d tasks acknowledged reset. ", nResetTasks ));

   blogStrFnc(( gBosBlog, "Exit TaskResetNotifyAll " ));
   return ( BOS_STATUS_OK );
}
...
PRIVATE BOS_STATUS TaskEnterShutdown( void )
{
   ...
   blogStrFnc(( gBosBlog, "Enter TaskEnterShutdown (task=%s) ", GetTaskNameSelf() ));
   ...
   blogStrFnc(( gBosBlog, "Exit TaskEnterShutdown (task=%s) ", GetTaskNameSelf() ));
   return ( BOS_STATUS_OK );
}
...
PRIVATE BOS_STATUS TaskEnterDone( void )
{
   ...
   blogStrFnc(( gBosBlog, "Enter TaskEnterDone (task=%s) ", GetTaskNameSelf() ));
   ...
   blogStrFnc(( gBosBlog, "Exit TaskEnterDone (task=%s) ", GetTaskNameSelf() ));
...
PRIVATE void AppResetTemplateTaskMain( BOS_TASK_ARG taskArg )
{
   ...
   if ( retval != BOS_STATUS_OK )
   {
      blogStrErr(( gBosBlog, "HUGE trouble in little China!!! Couldn't start bosTaskMain! taskid=0x%x", taskid ));
      return;
   }
   ...
   if ( retval != BOS_STATUS_OK )
   {
      ...
      blogStrErr(( gBosBlog, "HUGE trouble in big China!!! Couldn't start bosTaskMain! taskid=0x%x", taskid ));
      return;
   }
   ...
   {
      blogStrInfo(( gBosBlog, "bosTaskMain() task %s started suspended", GetTaskNameSelf() ));
      WaitForEvent( BOS_TASKSTART_EVENT );
      blogStrInfo(( gBosBlog, "BOS: bosTaskMain() task %s started (from suspended)", GetTaskNameSelf() ));
   }
   if ( initFnc )
   {
      (*initFnc)( taskArg );
...
PRIVATE void AppResetRootTask( BOS_TASK_ARG taskArg )
{
      ...
      blogStrAlways(( gBosBlog, "AppResetRootTask() - Is it morning already? Spawning app task..." ));
      ...
      */
      ...
      {
         blogStrAlways(( gBosBlog, "Exiting task aoRT" ));
         return;
      }

      blogStrAlways(( gBosBlog, "bosAppRootTask() - RESET NOW " ));
      ...
      blogStrAlways(( gBosBlog, "bosAppRootTask() - Reset completed, good night..." ));
...
PRIVATE void AppResetDetectionDisable( void )
{
   ...
   switch ( boszcb.resetDetectMode )
   {
      case BOS_RESETDETECT_DISABLED:
      {
         blogStrWarn(( gBosBlog, "AppResetDetectionDisable() - Reset detection is already disabled." ));
      }
      break;

      case BOS_RESETDETECT_ENABLED:
      {
         blogStrInfo(( gBosBlog, "AppResetDetectionDisable() - Reset detection has been disabled." ));
      }
      break;
