   ...
   switch (message)
   {
      case WM_COMMAND:
      {
         ...
         switch (wmId)
         {
            ...
            {
               char buf[400];
               sprintf( buf, "Win32 CMGR/CCTK 2.0 Application\n"
                             "Date:        %s\n",
                        __DATE__);
               MessageBox( hDlg, buf, "About CallManager", MB_OK|MB_ICONINFORMATION);
            }
            break;
            ...
            case IDM_LOGFILE:
            {
               ...
               else
               {
                  ...
                  LogFile = CreateFile("ccMgrLog.txt", GENERIC_READ|GENERIC_WRITE, 0, NULL, OPEN_ALWAYS,
...
LRESULT CALLBACK DlgMain(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
   ...
   switch (message)
   {
      case WM_INITDIALOG:
      {
         ...
         value = GetProfileInt( "CCMGR", "LogFile", 1 );
         if (value)
         {
      ...
      {    
         ...
         strcpy( sBuffer, 
                 ( GetMenuState( GetMenu(hWnd), IDM_LOGFILE, 0) & MF_CHECKED) ?
                   "1" : "0" );
         WriteProfileString( "CCMGR", "LogFile", sBuffer );
      }
      break;
      ...
      case WM_COMMAND:
      {
         ...
         else if ( appPicCtrlNotify(LOWORD(wParam), HIWORD(wParam)))
         {
            return TRUE;
         }
      }
