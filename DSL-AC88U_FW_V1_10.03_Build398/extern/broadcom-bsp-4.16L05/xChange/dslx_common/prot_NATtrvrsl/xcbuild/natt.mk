...
#
export NATT_BUILD_CONFIG_INCLUDE   := $(NATT_CONFIG_INCLUDE)
XCHG_C_GLOBAL_INCLUDE              += $(XCHG_PUBLIC_INC_assert)  \
                                      $(XCHG_PUBLIC_INC_blog)    \
                                      $(XCHG_PUBLIC_INC_bos)
export NATT_BUILD_IMPORTED_INCLUDE := $(XCHG_C_GLOBAL_INCLUDE)

include $(NATT_ROOT)/stacks/natt.mk
...
#
XCHG_SRC_FILES_$(XCHG_LIB_BASE) += $(NATT_BUILD_SRC_FILES)

XCHG_SRC_FILES += $(XCHG_SRC_FILES_$(XCHG_LIB_BASE))

vpath %.c $(NATT_BUILD_SRC_DIRS)

# $(error "TKEE SRCSDIRS" $(NATT_BUILD_SRC_DIRS))
...
#
XCHG_C_LOCAL_INCLUDE_$(XCHG_LIB_BASE) := $(NATT_BUILD_CONFIG_INCLUDE) \
                                         $(NATT_BUILD_LOCAL_INCLUDE)

#
# Add in private defs that are only needed when compiling this library.
#
XCHG_C_LOCAL_DEFS_$(XCHG_LIB_BASE) := $(NATT_BUILD_PRIVATE_DEFS)
