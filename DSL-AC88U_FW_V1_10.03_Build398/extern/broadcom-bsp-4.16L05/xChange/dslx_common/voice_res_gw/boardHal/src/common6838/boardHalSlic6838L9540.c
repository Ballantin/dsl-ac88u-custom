...
*****************************************************************************
*
*    Filename: boardHalSlic6838L9540.c
*
****************************************************************************
*    Description:
*
...
extern void slic6838L9540UpdateState( SLIC_6838_L9540_DRV *pDev );

/* ---- Public Variables ------------------------------------------------- */
/* ---- Private Constants and Types -------------------------------------- */
#define BOARD_HAL_LE9540_NUM_CH BOARD_HAL_6838_NUM_ENDPTS
...
*/
static SLICCFG_6838_L9540_CONNECT_INFO gSlicConnectInfo[BOARD_HAL_6838_NUM_ENDPTS] =
{
   /* Chan 0 */
   {
      XDRV_FALSE
...
static SLIC_6838_L9540_DRV gSlicDriver[ BOARD_HAL_6838_NUM_ENDPTS ];
...
static const BOARD_HAL_SLIC_FUNCS boardHal6838L9540DrvFuncs =
{
...
//static void boardHalApmGpioInit ( char* baseBoardIdStr, SLICCFG_6838_L9540_PIN_INFO * pPinInfo );
/* ---- Functions -------------------------------------------------------- */
...
int boardHalSlicInit9540( VRG_BOARDHAL_INIT_CFG *boardHalInitCfg, VRG_BOOL arg )
{
   ...
   if(    (0 == strcmp(baseBoardIdStr, "968380FHGU"))
       || (0 == strcmp(baseBoardIdStr, "968380FEHG"))
       || (0 == strcmp(baseBoardIdStr, "968380FGGU"))
       || (0 == strcmp(baseBoardIdStr, "968380FEGU"))
       || (0 == strcmp(baseBoardIdStr, "968380GERG"))
       || (0 == strcmp(baseBoardIdStr, "968380GWAN"))
       || (0 == strcmp(baseBoardIdStr, "968380LTE"))
       || (0 == strcmp(baseBoardIdStr, "968380EPRG"))
       || (0 == strcmp(baseBoardIdStr, "968380FFHG"))
       || (0 == strcmp(baseBoardIdStr, "968380FESFU"))
       || (0 == strcmp(baseBoardIdStr, "968385SFU"))
       || (0 == strcmp(baseBoardIdStr, "968385ESFU")) )
   {
      /* ADD pinmux for ZAR IF (TODO:Remove Magic Numbers) */
      set_pinmux( 0, PINMUX_ZAR_IF_D ); /* GPIO0 - D_ZAR_ENABLE */
      set_pinmux( 1, PINMUX_ZAR_IF_D ); /* GPIO1 - D_ZAR_SDOUT */
      set_pinmux( 2, PINMUX_ZAR_IF_D ); /* GPIO2 - D_ZAR_SDIN */
      set_pinmux( 3, PINMUX_ZAR_IF_D ); /* GPIO3 - D_ZAR_clk */

      /* LE9540_RESET_N tied to POR_RST_N */
      /* No need to explicitly reset 9540 */

   }
   else if(    (0 == strcmp(baseBoardIdStr, "968380FSV_G" ))
            || (0 == strcmp(baseBoardIdStr, "968380FSV_E" ))
            || (0 == strcmp(baseBoardIdStr, "968380FSV_GE")) )
   {

      /* ADD pinmux for ZAR IF (TODO:Remove Magic Numbers) */
//       set_pinmux( 38, PINMUX_ZAR_IF_C ); /* GPIO38 - C_ZAR_SDIN - RTL bug in A0, not connected */
      set_pinmux( 6, PINMUX_ZAR_IF_A ); /* GPIO6 - A_ZAR_SDIN */
      set_pinmux( 40, PINMUX_ZAR_IF_C ); /* GPIO40 - C_ZAR_ENABLE */
      set_pinmux( 41, PINMUX_ZAR_IF_C ); /* GPIO41 - C_ZAR_clk */
      set_pinmux( 42, PINMUX_ZAR_IF_C ); /* GPIO42 - C_ZAR_SDOUT */

      /* ADD pinmux for LE9540_RESET_N  (TODO:Remove Magic Numbers) */
      set_pinmux( 4, PINMUX_GPIO ); /* GPIO4 - LE9540_RST_N */

      /* Reset the 9540 */
      kerSysSetGpioDir(BP_GPIO_4_AL & BP_GPIO_NUM_MASK);
      kerSysSetGpio( BP_GPIO_4_AL & BP_GPIO_NUM_MASK, kGpioActive );      
      status = bosSleep( 10 );  
      kerSysSetGpio( BP_GPIO_4_AL & BP_GPIO_NUM_MASK, kGpioInactive );
   }
   else if(   0 == strcmp(baseBoardIdStr, "968385SV_G") || 
              0 == strcmp(baseBoardIdStr, "968385SV_E") ||
              0 == strcmp(baseBoardIdStr, "968380SV_G") ||
              0 == strcmp(baseBoardIdStr, "968380SV_E") ) 
   {
      /* ADD pinmux for ZAR IF (TODO:Remove Magic Numbers) */
      /* set_pinmux( 38, PINMUX_ZAR_IF_C ); *//* GPIO38 - C_ZAR_SDIN - RTL bug in A0, not connected, */
      /* on schematic, R142,R331 are populated with 0 ohm resistors. RD4 , RD19 , default resistors are removed, this brings GPIO6 as SDIN of Le9540 */
   ...
   boardHalInitCfg->pDrvSlicFuncs = &boardHal6838L9540DrvFuncs;
   ...
   for ( endpt = 0; endpt < numEndpt; endpt++ )
   {
      ...
      slic6838L9540Init( pDev, endpt, &gSlicConnectInfo[endpt], pApmDrv);
...
int boardHalSlicEnable9540( int lineId, VRG_BOOL enableSlic )
{
   ...
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicEnable: line out of range %d/%d", lineId, BOARD_HAL_6838_NUM_ENDPTS));
      return ( 1 );
   }
}
...
void boardHalSetAnalogLoopback9540( int lineId, VRG_BOOL enabled )
{
   ...
   SLIC_6838_L9540_DRV *pDev = &(gSlicDriver[lineId]);
   ...
   slic6838L9540SetTestLoad( (XDRV_SLIC*)&gSlicDriver[lineId], enabled );
      ...
      if( gSlicTaskHalted == XDRV_FALSE )
      {
         for ( endpt = 0; endpt < BOARD_HAL_6838_NUM_ENDPTS; endpt++ )
         {
            if ( gSlicDriver[endpt].bDrvEnabled )
            {
               ...
               slic6838L9540UpdateState( &(gSlicDriver[endpt]) );
            }
         }
      }
...
int boardHalSlicL9540UpdateLineStatus( int lineId, VRG_BOOL enableSlic, VRG_BOOL enableHvg )
{
   ...
   if (( lineId < 0 ) && ( lineId >= BOARD_HAL_6838_NUM_ENDPTS ))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicL9540UpdateLineStatus: line out of range %d/%d", lineId, BOARD_HAL_6838_NUM_ENDPTS ));
      return ( 1 );
   }
