 ...
 *    Description:
 *
 *      This file contains a hardware-specific (6328 platform) interface for
...
#include <ntr6328.h>          /* Implementation */
...
{
   ntr6328_getCounts( localCount, ntrCount );
   return;
}
...
{
   ntr6328_setFeedback( pcmStepsAdjust, isEnbl );
   return;
}
...
{
   ntr6328_print( numSamples, sampleIntervalMs );   
   return;
}
...
{
   ntr6328_getNdiv( ndivInt, ndivFrac ); 
   return; 
}
