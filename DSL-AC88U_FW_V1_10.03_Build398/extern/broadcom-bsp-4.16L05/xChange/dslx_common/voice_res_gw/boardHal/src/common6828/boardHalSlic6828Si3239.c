...
*****************************************************************************
*
*    Filename: boardHalSlic6828Si3239.c
*
****************************************************************************
*    Description:
*
...
extern void slic6828Si3239UpdateState( SLIC_6828_SI3239_DRV *pDev );
...
static SLIC_6828_SI3239_DRV gSlicDriver[ BOARD_HAL_6828_NUM_ENDPTS ];
static int ringDcOffset[ BOARD_HAL_6828_NUM_ENDPTS ];
...
*/
static SLICCFG_6828_SI3239_INFO gSlicCfgInfo[ BOARD_HAL_6828_NUM_ENDPTS ];
...
static const BOARD_HAL_SLIC_FUNCS boardHal6828Si3239DrvFuncs =
{
...
int boardHalSlicInit3239( VRG_BOARDHAL_INIT_CFG *boardHalInitCfg, VRG_BOOL arg )
{
   ...
   boardHalInitCfg->pDrvSlicFuncs = &boardHal6828Si3239DrvFuncs;
   ...
   for ( endpt = 0; endpt < BOARD_HAL_6828_NUM_ENDPTS; endpt++ )
   {
      ...
      slic6828Si3239Init( endpt, pDev, &gSlicCfgInfo[endpt], pApmDrv );

      /* Start the High voltage Generator */
      if (( result = hvg6828Start( endpt )))
      {
         VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "hvg6828Start failed, err code (%d)", result));
         /* Release mutex */
...
int boardHalSlicEnable3239( int lineId, VRG_BOOL enableSlic )
{
   ...
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicEnable3239: line out of range %d/%d", lineId, BOARD_HAL_6828_NUM_ENDPTS));
      return ( 1 );
   }
}
...
/*
*****************************************************************************
** FUNCTION:   boardHalSlicReadWriteMemory3239
**
...
int boardHalSlicReadWriteMemory3239( int endpt, int addr, int data, int write )
{
   ...
   if( write )
   {
      slic6828Si3239RegisterWrite( endpt, ( VRG_UINT8 )addr, ( VRG_UINT16 )data );
      VRG_LOG_DBG(( VRG_LOG_MOD_BOARDHAL, "Data value 0x%04X written to address %d.", ( VRG_UINT16 )data, ( VRG_UINT8 )addr ));
   }
   else
   {
      slic6828Si3239RegisterRead( endpt, ( VRG_UINT8 )addr, &value );
...
/*
*****************************************************************************
** FUNCTION:   boardHalSlicForceHvgShutdown3239
**
...
void boardHalSetAnalogLoopback3239( int lineId, VRG_BOOL enabled )
{
   ...
   if (enabled)
   {
      ...
      slic6828Si3239RegisterMaskWrite( lineId, SI3239_REG_LINEFEED_CONFIG_2,
...
/*
*****************************************************************************
** FUNCTION:   boardHalSlicSetRingParms3239
**
      ...
      if( gSlicTaskHalted == XDRV_FALSE )
      {
         for ( endpt = 0; endpt < BOARD_HAL_6828_NUM_ENDPTS; endpt++ )
         {
            if ( gSlicDriver[endpt].bDrvEnabled )
            {
               ...
               slic6828Si3239UpdateState( &(gSlicDriver[endpt]) );
            }
         }
      }
...
int boardHalSlicSi3239UpdateLineStatus( int lineId, VRG_BOOL enableSlic, VRG_BOOL enableHvg )
{
   ...
   if (( lineId < 0 ) && ( lineId >= BOARD_HAL_6828_NUM_ENDPTS ))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicSi3239UpdateLineStatus: line out of range %d/%d", lineId, BOARD_HAL_6828_NUM_ENDPTS ));
      return ( 1 );
   }
