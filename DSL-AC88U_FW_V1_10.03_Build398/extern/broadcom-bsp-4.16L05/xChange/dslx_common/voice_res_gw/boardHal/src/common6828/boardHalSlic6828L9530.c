...
*****************************************************************************
*
*    Filename: boardHalSlic6828L9530.c
*
****************************************************************************
*    Description:
*
...
extern void slic6828L9530UpdateState( SLIC_6828_L9530_DRV *pDev );
...
static SLIC_6828_L9530_DRV gSlicDriver[ BOARD_HAL_6828_NUM_ENDPTS ];
static VRG_BOOL slicEnabled[ BOARD_HAL_6828_NUM_ENDPTS ];
static VRG_BOOL slicInit[ BOARD_HAL_6828_NUM_ENDPTS ] = { VRG_FALSE, VRG_FALSE };
static VRG_BOOL hvgEnabled[ BOARD_HAL_6828_NUM_ENDPTS ];
...
static const BOARD_HAL_SLIC_FUNCS boardHal6828L9530DrvFuncs =
{
...
/*
** 6828 <-> L9530 SLIC connectivity configuration structure.
*/
static const SLICCFG_6828_L9530_PIN_INFO gSlicPinInfo6828HGW[BOARD_HAL_6828_NUM_ENDPTS] =
{
   /* Chan 0 */
   {
      36,   //B0 pin
...
static const SLICCFG_6828_L9530_PIN_INFO gSlicPinInfo6828EGG[BOARD_HAL_6828_NUM_ENDPTS] =
{
   /* Chan 0 */
   {
      36,   //B0 pin
...
/* Mapping from BoardId to PIN_INFO structure */
typedef struct
{
   char boardId[20];
   SLICCFG_6828_L9530_PIN_INFO * pPinInfoStruct;
} BOARDID_PININFO_MAP;

BOARDID_PININFO_MAP boardIdPinInfoMap [] = 
{
   { "96828HGW"    , (SLICCFG_6828_L9530_PIN_INFO *)&gSlicPinInfo6828HGW[0]   },
   { "96828HGW_F42", (SLICCFG_6828_L9530_PIN_INFO *)&gSlicPinInfo6828HGW[0]   },
   { "96828HGW_G42", (SLICCFG_6828_L9530_PIN_INFO *)&gSlicPinInfo6828HGW[0]   },
   { "96828HGW_F21", (SLICCFG_6828_L9530_PIN_INFO *)&gSlicPinInfo6828HGW[0]   },
   { "96828HGW_G21", (SLICCFG_6828_L9530_PIN_INFO *)&gSlicPinInfo6828HGW[0]   },                                                
   { "96828EGG",     (SLICCFG_6828_L9530_PIN_INFO *)&gSlicPinInfo6828EGG[0]   },
   { "96828HGW_G21", (SLICCFG_6828_L9530_PIN_INFO *)NULL                      },                                                
};
...
static void boardHalApmGpioInit ( char* baseBoardIdStr, SLICCFG_6828_L9530_PIN_INFO * pPinInfo );
static SLICCFG_6828_L9530_PIN_INFO * boardHalMapBoardIdToPinInfoStruct( char* baseBoardIdStr);
...
int boardHalSlicInit9530( VRG_BOARDHAL_INIT_CFG *boardHalInitCfg, VRG_BOOL arg )
{
   ...
   SLICCFG_6828_L9530_PIN_INFO * pPinInfoStruct = NULL;
   ...
   boardHalInitCfg->pDrvSlicFuncs = &boardHal6828L9530DrvFuncs;


   /* TODO- ADD M and USE MAP FUNCTION PER ENDOINT */
   ...
   for ( endpt = 0; endpt < BOARD_HAL_6828_NUM_ENDPTS; endpt++ )
   {
      ...
      {
         /* Get Pin info structure */
         pPinInfoStruct = boardHalMapBoardIdToPinInfoStruct( baseBoardIdStr );
         
         if( pPinInfoStruct )
         {
            /* Initialize 9530 for the 6828 */
            boardHalApmGpioInit( baseBoardIdStr,  (SLICCFG_6828_L9530_PIN_INFO*)&pPinInfoStruct[endpt] );
            slic6828L9530Init( pDev, endpt, (SLICCFG_6828_L9530_PIN_INFO*)&pPinInfoStruct[endpt], pApmDrv);            
         }
         else
         {
            VRG_LOG_ERROR(( VRG_LOG_MOD_BOARDHAL, "Failed to obtain Pin info struct !!!" ));
            /* Release mutex */
      ...
      {
         VRG_LOG_ERROR(( VRG_LOG_MOD_BOARDHAL, "Failed to obtain Base board id string !!!" ));
         /* Release mutex */
         bosMutexRelease( &gSlicBosMutex );
         return ( result );
      }
      ...
      if ((result = hvg6828Start(endpt)))
      {
         VRG_LOG_ERROR(( VRG_LOG_MOD_BOARDHAL, "hvg6828Start failed, err code (%d)", result));
         /* Release mutex */
         bosMutexRelease( &gSlicBosMutex );
         return ( result );
      }
...
{
   SLICCFG_6828_L9530_PIN_INFO * pPinInfoStruct = NULL;
   BOARDID_PININFO_MAP * pPinInfoMap =   &boardIdPinInfoMap[0];
   
   if( baseBoardIdStr )
   {
      while( pPinInfoMap->pPinInfoStruct != NULL )
      {
         if ( strncmp(  baseBoardIdStr, pPinInfoMap->boardId, strlen(baseBoardIdStr) ) == 0 ) 
         {
            pPinInfoStruct = pPinInfoMap->pPinInfoStruct;
            break;
         }
   ...
   if(  pPinInfoStruct == NULL )
   {
      VRG_LOG_ERROR(( VRG_LOG_MOD_BOARDHAL, "Failed to find matching pinInfo struct for boardId %s", baseBoardIdStr));           
   }
   
   return pPinInfoStruct;
}

static void boardHalApmGpioInit ( char* baseBoardIdStr, SLICCFG_6828_L9530_PIN_INFO * pPinInfo )
{
   
   XCHG_ASSERT( baseBoardIdStr );
   XCHG_ASSERT( pPinInfo );

   /* This is already taken care of in  kernel/linux/arch/mips/bcm963xx/setup.c bcm6828_hw_init()     */
#if 0   
   XDRV_UINT32 * pPin = NULL;
   int i;
   
   /* We need to claim a few of the GPIOs used by the pPinInfo struct for use by the peripheral block */

   /* 96828HGW specific GPIO configuration */
   if( !strcmp(baseBoardIdStr, "96828HGW") )
   {
      pPin = (XDRV_UINT32 *)pPinInfo;      
      
      for( i=0; i< sizeof(SLICCFG_6828_L9530_PIN_INFO)/sizeof(XDRV_UINT32); i++ )
      {
         if ( 41 > *pPin && *pPin > 36 )
         {
            /* This is code reserves GPIOs in the range 37-40 for use by the *
             * Periph block and disables internal APM control of these GPIOS */
            GPIO->GPIOCtrl &=  ~(GPIO_NUM_TO_MASK(*pPin) >> GPIO_CTRL_APM_CTRL_OVERRIDE_SHIFT );         	  
         }
         else if ( 53 > *pPin && *pPin > 40 )
         {            
            /* This is code reserves GPIOs in the range 52-41 for use by the  *
             * Periph block and disables internal EPON control of these GPIOS */
            GPIO->GPIOCtrl |=  (GPIO_NUM_TO_MASK(*pPin) >> GPIO_CTRL_EPON_CTRL_OVERRIDE_SHIFT );	 	 
         }
         pPin++;
      }                  
   }      
...
int boardHalSlicEnable9530( int lineId, VRG_BOOL enableSlic )
{
   ...
   if ((lineId >= 0)&&(lineId < BOARD_HAL_6828_NUM_ENDPTS))
   {
   ...
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicEnable: line out of range %d/%d", lineId, BOARD_HAL_6828_NUM_ENDPTS));
      return ( 1 );
   }
}
...
int boardHalSlicForceHvgShutdown9530( void )
{
   ...
   for ( endpt = 0; endpt < BOARD_HAL_6828_NUM_ENDPTS; endpt++ )
   {
      if ( hvg6828IsEnabled( endpt ) )
      {
         if (hvg6828Stop(endpt))
         {
            VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicForceHvgShutdown: chan %d hvg6828Stop failed !!!", endpt));
            return ( -1 );
         }
         else
         {
...
XDRV_SLIC* boardHalSlicGetDriver9530( int chan )
{
   XCHG_ASSERT( ( 0 <= chan ) && ( chan < BOARD_HAL_6828_NUM_ENDPTS ) );
...
void boardHalSetAnalogLoopback9530( int lineId, VRG_BOOL enabled )
{
   ...
   if (lineId >= BOARD_HAL_6828_NUM_ENDPTS)
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "Incorrect chan id %d !!!", lineId));
      return;
   }

   SLIC_6828_L9530_DRV *pDev = &(gSlicDriver[lineId]);
      ...
      if( gSlicTaskHalted == XDRV_FALSE )
      {
         for ( endpt = 0; endpt < BOARD_HAL_6828_NUM_ENDPTS; endpt++ )
         {
            if ( gSlicDriver[endpt].bDrvEnabled )
            {
               ...
               slic6828L9530UpdateState( &(gSlicDriver[endpt]) );
            }
         }
      }
...
int boardHalSlicL9530UpdateLineStatus( int lineId, VRG_BOOL enableSlic, VRG_BOOL enableHvg )
{
   ...
   if (( lineId < 0 ) && ( lineId >= BOARD_HAL_6828_NUM_ENDPTS ))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicL9530UpdateLineStatus: line out of range %d/%d", lineId, BOARD_HAL_6828_NUM_ENDPTS ));
      return ( 1 );
   }
