...
*****************************************************************************
*
*    Filename: boardHalSlic6816Si3239.c
*
****************************************************************************
*    Description:
*
...
extern void slic6816Si3239UpdateState( SLIC_6816_SI3239_DRV *pDev );
...
static SLIC_6816_SI3239_DRV gSlicDriver[ BOARD_HAL_6816_NUM_ENDPTS ];
static int ringDcOffset[ BOARD_HAL_6816_NUM_ENDPTS ];
static VRG_BOOL slicEnabled[ BOARD_HAL_6816_NUM_ENDPTS ];
static VRG_BOOL slicInit[ BOARD_HAL_6816_NUM_ENDPTS ] = { VRG_FALSE, VRG_FALSE };
static VRG_BOOL hvgEnabled[ BOARD_HAL_6816_NUM_ENDPTS ];

/*
** 6816 <-> SI3239 SLIC configuration structure.
*/
static SLICCFG_6816_SI3239_INFO gSlicCfgInfo[ BOARD_HAL_6816_NUM_ENDPTS ];
...
static const BOARD_HAL_SLIC_FUNCS boardHal6816Si3239DrvFuncs =
{
...
int boardHalSlicInit3239( VRG_BOARDHAL_INIT_CFG *boardHalInitCfg, VRG_BOOL arg )
{
   ...
   boardHalInitCfg->pDrvSlicFuncs = &boardHal6816Si3239DrvFuncs;
   ...
   for ( endpt = 0; endpt < BOARD_HAL_6816_NUM_ENDPTS; endpt++ )
   {
      ...
      slic6816Si3239Init( endpt, pDev, &gSlicCfgInfo[endpt], pApmDrv );

      /* Start the High voltage Generator */
      if (( result = hvg6816Start( endpt )))
      {
         VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "hvg6816Start failed, err code (%d)", result));
         /* Release mutex */
...
int boardHalSlicEnable3239( int lineId, VRG_BOOL enableSlic )
{
   ...
   if ((lineId >= 0)&&(lineId < BOARD_HAL_6816_NUM_ENDPTS))
   {
   ...
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicEnable3239: line out of range %d/%d", lineId, BOARD_HAL_6816_NUM_ENDPTS));
      return ( 1 );
   }
}
...
int boardHalSlicReadWriteMemory3239( int endpt, int addr, int data, int write )
{
   ...
   if( write )
   {
      slic6816Si3239RegisterWrite( endpt, ( VRG_UINT8 )addr, ( VRG_UINT16 )data );
      VRG_LOG_DBG(( VRG_LOG_MOD_BOARDHAL, "Data value 0x%04X written to address %d.", ( VRG_UINT16 )data, ( VRG_UINT8 )addr ));
   }
   else
   {
      slic6816Si3239RegisterRead( endpt, ( VRG_UINT8 )addr, &value );
...
int boardHalSlicForceHvgShutdown3239( void )
{
   ...
   for ( endpt = 0; endpt < BOARD_HAL_6816_NUM_ENDPTS; endpt++ )
   {
      if ( hvg6816IsEnabled( endpt ) )
      {
         if ( hvg6816Stop( endpt ) )
         {
            VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicForceHvgShutdown: chan %d hvg6816Stop failed !!!", endpt));
            return ( -1 );
         }
         else
         {
...
XDRV_SLIC* boardHalSlicGetDriver3239( int chan )
{
   XCHG_ASSERT( ( 0 <= chan ) && ( chan < BOARD_HAL_6816_NUM_ENDPTS ) );
...
void boardHalSetAnalogLoopback3239( int lineId, VRG_BOOL enabled )
{
   ...
   if (lineId >= BOARD_HAL_6816_NUM_ENDPTS)
   {
   ...
   if (enabled)
   {
      ...
      slic6816Si3239RegisterMaskWrite( lineId, SI3239_REG_LINEFEED_CONFIG_2,
      ...
      if( gSlicTaskHalted == XDRV_FALSE )
      {
         for ( endpt = 0; endpt < BOARD_HAL_6816_NUM_ENDPTS; endpt++ )
         {
            if ( gSlicDriver[endpt].bDrvEnabled )
            {
               ...
               slic6816Si3239UpdateState( &(gSlicDriver[endpt]) );
            }
         }
      }
...
int boardHalSlicSi3239UpdateLineStatus( int lineId, VRG_BOOL enableSlic, VRG_BOOL enableHvg )
{
   ...
   if (( lineId < 0 ) && ( lineId >= BOARD_HAL_6816_NUM_ENDPTS ))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "boardHalSlicSi3239UpdateLineStatus: line out of range %d/%d", lineId, BOARD_HAL_6816_NUM_ENDPTS ));
      return ( 1 );
   }
