...
/* ---- Private Variables ------------------------------------------------ */
static APM_6838_DRV gApmDriver;
                     ...
                     int packetRateSamples, int useRingGen )
{
   ...
   */
   ...
   if ( devType == BP_VD_NONE )
   {
      /* Voice device is not found, use values corresponding to 9540 as default 
       * Note: No device programming here. 9540 is just a placeholder for initialization
   ...
   apmCfg.pFlexiCalc = (APM6838_FLEXICALC_CFG*)dynFlexicalc;
   ...
   ** so use endpoint 0 when calling endptProvGetValue */
   ...
   else
   {
      ...
      ** Externally provisioned flexicalc settings are NOT used. */
      ...
      apmCfg.pFlexiCalc = flexiCalcGetCfg6838( boardHalInitCfg->country, 
   ...
   /* set APM channel swap flag */
   gApmDriver.channelSwap = boardHalInitCfg->apmChannelSwap;
   
   /* Initialize the APM module */
   if (( result = apm6838_init( &gApmDriver, PERF, &apmCfg )))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "apm6838_init failed, err code (%d)", result ));
      return ( result );
   }

   /* Initialize the High Voltage Generator module */
   if (( result = hvg6838Init( (XDRV_APM*)&gApmDriver )))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "hvg6838Init failed, err code (%d)", result ));
      return ( result );
   }
...
void boardHalApmSyncClkSource( int clkSource )
{
   ...
   apm6838SetClkSource( pApmDriver, clkSource );
}
...
void boardHalApmFlexicalcReinit( VRG_BOARDHAL_INIT_CFG *boardHalInitCfg )
{
   switch( boardHalInitCfg->slicType )
   {
      ...
      case XDRV_SLIC_L9541:
      {
         ...
         gApmDriver.Cfg.pFlexiCalc = flexiCalcGetCfg6838( boardHalInitCfg->country,
   ...
   apm6838_flexicalcConfig( gApmDriver.pApm, gApmDriver.Cfg.pFlexiCalc,&gApmDriver );
}
/*
*****************************************************************************
** FUNCTION:   ntrInit
...
{   
    APM_6838_DRV *pApmDrv;
    pApmDrv = ( APM_6838_DRV *)boardHalApmGetDriver( 0 );
    return pApmDrv->channelSwap;
}
