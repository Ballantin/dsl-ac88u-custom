...
/* ---- Private Variables ------------------------------------------------ */
static APM_6818_DRV gApmDriver;
...
static XDRV_SINT32 boardHalGetHvgSubType( void );
/* ---- Functions -------------------------------------------------------- */
                     ...
                     int packetRateSamples, int useRingGen )
{
   ...
   */
   ...
   while ( voiceParams->voiceDevice[deviceId].voiceDeviceType != BP_VD_NONE )
   {
      switch ( voiceParams->voiceDevice[deviceId].voiceDeviceType )
      {
         case BP_VD_ZARLINK_9530:
         {
            ...
            apmCfg.hvgSubType = boardHalGetHvgSubType();
         ...
         {
            boardHalInitCfg->hvgDesignType = XDRV_APM_HVG_DESIGN_BUCKBOOST;
            boardHalInitCfg->slicType = XDRV_SLIC_L9540;
            apmCfg.hvgType = XDRV_APM_HVG_DESIGN_BUCKBOOST;
            apmCfg.hvgSubType = boardHalGetHvgSubType();
            devType = BP_VD_ZARLINK_9540;
         }
         break;
         ...
         {
            boardHalInitCfg->hvgDesignType = XDRV_APM_HVG_DESIGN_BUCKBOOST;
            boardHalInitCfg->slicType = XDRV_SLIC_L9541;
            apmCfg.hvgType = XDRV_APM_HVG_DESIGN_BUCKBOOST;
            apmCfg.hvgSubType = boardHalGetHvgSubType();
            devType = BP_VD_ZARLINK_9541;
         }
         break;
   ...
   apmCfg.pFlexiCalc = (APM6818_FLEXICALC_CFG*)dynFlexicalc;
   ...
   ** so use endpoint 0 when calling endptProvGetValue */
   ...
   else
   {
      ...
      ** Externally provisioned flexicalc settings are NOT used. */
      ...
      apmCfg.pFlexiCalc = flexiCalcGetCfg6818( boardHalInitCfg->country, 
   ...
   /* Fill in the HVG input supply voltage */
   apmCfg.inputSupplyVoltage = boardHalInitCfg->inputSupplyVoltage;

   /* Initialize the APM module */
   if (( result = apm6818_init( &gApmDriver, PERF, &apmCfg )))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "apm6818_init failed, err code (%d)", result ));
      return ( result );
   }

   /* Initialize the High Voltage Generator module */
   if (( result = hvg6818Init( (XDRV_APM*)&gApmDriver )))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "hvg6818Init failed, err code (%d)", result ));
      return ( result );
   }
...
void boardHalApmSyncClkSource( int clkSource )
{
   ...
   apm6818SetClkSource( pApmDriver, clkSource );
}
...
void boardHalApmFlexicalcReinit( VRG_BOARDHAL_INIT_CFG *boardHalInitCfg )
{
   switch( boardHalInitCfg->slicType )
   {
      case XDRV_SLIC_L9530:
      {
         ...
         gApmDriver.Cfg.pFlexiCalc = flexiCalcGetCfg6818( boardHalInitCfg->country,
   ...
   apm6818_flexicalcConfig( gApmDriver.pApm, gApmDriver.Cfg.pFlexiCalc );
}
/*
*****************************************************************************
** FUNCTION:   ntrInit
...
/*
*****************************************************************************
** FUNCTION:   boardHalGetHvgSubType
**
** PURPOSE:    Get board HVG design sub type 
**
** PARAMETERS: none
**
...
static XDRV_SINT32 boardHalGetHvgSubType( void )
{
   ...
   if( 0 == strcmp(baseBoardIdStr, "96818GRRG2LBOSA") ) /* this board populated SLIC Le9540 and its HVG*/
   {
      return HVG_DESIGN_GRRG2LBOSA;
   }
   else
   {
      return HVG_DESIGN_GENERAL;
   }
}
...
**
** PURPOSE:    Determines if chanenel swap is set in the APM
**
** PARAMETERS: none
**
...
{   
    /* no board design as channel swap so far */
    return XDRV_FALSE;
}
