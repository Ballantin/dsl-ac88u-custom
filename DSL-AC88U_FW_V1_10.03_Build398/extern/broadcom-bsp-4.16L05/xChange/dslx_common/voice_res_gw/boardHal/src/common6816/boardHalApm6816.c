...
/* ---- Private Variables ------------------------------------------------ */
static APM_6816_DRV gApmDriver;
                     ...
                     int packetRateSamples, int useRingGen )
{
   ...
   ** so use endpoint 0 when calling endptProvGetValue */
   ...
   if (( result = apm6816_init( &gApmDriver, PERF, &apmCfg )))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "apm6816_init failed, err code (%d)", result ));
      return ( result );
   }

   /* Initialize the High Voltage Generator module */
   if (( result = hvg6816Init( (XDRV_APM*)&gApmDriver )))
   {
      VRG_LOG_WARN(( VRG_LOG_MOD_BOARDHAL, "hvg6816Init failed, err code (%d)", result ));
      return ( result );
   }
...
void boardHalApmSyncClkSource( int clkSource )
{
   ...
   apm6816SetClkSource( pApmDriver, clkSource );
}
...
void boardHalApmFlexicalcReinit( VRG_BOARDHAL_INIT_CFG *boardHalInitCfg )
{
   switch( boardHalInitCfg->slicType )
   {
      case XDRV_SLIC_L9530:
      {
         ...
         gApmDriver.Cfg.pFlexiCalc = flexiCalcGetCfg6816( boardHalInitCfg->country,
                                                          gApmDriver.Cfg.sampleRate * 1000, BP_VD_ZARLINK_9530 );
      }
      break;
      ...
      case XDRV_SLIC_SI3239:
      {
         /* Fill in the FlexiCalc config parameters */
         gApmDriver.Cfg.pFlexiCalc = flexiCalcGetCfg6816( boardHalInitCfg->country,
                                                          gApmDriver.Cfg.sampleRate * 1000, BP_VD_SILABS_3239 );
      }
      break;
   ...
   apm6816_flexicalcConfig( gApmDriver.pApm, gApmDriver.Cfg.pFlexiCalc );
}
