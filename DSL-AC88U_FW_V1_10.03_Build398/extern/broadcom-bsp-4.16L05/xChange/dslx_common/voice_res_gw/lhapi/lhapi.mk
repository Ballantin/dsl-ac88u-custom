###########################################################################
#
# This makefile is used to build the lhapi (low-level hapi) module.
#
###########################################################################
...
LHAPI_ROOT     := $(VRG_DIR_$(XCHG_LIB_BASE))


#
# List of all source files. Do not specify directories, we use vpath below.
#
XCHG_SRC_FILES_$(XCHG_LIB_BASE) := $(notdir $(wildcard $(LHAPI_ROOT)/src/*.c))

ifeq ($(strip $(BRCM_NONDIST_BUILD)),1)
XCHG_SRC_FILES_$(XCHG_LIB_BASE) := $(filter-out lhapiIpc.c,$(XCHG_SRC_FILES_$(XCHG_LIB_BASE)))
endif								   

ifeq ($(XCHG_OS),LinuxKernel)
XCHG_SRC_FILES_$(XCHG_LIB_BASE) := $(filter-out lhapiThreadDispatchHostOnly.c,$(XCHG_SRC_FILES_$(XCHG_LIB_BASE)))
else
XCHG_SRC_FILES_$(XCHG_LIB_BASE) := $(filter-out lhapiThreadDispatchHostOnlyLinux.c,$(XCHG_SRC_FILES_$(XCHG_LIB_BASE)))
endif
