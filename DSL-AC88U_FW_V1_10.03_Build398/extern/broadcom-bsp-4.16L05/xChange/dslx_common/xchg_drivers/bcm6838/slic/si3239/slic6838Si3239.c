...
#include "slic6838Si3239.h"
#include "slic6838Si3239Defs.h"
...
static void slic6838Si3239ModeControl( XDRV_SLIC *pDrv, XDRV_SLIC_MODE mode );
static void slic6838Si3239LedControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static void slic6838Si3239PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slic6838Si3239IsOffhook( XDRV_SLIC *pDrv );
static int slic6838Si3239SetRingParms( XDRV_SLIC *pDrv, int ringFrequency, int ringWaveshape,
                                       int ringVoltage, int ringOffset, int ringOffsetCal );
static int slic6838Si3239GetRingParms( XDRV_SLIC *pDrv, int *ringFrequency, int *ringWaveshape,
                                       int *ringVoltage, int *ringOffset, int *ringOffsetCal );
static int slic6838Si3239SetBoostedLoopCurrent( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static int slic6838Si3239SetPowerSource( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slic6838Si3239GetOverCurrentStatus( XDRV_SLIC *pDrv );
static XDRV_SINT16 slic6838Si3239GetDlp( XDRV_SLIC *pDrv );
static void slic6838Si3239ProcessEvents( XDRV_SLIC *pDrv );
static XDRV_UINT32 slic6838Si3239Probe( XDRV_SLIC *pDrv, XDRV_UINT32 deviceId, XDRV_UINT32 chan, XDRV_UINT32 reg,
                                      XDRV_UINT32 regSize, void* value, XDRV_UINT32 valueSize, XDRV_UINT32 indirect, 
                                      XDRV_UINT8 set );
static int slic6838Si3239GetSlicParms( XDRV_SLIC *pDrv, int *phaseReversal, int *loopCurrent, int *powerSource, int *slicMode );
/*
...
static const XDRV_SLIC_FUNCS slic6838Si3239DrvFuncs =
{
   slic6838Si3239ModeControl,
   slic6838Si3239LedControl,
   slic6838Si3239PhaseReversalControl,
   slic6838Si3239IsOffhook,
   slic6838Si3239SetRingParms,
   slic6838Si3239GetRingParms,
   slic6838Si3239SetBoostedLoopCurrent,
   slic6838Si3239SetPowerSource,
   slic6838Si3239GetOverCurrentStatus,
   slic6838Si3239GetDlp,
   slic6838Si3239ProcessEvents,
   slic6838Si3239Probe,
   slic6838Si3239GetSlicParms
};

/* Private Functions */
static void OpenSlic( SLIC_6838_SI3239_DRV *pDev );
static void CloseSlic( SLIC_6838_SI3239_DRV *pDev );
static void InitSlicRegs( SLIC_6838_SI3239_DRV *pDev );
static void setSlicState( SLIC_6838_SI3239_DRV *pDev, XDRV_SLIC_MODE mode );
...
*****************************************************************************
*/
void slic6838Si3239Init
(
   int                   chan,
   SLIC_6838_SI3239_DRV *pDev,
   const SLICCFG_6838_SI3239_INFO *pSlicCfgInfo,
   XDRV_APM             *pApmDrv
)
{
   int status;
   ...
   memset( pDev, 0, sizeof( SLIC_6838_SI3239_DRV ) );

   /* Initialize the SLIC information for each channel */
   pDev->chan = chan;
   pDev->pDrvFuncs = &slic6838Si3239DrvFuncs;
...
*****************************************************************************
*/
void slic6838Si3239Deinit( SLIC_6838_SI3239_DRV *pDev )
{
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239IsOffhook
**
...
static XDRV_BOOL slic6838Si3239IsOffhook( XDRV_SLIC *pDrv )
{
   SLIC_6838_SI3239_DRV *pDev = (SLIC_6838_SI3239_DRV *)pDrv;
   int                   chan = pDev->chan;
   XDRV_UINT16           rc;
   ...
   rc = MSPI_Read( pDev->spiDevId,  chan, SI3239_REG_DEVICE_STATUS );
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239ModeControl
**
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239LedControl
**
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239PhaseReversalControl
**
...
static void slic6838Si3239PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value )
{
   ...
   {
      slic6838Si3239ModeControl( pDrv, pDev->slicMode );
   }
}

/*
*****************************************************************************
** FUNCTION:   slic6838Si3239SetRingParms
**
                                      ...
                                      int ringVoltage, int ringOffset, int ringOffsetCal )
{
   ...
   slicControlReg = MSPI_Read( pDev->spiDevId,  pDev->chan, SI3239_REG_LINEFEED_CONFIG_2 );
   slicControlReg &= ~SI3239_RSPOL_MASK;
   if ( ringOffset < 0 )
   {
      slicControlReg |= SI3239_RSPOL_REVERSED;
   }
   MSPI_Write( pDev->spiDevId,  pDev->chan, SI3239_REG_LINEFEED_CONFIG_2, slicControlReg );
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239GetRingParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239GetSlicParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239SetBoostedLoopCurrent
**
...
static int slic6838Si3239SetBoostedLoopCurrent( XDRV_SLIC *pDrv, XDRV_UINT16 value )
{
   ...
   */
   if ( value )
   {
      MSPI_Mask_Write( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_2,
...
static void OpenSlic( SLIC_6838_SI3239_DRV *pDev )
{
   ...
   MSPI_Mask_Write( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_1,
...
static void InitSlicRegs( SLIC_6838_SI3239_DRV *pDev )
{
   ...
   MSPI_Write( pDev->spiDevId,  chan, SI3239_REG_DEVICE_STATUS, 0x0000 );
   ...
   */
   if(( pDev->chan % 2 ) == 0 )
   {
      MSPI_Write( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_1, 0x0000 );
   }
   MSPI_Write( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_2, SI3239_RINGTRIP_DETECT_THRESH_70 );
   MSPI_Write( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_3, 0x0000 );
}
...
static void setSlicState( SLIC_6838_SI3239_DRV *pDev, XDRV_SLIC_MODE mode )
{
   ...
   /* Retrieve old data */
   slicStateReg = MSPI_Read( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_1 );
   slicControlReg = MSPI_Read( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_3 );
   ...
   MSPI_Write( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_1, slicStateReg );

   /* If necessary update the temperature control data to the SLIC */
   if( requiredTemp != currentTemp )
   {
      MSPI_Write( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_3, slicControlReg );
   }
}
#endif 
/*
...
/*
******************************************************************************
** FUNCTION:  slic6838Si3239RegisterRead
**
** PURPOSE:   Read from a memory address via MSPI
**
** PARAMETERS:
...
*****************************************************************************
*/
void slic6838Si3239RegisterRead( SLIC_6838_SI3239_DRV *pDev, XDRV_UINT8 addr, XDRV_UINT16 *data )
{
   int chan = pDev->chan;;   
...
/*
******************************************************************************
** FUNCTION:  slic6838Si3239RegisterWrite
**
** PURPOSE:   Write to a memory address via MSPI
**
** PARAMETERS:
...
*****************************************************************************
*/
void slic6838Si3239RegisterWrite( SLIC_6838_SI3239_DRV *pDev, XDRV_UINT8 addr, XDRV_UINT16 data )
{
   int chan = pDev->chan;   
...
/*
******************************************************************************
** FUNCTION:  slic6838Si3239RegisterMaskWrite
**
** PURPOSE:   Write to the masked location of a memory address via MSPI
**            and verify the write if requested to
**
...
*****************************************************************************
*/
void slic6838Si3239RegisterMaskWrite( SLIC_6838_SI3239_DRV *pDev, XDRV_UINT8 addr, XDRV_UINT16 data, XDRV_UINT16 mask, int verify )
{
   int chan = pDev->chan;   
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239SetPowerSource
**
...
static int slic6838Si3239SetPowerSource( XDRV_SLIC *pDrv, XDRV_UINT16 value )
{
   ...
   if ( pDev->bEnhancedControl )
   {
      if ( value & 0xF0 )
      {
         configRegister = MSPI_Read( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_2 );
         configRegister |= ( SI3239_CHAN0_ENHANCED_CONTROL_ON | SI3239_CHAN1_ENHANCED_CONTROL_ON );
         MSPI_Write( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_2, configRegister );
      }
      else if ( value == 0 )
      {
         configRegister = MSPI_Read( pDev->spiDevId,  chan, SI3239_REG_LINEFEED_CONFIG_2 );
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239GetOverCurrentStatus
**
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239SetSpecialMode
**
...
*****************************************************************************
*/
int slic6838Si3239SetSpecialMode( SLIC_6838_SI3239_DRV *pDev, SLIC_6838_SI3239_SPECIAL_MODE mode, XDRV_BOOL enable )
{
   switch( mode )
   {
      case SLIC_6838_SI3239_SPECIAL_DISABLE_RING_TRANISTION_MODE:
      {
...
/*
*****************************************************************************
** FUNCTION:   slic6838Si3239UpdateState
**
...
*****************************************************************************
*/
void slic6838Si3239UpdateState( SLIC_6838_SI3239_DRV *pDev )
{
   if( pDev->pendingSlicTime > 0 )
   {
      ...
      else
      {
         ...
         /* Update SLIC state */
         setSlicState( pDev, pDev->pendingSlicState );
