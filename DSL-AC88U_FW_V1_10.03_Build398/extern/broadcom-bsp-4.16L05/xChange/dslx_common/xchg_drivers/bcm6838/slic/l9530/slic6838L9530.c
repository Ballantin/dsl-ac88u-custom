...
/* Private Functions */
static void OpenSlic( SLIC_6838_L9530_DRV *pDev );
static void CloseSlic( SLIC_6838_L9530_DRV *pDev );
static void SetIOCtrl( SLIC_6838_L9530_DRV *pDev, int mode );
static void SetState( SLIC_6838_L9530_DRV *pDev, int mode );
static XDRV_APM_HVG_VOLTAGE GetVoltage( XDRV_UINT16 mode );
static void SetupGpioControl( SLIC_6838_L9530_DRV *pDev, const SLICCFG_6838_L9530_PIN_INFO* pSlicPinInfo );
static void SetupGpioRegs( XDRV_UINT32 pin, XDRV_UINT32* ctrlp, XDRV_UINT32* datap, XDRV_UINT32* mask );
static XDRV_UINT64 ConvertBpinsToMask( SLICCFG_6838_L9530_PIN_INFO* pDev, XDRV_UINT32 value );
...
{
   memset( pDev, 0, sizeof( SLIC_6838_L9530_DRV ) );
...
static XDRV_BOOL slicIsOffhook( XDRV_SLIC *pDrv )
{
   SLIC_6838_L9530_DRV *pDev  = (SLIC_6838_L9530_DRV *)pDrv;
...
static void OpenSlic( SLIC_6838_L9530_DRV *pDev )
{
   ...
   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "OpenSlic pre : *slicCfgInfo.slicIoStateMask = 0x%.8x", (unsigned int)pDev->slicCfgInfo.slicIoStateMask));
   
   /* Enable B0-B2 as output pins - Add 6838 support */
   XCHG_ASSERT(0);
//    GPIO->GPIODir |=(((XDRV_UINT64)1<<(pDev->slicCfgInfo.slicIoStateMask & 0xFF)) | 
//   					 				((XDRV_UINT64)1<<((pDev->slicCfgInfo.slicIoStateMask>>8) & 0xFF)) | 
//   					 				((XDRV_UINT64)1<<((pDev->slicCfgInfo.slicIoStateMask>>16) & 0xFF)));
...
static void SetIOCtrl( SLIC_6838_L9530_DRV *pDev, int value )
{
   XDRV_UINT64 slicData = 0;
   XDRV_UINT64 valueMask = 0;

   valueMask = ConvertBpinsToMask( &pDev->pinInfo, value );
 
   /* get content of the SLIC data register - Add 6838 support*/
   XCHG_ASSERT(0);
//    slicData=GPIO->GPIOio;

   /* clear the state for this channel */
   slicData &=~(((XDRV_UINT64)1<<(pDev->slicCfgInfo.slicIoStateMask & 0xFF)) | 
               ((XDRV_UINT64)1<<((pDev->slicCfgInfo.slicIoStateMask>>8) & 0xFF)) | 
               ((XDRV_UINT64)1<<((pDev->slicCfgInfo.slicIoStateMask>>16) & 0xFF)));

   /* update the state for this channel */
   slicData |= valueMask;
   /* write the state to the slic control register - Add 6838 support*/
   XCHG_ASSERT(0);
//    GPIO->GPIOio=slicData;
}
...
static void SetupGpioControl( SLIC_6838_L9530_DRV *pDev, const SLICCFG_6838_L9530_PIN_INFO* pSlicPinInfo )
{
   ...
   pDev->slicCfgInfo.slicIoStateMask = pSlicPinInfo->B0_pin | (pSlicPinInfo->B1_pin<<8) | (pSlicPinInfo->B2_pin<<16);
                           ...
                           XDRV_UINT32* mask )
{
   if ( pin >= 32 )
   {
      ...
      ** the pin number is higher than 32 - add 6838 Support */
      XCHG_ASSERT(0);      
...
*****************************************************************************
*/
static XDRV_UINT64 ConvertBpinsToMask( SLICCFG_6838_L9530_PIN_INFO* pinInfo, XDRV_UINT32 value )
{
   XDRV_UINT64 mask = 0;
   XDRV_UINT8  bit;
   ...
   /* Bit 0 of value */
   bit = value & 1;
   mask |= (XDRV_UINT64)bit << pinInfo->B0_pin;

   /* Bit 1 of value */
   bit = (value & (1 << BIT1POSITION)) >> BIT1POSITION;
   mask |= (XDRV_UINT64)bit << pinInfo->B1_pin;

   bit = (value & (1 << BIT2POSITION)) >> BIT2POSITION;
   mask |= (XDRV_UINT64)bit << pinInfo->B2_pin;
...
/*
*****************************************************************************
** FUNCTION:   slic6838L9530UpdateState
**
...
*****************************************************************************
*/
void slic6838L9530UpdateState( SLIC_6838_L9530_DRV *pDev )
{
   if( pDev->pendingSlicTime > 0 )
   {
