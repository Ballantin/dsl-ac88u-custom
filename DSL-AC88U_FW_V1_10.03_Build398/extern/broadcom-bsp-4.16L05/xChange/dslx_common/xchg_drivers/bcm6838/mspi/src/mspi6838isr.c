...
unsigned int mspi6838Isr( XDRV_INT_CTRL_ISR_PARM contextp )
{
   ...
   pMspiRegs = pMspiDrv->p6838SpiRegs;

   /*
   ** Check if the current transfer is done
   */
   
   if( !( pMspiRegs->spiIntStatus & MMR6838_MSPI_INTSTATUS_CMNDDONE ))
   {
      XCHG_ASSERT( 0 );
   }
   ...
   */
   pMspiRegs->spiIntStatus |= MMR6838_MSPI_INTSTATUS_MASK;
   ...
   */
   ...
   else
   {
      ...
      */
      if( pMspiDrv->bytesRemaining >= ( BCM6838_MSPI_TX_QUEUE_SIZE - 2 ))
      {
         numBytesToXfer = ( BCM6838_MSPI_TX_QUEUE_SIZE - 2 );
      }
      else
      {
      ...
      */
      ...
      pMspiRegs->spiIntStatus |= MMR6838_MSPI_INTSTATUS_MASK;
      pMspiRegs->spiCmd |= MMR6838_MSPI_START_IMMEDIATE;
   }
