...
/* ---- Include Files ---------------------------------------- */
#include <mspi6838.h>
#include <mspi6838defs.h>

/* ---- Private Constants and Types -------------------------- */
static const BCM_MSPI6838_REGS mspi6838DefaultRegs =
{
   2,       /* SerialClk Divider setting  */
...
#if MSPI6838_USE_INTERRUPTS
   BCM_MSPI6838_CTRL_MODE_INTERRUPT  /* Interrupt mode */
#else
   BCM_MSPI6838_CTRL_MODE_POLLING /*Polling mode*/
#endif /* MSPI6838_USE_INTERRUPTS */
};

/* ---- Private Variables ------------------------------------ */
/* ---- Private Function Prototypes -------------------------- */
static int setMspiRegs( BCM_MSPI6838_DRV *pMspiDrv, BCM_MSPI6838_REGS *pNewMspiRegs );
static int getMspiRegs( BCM_MSPI6838_DRV *pMspiDrv, BCM_MSPI6838_REGS *pNewMspiRegs );
...
*****************************************************************************
*/
int mspiIOctl( BCM_MSPI6838_CMD command, void *cmdp )
{
   int result;
   BCM_MSPI6838_DRV *pMspiDrv;

   /* Retrieve MSPI driver */
   pMspiDrv = &mspi6838Drv;

   switch ( command )
   {
      case BCM_MSPI6838_INITREGS_CMD:
      {
         result = setMspiRegs( pMspiDrv, (BCM_MSPI6838_REGS *)&mspi6838DefaultRegs );
      }
      break;

      case BCM_MSPI6838_SETREGS_CMD:
      {
         result = setMspiRegs( pMspiDrv, cmdp );
      }
      break;

      case BCM_MSPI6838_GETREGS_CMD:
      {
         result = getMspiRegs( pMspiDrv, cmdp );
      }
      break;

      case BCM_MSPI6838_RXCALLBACK_INSTALL_CMD:
      {
      ...
      case BCM_MSPI6838_TXCALLBACK_INSTALL_CMD:
      {
...
*****************************************************************************
*/
static int setMspiRegs( BCM_MSPI6838_DRV *pMspiDrv, 
                        BCM_MSPI6838_REGS *pNewMspiRegs )
{
   volatile SpiControl *pMspiHwRegs = pMspiDrv->p6838SpiRegs;

   /* If MSPI is busy then abort */
   if (( pMspiHwRegs->spiStatus & MMR6838_MSPI_STATUS_SERBUSY ) || ( pMspiHwRegs->spiStatus & MMR6838_MSPI_STATUS_CMDBUSY ))
   {
   ...
   */
   if( pNewMspiRegs->mspiCtrlMode == BCM_MSPI6838_CTRL_MODE_POLLING )
   {
      XDRV_LOG_DBG((XDRV_LOG_MOD_XDRV, "INFO: MSPI POLLING MODE"));

      pMspiHwRegs->spiIntMask = 0x00;
      mspi6838intc_IrqDisable();
   }
#if MSPI6838_USE_INTERRUPTS
   else
   {
...
*****************************************************************************
*/
static int getMspiRegs( BCM_MSPI6838_DRV *pMspiDrv, 
                        BCM_MSPI6838_REGS *pNewMspiRegs )
{
   volatile SpiControl *pMspiHwRegs = pMspiDrv->p6838SpiRegs;
   ...
   {
      pNewMspiRegs->mspiCtrlMode = BCM_MSPI6838_CTRL_MODE_INTERRUPT;
   }
   else
   {
      pNewMspiRegs->mspiCtrlMode = BCM_MSPI6838_CTRL_MODE_POLLING;
   }
