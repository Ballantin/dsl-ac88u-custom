...
****************************************************************************
*
*    Description:  NTR device driver for BCM6368
*
****************************************************************************/
...
#include <6368_map.h>         /* chip-specific PCM memory maps */
#include <ntr6368.h>
...
/* On the BCM96368VVWG reference platform, the NTR_IN pin / GPIO[15] is wired to the reset pin
 * of the SLICs.  Therefore, a special modified board must be used to move the SLIC reset to a
 * different GPIO.  This code ensures that GPIO15 is in the correct mode of operation. */
static XDRV_UINT8 gpio15InitFlag = 0;

static void ntr6368_init()
{
   if ( !gpio15InitFlag )
   {
      gpio15InitFlag = 1;
...
void ntr6368_getCounts( XDRV_UINT32 * localCount, XDRV_UINT32 * ntrCount )
{
   /* Macro that accesses the mips counter (200MHz on 6368, 150MHz on 6358) */
...
void ntr6368_getNdiv( XDRV_UINT32 * ndivInt, XDRV_UINT32 * ndivFrac )
{
...
)
{
   ...
   NTR_LOGDEBUG(("\n\nBeginning of ntr6368_setFeedback.  Dispaying unmodified registers."));
...
void ntr6368_print( XDRV_BOOL numSamples, XDRV_SINT32 sampleIntervalMs)
{
