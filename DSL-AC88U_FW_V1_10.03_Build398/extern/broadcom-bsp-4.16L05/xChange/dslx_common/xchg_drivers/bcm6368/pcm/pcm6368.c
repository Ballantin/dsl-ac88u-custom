...
****************************************************************************
*
*    Description:  PCM device driver for BCM6368
*
****************************************************************************/
...
#elif defined PCM_WIDEBAND_DECT_SITEL_NOFXS
#define P_TSALLOC_CHAN0        0 // Dect Wideband channel 0 (16-bit consist of 2 8-bit little endian)
#define P_TSALLOC_CHAN1        1 // Dect Wideband channel 1 (16-bit consist of 2 8-bit little endian)
#define P_TSALLOC_CHAN2        4 // FX0 to TS 4  (16-bit linear)
#else
...
BCM6368_IUDMA_DESC *dmaRxDesc_c[PCM6368_NUM_DMA_CHANNELS*DMA_RX_NUM_BUFFERS];
BCM6368_IUDMA_DESC *dmaTxDesc_c[PCM6368_NUM_DMA_CHANNELS*DMA_TX_NUM_BUFFERS];

PCM6368_DMA_BUFFER dmaRxData[PCM6368_NUM_DMA_CHANNELS*DMA_RX_NUM_BUFFERS];
PCM6368_DMA_BUFFER dmaTxData[PCM6368_NUM_DMA_CHANNELS*DMA_TX_NUM_BUFFERS];

static int  pcm6368_PLLInit( void );
static int  pcm6368_PLLDeInit( void );
static int  pcm6368_iudmaInit(void);
static int  pcm6368_iudmaDeinit(void);
static int  pcm6368_dmaDescrInit(void);
static int  pcm6368_dmaDescrDeinit(void);
static int  pcm6368_regInit( PCM_CHANNEL_TIMESLOT_MAP* chanTsMap );
static int  pcm6368_regDeinit( void );
static void pcm6368_timeslotAlloc( int chnum, int ts);
...
typedef struct PCM6368_DMA_SIMPLEX_CHANNEL_INFO
{
   int                  iuChanNum;     /* IUDMA channel number assosiated with this APM channel */
   BCM6368_IUDMA_DESC   *dmaDescp;     /* Where are the descriptors for this channel? */
   DMA_HANDLE           dmaDescHandle;
   PCM6368_DMA_BUFFER   *dmaDatap;     /* Where are the DMA buffers to store/fetch the data to/from for each channel */
   DMA_HANDLE           dmaBufHandle;
}  PCM6368_DMA_SIMPLEX_CHANNEL_INFO;

/* DMA Channel info (duplex - both ways) for easy access to DMA channels */
typedef struct PCM6368_DMA_DUPLEX_CHANNEL_INFO
{
   PCM6368_DMA_SIMPLEX_CHANNEL_INFO rxChan;  /* RX channel */
   PCM6368_DMA_SIMPLEX_CHANNEL_INFO txChan;  /* TX channel */
} PCM6368_DMA_DUPLEX_CHANNEL_INFO;
...
static PCM6368_DMA_DUPLEX_CHANNEL_INFO gPcmDmaInfo[PCM_NUM_DMA_CHANNELS] =
{
   {
      /* RX Channel 0 */
      {
...
/*****************************************************************************
*
*  FUNCTION: pcm6368_init
*
*  PURPOSE:
*      Intialize 6368 PCM Module
*
...
int pcm6368_init( PCM_CHANNEL_TIMESLOT_MAP* chanTsMap, XDRV_UINT32 channelMask, XDRV_UINT8 supportMode )
{
   (void)supportMode;
   ...
   pcm6368_regInit( chanTsMap );
  
   pcm6368_dmaDescrInit();
...
/*****************************************************************************
*
*  FUNCTION: pcm6368_deinit
*
*  PURPOSE:
*      Deintialize 6368 PCM Module
*
*  PARAMETERS:
*
*  RETURNS: 0 on SUCCESS
...
static int pcm6368_PLLInit( void )
{
 retry_lock:
   ...
   PCM->pcm_pll_ctrl3 = 0x382c2860;    /* [31:0] = PLL_ctrl[31:0], VCO > 1.6GHz */  
   PCM->pcm_pll_ctrl2 = 0x10000000;    /* VCO is 2048MHZ, NDIV_MODE=0, NDIV_FRAC=0 */
   PCM->pcm_pll_ctrl1 = 0x1C80147D;    /* MDIV=125(0x7D), P1DIV=4(0x4), P2DIV=1(0x1), NDIV_INT=128(0x080) */
#else
   PCM->pcm_pll_ctrl4 = 0x00000015;         /* [5:0] = PLL_ctrl[37:32] */
   PCM->pcm_pll_ctrl3 = 0x38000700;         /* [31:0] = PLL_ctrl[31:0] */
   PCM->pcm_pll_ctrl2 = 0x10000000;         /* VCO is 2040MHZ */
   PCM->pcm_pll_ctrl1 = 0x1C80147D;         /* VCO is 2040MHZ */
#endif
   ...
   XDRV_LOG_DBG((XDRV_LOG_MOD_PCM, "PLL init completed. PLL registers set to:"));
   XDRV_LOG_DBG((XDRV_LOG_MOD_PCM, "PCM->pcm_pll_ctrl1       = 0x%08X ", (unsigned int)PCM->pcm_pll_ctrl1));
   XDRV_LOG_DBG((XDRV_LOG_MOD_PCM, "PCM->pcm_pll_ctrl2       = 0x%08X ", (unsigned int)PCM->pcm_pll_ctrl2));
   XDRV_LOG_DBG((XDRV_LOG_MOD_PCM, "PCM->pcm_pll_ctrl3       = 0x%08X ", (unsigned int)PCM->pcm_pll_ctrl3));
   XDRV_LOG_DBG((XDRV_LOG_MOD_PCM, "PCM->pcm_pll_ctrl4       = 0x%08X ", (unsigned int)PCM->pcm_pll_ctrl4));
...
/*****************************************************************************
*
*  FUNCTION: pcm6368_regInit
*
*  PURPOSE:
*      Intialize 6368 PCM Module Registers
*
*  PARAMETERS:
*
*  RETURNS: 0 on SUCCESS
...
#  endif
   ...
   for ( i = 0; i < PCM_MAX_CHANNELS; i++ )
   {
      ...
      XDRV_LOG_DBG((XDRV_LOG_MOD_PCM,  "PCM: Channel/Line %d Timeslot %d ", i, (int)chanTsMap[i].txTimeslot ));
      pcm6368_timeslotAlloc( i, chanTsMap[i].txTimeslot );
   }
...
{
   XDRV_UINT8                       i,chnum;
   volatile BCM6368_IUDMA_DESC      *dmaTxDesc;
   volatile BCM6368_IUDMA_DESC      *dmaRxDesc;
   volatile BCM6368_IUDMA_DESC      *dmaTxDesc_c;
   volatile BCM6368_IUDMA_DESC      *dmaRxDesc_c;
   volatile PCM6368_DMA_BUFFER      *dmaRxData;
   volatile PCM6368_DMA_BUFFER      *dmaTxData;
   PCM6368_DMA_SIMPLEX_CHANNEL_INFO *dmaChanInfop;
   ...
   rxDescriptorArea = sizeof(BCM6368_IUDMA_DESC) * PCM6368_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6368_DMA_UBUS_BYTES;
   txDescriptorArea = sizeof(BCM6368_IUDMA_DESC) * PCM6368_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6368_DMA_UBUS_BYTES;
   ...
   for (i=0; i < PCM6368_NUM_DMA_CHANNELS; i++)
   {
      /* Align DMA descriptior to 64Bit Ubus */
      dmaRxDesc_c = (volatile BCM6368_IUDMA_DESC *) ALIGN_DATA( (unsigned int) dmaRxDesc_c, BCM6368_DMA_UBUS_BYTES );
      dmaTxDesc_c = (volatile BCM6368_IUDMA_DESC *) ALIGN_DATA( (unsigned int) dmaTxDesc_c, BCM6368_DMA_UBUS_BYTES );
      
      gPcmDmaInfo[i].rxChan.dmaDescp = (BCM6368_IUDMA_DESC*)(dmaRxDesc_c + i * DMA_RX_NUM_BUFFERS);
      gPcmDmaInfo[i].txChan.dmaDescp = (BCM6368_IUDMA_DESC*)(dmaTxDesc_c + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   rxBufferArea = sizeof(XDRV_UINT32) * (PCM6368_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * PCM6368_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS;
   txBufferArea = sizeof(XDRV_UINT32) * (PCM6368_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * PCM6368_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS;
   ...
   for (i=0; i < PCM6368_NUM_DMA_CHANNELS; i++)
     {
      gPcmDmaInfo[i].rxChan.dmaDatap = (PCM6368_DMA_BUFFER*)(dmaRxData + i * DMA_RX_NUM_BUFFERS);
      gPcmDmaInfo[i].txChan.dmaDatap = (PCM6368_DMA_BUFFER*)(dmaTxData + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   for (chnum=0; chnum<PCM6368_NUM_DMA_CHANNELS; chnum++)
   {
      ...
      dmaTxDesc = (volatile BCM6368_IUDMA_DESC *)CacheToNonCacheVirtAddress((XDRV_UINT32)dmaChanInfop->dmaDescp);
      ...
      for (i=0; i<DMA_TX_NUM_BUFFERS; i++)
      {
         ...
         bufferSizeBytes = PCM6368_DMA_MAX_BUFFER_SIZE_BYTES;

         /* Each buffer is a complete packet */
         dmaTxDesc[i].flagsLengthStatus.status = BCM6368_IUDMA_DESC_STATUS_SOP |
                                                 BCM6368_IUDMA_DESC_STATUS_EOP;

         if( i == 0 )
         {
            /* First buffer - prime the TX fifo */
            dmaTxDesc[i].flagsLengthStatus.status |= BCM6368_IUDMA_DESC_STATUS_OWN;
         ...
         if( i == ( DMA_TX_NUM_BUFFERS - 1 ) )
         {
            ...
            dmaTxDesc[i].flagsLengthStatus.status |= BCM6368_IUDMA_DESC_STATUS_WRAP;
         }

         /* Check if we can accomodate the desired buffer size */
         if (bufferSizeBytes <= PCM6368_DMA_MAX_BUFFER_SIZE_BYTES)
         {
         ...
         else
         {
            ...
            ** You can either increase the compile time buffer size (PCM6368_DMA_MAX_BUFFER_SIZE),
      ...
      dmaRxDesc = (volatile BCM6368_IUDMA_DESC *)CacheToNonCacheVirtAddress((XDRV_UINT32)dmaChanInfop->dmaDescp);
      ...
      for( i = 0; i < DMA_RX_NUM_BUFFERS; i++ )
      {
         ...
         dmaRxDesc[i].flagsLengthStatus.status = BCM6368_IUDMA_DESC_STATUS_OWN;

         if( i == ( DMA_RX_NUM_BUFFERS - 1 ) )
         {
            /* Last descriptor - set the wrap bit */
            dmaRxDesc[i].flagsLengthStatus.status |= BCM6368_IUDMA_DESC_STATUS_WRAP;
         }
         ...
         ** buffers before and after DMA operations. Since cache operations
         ** operate on the entire cache-line, it is necessary to make the
         ** buffers aligned on the cache-line size (16-bytes for the 6368).
         ** Failure to do so may result in corruption of data that shares the
         ** same cache-line as the DMA buffer. */
         dmaDatap = dmaChanInfop->dmaDatap[i].dmaBuff;
         dmaDatap = (volatile XDRV_UINT32 *) ALIGN_DATA( (unsigned int) dmaDatap, DCACHE_LINE_SIZE_BYTES );
...
static int pcm6368_dmaDescrDeinit( void )
{
   ...
   rxDescriptorArea = sizeof(BCM6368_IUDMA_DESC) * PCM6368_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS;
   txDescriptorArea = sizeof(BCM6368_IUDMA_DESC) * PCM6368_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS;
...
static int pcm6368_iudmaInit( void)
{
   ...
   memset( (void*)stRamp, 0, BCM6368_MAX_PCM_DMA_CHANNELS * sizeof(PcmIudmaStateRam) );
   ...
   /* Init RX side */
      ...
      iuDmap->ctrl[gPcmDmaInfo[chnum].rxChan.iuChanNum].intMask = BCM6368_IUDMA_INTMASK_BDONE | BCM6368_IUDMA_INTMASK_NOTVLD;
      ...
      iuDmap->ctrl[gPcmDmaInfo[chnum].txChan.iuChanNum].intMask = BCM6368_IUDMA_INTMASK_BDONE | BCM6368_IUDMA_INTMASK_NOTVLD;
   }

   /* Enable the IUDMA master */
   iuDmap->regs.ctrlConfig = BCM6368_IUDMA_REGS_CTRLCONFIG_MASTER_EN;
...
static int pcm6368_iudmaDeinit( void)
{
   ...
   for (chnum=0; chnum<PCM6368_NUM_DMA_CHANNELS; chnum++)
   {
      ...
      iuDmap->ctrl[gPcmDmaInfo[chnum].rxChan.iuChanNum].config = ( BCM6368_IUDMA_CONFIG_BURSTHALT );      

      while( (iuDmap->ctrl[gPcmDmaInfo[chnum].rxChan.iuChanNum].config & BCM6368_IUDMA_CONFIG_ENDMA) != 0x00000000 )
      {
         /* Set the burstHalt bit while clearing endma bit */
         iuDmap->ctrl[gPcmDmaInfo[chnum].rxChan.iuChanNum].config = ( BCM6368_IUDMA_CONFIG_BURSTHALT );      
      ...
      /* Set the burstHalt bit while clearing endma bit */
      iuDmap->ctrl[gPcmDmaInfo[chnum].txChan.iuChanNum].config = ( BCM6368_IUDMA_CONFIG_BURSTHALT );      

      while( (iuDmap->ctrl[gPcmDmaInfo[chnum].txChan.iuChanNum].config & BCM6368_IUDMA_CONFIG_ENDMA) != 0x00000000 )
      {
         /* Set the burstHalt bit while clearing endma bit */
         iuDmap->ctrl[gPcmDmaInfo[chnum].txChan.iuChanNum].config = ( BCM6368_IUDMA_CONFIG_BURSTHALT );               
...
/*****************************************************************************
*
*  FUNCTION: pcm6368_timeslotAlloc
*
*  PURPOSE:
...
static void pcm6368_timeslotAlloc( int chnum, int ts)
{
   ...
   */
   ...
   XDRV_LOG_INFO(( XDRV_LOG_MOD_PCM, "%s 0x%08X: 0x%08X", __FUNCTION__, (unsigned int)&PCM->pcm_slot_alloc_tbl[ts_register_offset], (unsigned int)entry ));
