...
*****************************************************************************
*/
int mspi6368Read( volatile XDRV_UINT8 *pRxBuf,
                  unsigned length )
{
   ...
   if( length > MSPI6368_MAX_RX_SIZE )
   {
      /* Length must be less than MAX_RX_SIZE  */
      XDRV_LOG_ERROR((XDRV_LOG_ERROR_MOD_XDRV, "ERROR: invalid length"));
      return ( MSPI_ERROR );
   }
   ...
   {
      if( bosSemTimedTake( &mspi6368IsrSem, 100 ))
      {
         XDRV_LOG_ERROR((XDRV_LOG_ERROR_MOD_XDRV, "ERROR: Failed to take semaphore"));
         return ( MSPI_ERROR );
      }
   }
   ...
   if( bosSemGive( &mspi6368IsrSem ))
   {
      XDRV_LOG_ERROR((XDRV_LOG_ERROR_MOD_XDRV, "ERROR: Failed to give semaphore"));
      return ( MSPI_ERROR );
   }
#endif /* MSPI6368_USE_INTERRUPTS */
