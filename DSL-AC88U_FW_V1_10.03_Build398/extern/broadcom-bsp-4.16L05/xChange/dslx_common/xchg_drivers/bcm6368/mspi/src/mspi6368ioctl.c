...
static const BCM_MSPI6368_REGS mspi6368DefaultRegs =
{
   ...
   BCM_MSPI6368_CTRL_MODE_INTERRUPT  /* Interrupt mode */
#else
   BCM_MSPI6368_CTRL_MODE_POLLING /*Polling mode*/
#endif /* MSPI6368_USE_INTERRUPTS */
};

/* ---- Private Variables ------------------------------------ */
/* ---- Private Function Prototypes -------------------------- */
static int setMspiRegs( BCM_MSPI6368_DRV *pMspiDrv, BCM_MSPI6368_REGS *pNewMspiRegs );
static int getMspiRegs( BCM_MSPI6368_DRV *pMspiDrv, BCM_MSPI6368_REGS *pNewMspiRegs );

/* ---- Functions -------------------------------------------- */

/*
*****************************************************************************
** FUNCTION:   mspi6368IOctl
**
...
*****************************************************************************
*/
int mspi6368IOctl( BCM_MSPI6368_CMD command, void *cmdp )
{
   int result;
   ...
   switch ( command )
   {
      case BCM_MSPI6368_INITREGS_CMD:
      {
         result = setMspiRegs( pMspiDrv, (BCM_MSPI6368_REGS *)&mspi6368DefaultRegs );
      }
      break;

      case BCM_MSPI6368_SETREGS_CMD:
      {
         result = setMspiRegs( pMspiDrv, cmdp );
      }
      break;

      case BCM_MSPI6368_GETREGS_CMD:
      {
         result = getMspiRegs( pMspiDrv, cmdp );
      }
      break;

      case BCM_MSPI6368_RXCALLBACK_INSTALL_CMD:
      {
      ...
      case BCM_MSPI6368_TXCALLBACK_INSTALL_CMD:
      {
...
*****************************************************************************
*/
static int setMspiRegs( BCM_MSPI6368_DRV *pMspiDrv, 
                        BCM_MSPI6368_REGS *pNewMspiRegs )
{
   volatile SpiControl *pMspiHwRegs = pMspiDrv->p6368SpiRegs;

   /* If MSPI is busy then abort */
   if (( pMspiHwRegs->spiStatus & MMR6368_MSPI_STATUS_SERBUSY ) || ( pMspiHwRegs->spiStatus & MMR6368_MSPI_STATUS_CMDBUSY ))
   {
   ...
   */
   if( pNewMspiRegs->mspiCtrlMode == BCM_MSPI6368_CTRL_MODE_POLLING )
   {
...
*****************************************************************************
*/
static int getMspiRegs( BCM_MSPI6368_DRV *pMspiDrv, 
                        BCM_MSPI6368_REGS *pNewMspiRegs )
{
   volatile SpiControl *pMspiHwRegs = pMspiDrv->p6368SpiRegs;
   ...
   {
      pNewMspiRegs->mspiCtrlMode = BCM_MSPI6368_CTRL_MODE_INTERRUPT;
   }
   else
   {
      pNewMspiRegs->mspiCtrlMode = BCM_MSPI6368_CTRL_MODE_POLLING;
   }
