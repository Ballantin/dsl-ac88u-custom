...
*****************************************************************************
*/
int mspi6368Write( XDRV_UINT8 *pTxBuf,
                   int chan,
                   unsigned length,
                   unsigned *pNumTxfr )
{
   ...
   */
   ...
   while( bytesRemaining > 0 )
   {
      ...
      */
      if( bytesRemaining >= ( BCM6368_MSPI_TX_QUEUE_SIZE - 2 ))
      {
         numBytesToXfer = ( BCM6368_MSPI_TX_QUEUE_SIZE - 2 );
      }
      else
      {
      ...
      */
      if( pMspiRegs->spiIntMask != 0x00 )
      {
         ...
         *pNumTxfr = 0;
         ...
         pMspiRegs->spiIntStatus |= MMR6368_MSPI_INTSTATUS_MASK;
...
#endif /* MSPI6368_USE_INTERRUPTS */
      {
        /* Clear MSPI finished flags and begin transmission */
         ...
         while(!(pMspiRegs->spiIntStatus & MMR6368_MSPI_INTSTATUS_CMNDDONE));
         /*
         ** Read out rx data
         */
         for( indx=0; indx < numBytesToXfer; indx++ )
