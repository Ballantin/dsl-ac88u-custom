...
****************************************************************************
*
*    Description:  PCM device driver for BCM63138
*
****************************************************************************/
...
#define NTR_EXT_REF_MHZ  26   /* 26 MHz RF clock */
...
static int g_external_ntr = -1;
static BCM_IUDMA_ADDR_CTL* gDmaCtl = NULL;

/* -- Public & externed variables -- */
extern struct dma_pool *pcm_dma_pool;
extern int BpGetNTRExternReference ( void );
/* -- Private functions -- */
static void printRegs( void );
static void printCtl( void );
static void pcm63138_enable( int chan_mask );
static void pcm63138_disable( void );
static int  pcm63138_clkInit( void );
static int  pcm63138_iudmaInit( BCM_IUDMA_ADDR_CTL* dmaCtl );
static int  pcm63138_iudmaDeinit( void );
static int  pcm63138_dmaDescrInit( BCM_IUDMA_ADDR_CTL* dmaCtl );
static int  pcm63138_dmaDescrDeinit( void );
static int  pcm63138_regInit( PCM_CHANNEL_TIMESLOT_MAP* chanTsMap, BCM_IUDMA_ADDR_CTL* dmaCtl );
static int  pcm63138_regDeinit( void );
static void pcm63138_timeslotAlloc( int chnum, int ts);
...
/*****************************************************************************
*
*  FUNCTION: pcm63138_init
*
*  PURPOSE:
*      Intialize 63138 PCM Module
*
...
int pcm63138_init( PCM_CHANNEL_TIMESLOT_MAP* chanTsMap, XDRV_UINT32 channelMask, XDRV_UINT8 supportMode, BCM_IUDMA_ADDR_CTL* dmaCtl, XDRV_UINT32* dmaCount )
{
   ...
   pcm63138_regInit( chanTsMap, gDmaCtl );
   pcm63138_enable( channelMask );
   pcm63138_dmaDescrInit( gDmaCtl );
   pcm63138_iudmaInit( gDmaCtl );

   /* Increase our DMA count by the number of DMAs allocated in this function */
   *dmaCount += PCM63138_NUM_DMA_CHANNELS;
...
/*****************************************************************************
*
*  FUNCTION: pcm63138_deinit
*
*  PURPOSE:
*      Deintialize 63138 PCM Module
*
*  PARAMETERS:
*
*  RETURNS: 0 on SUCCESS
...
*
*****************************************************************************/
int pcm63138_deinit( void )
{
   pcm63138_disable();
   pcm63138_iudmaDeinit();
   pcm63138_dmaDescrDeinit();
   pcm63138_regDeinit();
...
static int pcm_pllStart(void)
{
 ...
 retry_lock:
   ...
   if ( HW_NTR_SUPPORT || (g_external_ntr >= 0)) {
       if ( g_external_ntr >= 0 )
       {
#if (NTR_EXT_REF_MHZ == 26)
           /* 26Mhz refclk / 13000          = 2Khz clock to comparator     *
            * 65.536Mhz local clock / 32768 = 2Khz clock to comparator     */
           APM->dpll_div = (12999 << DPLL_REF_DIV_SHIFT) | 32767;   // 26MHz/65.536MHz = 12999/32767
#elif (NTR_EXT_REF_MHZ == 13)
           /* 13Mhz refclk / 6500           = 2Khz clock to comparator     *
            * 65.536Mhz local clock / 32768 = 2Khz clock to comparator     */
           APM->dpll_div = (6499 << DPLL_REF_DIV_SHIFT) | 32767;    // 13MHz/65.536MHz = 6499/32767
#endif
           XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "%s: Synching PLL to external clock", __FUNCTION__));
       }
       else {
...
static int pcm63138_clkInit( void )
{
   ...
   g_external_ntr = BpGetNTRExternReference();
   ...
   if( HW_NTR_SUPPORT || (g_external_ntr >= 0)) {
       ...
       if ( g_external_ntr >= 0 )
       {
           bcm_set_pinmux ( g_external_ntr & BP_PINMUX_PIN_MASK, (g_external_ntr & BP_PINMUX_VAL_MASK) >> BP_PINMUX_VAL_SHIFT );
           MISC->miscAdsl_clock_sample  |= ( PCM_NTR_IN_CTRL_SEL_VDSL_GPIO << PCM_NTR_IN_CTRL_SHIFT);  /* set PCM_NTR_IN_CTRL from GPIO  */
       }
       else {
   ...
   if( HW_NTR_SUPPORT || ( g_external_ntr >= 0 )|| ( gPcmSpecialMode & MODE_DECT ) )
   {
...
/*****************************************************************************
*
*  FUNCTION: pcm63138_regInit
*
*  PURPOSE:
*      Intialize 63138 PCM Module Registers
*
*  PARAMETERS:
*
*  RETURNS: 0 on SUCCESS
...
static int pcm63138_regInit( PCM_CHANNEL_TIMESLOT_MAP* chanTsMap, BCM_IUDMA_ADDR_CTL* dmaCtl )
{
   ...
   for(i = 0; i < PCM_MAX_CHANNELS; i++)
   {
      ...
      pcm63138_timeslotAlloc( i, chanTsMap[i].txTimeslot );
      dmaCtl[0].chanCount++;
   }
...
/*****************************************************************************
*
*  FUNCTION: pcm63138_enable
*
*  PURPOSE:
*      Enable 63138 PCM Module
*
*  PARAMETERS:
*
*  RETURNS: 0 on SUCCESS
...
/*****************************************************************************
*
*  FUNCTION: pcm63138_disable
*
*  PURPOSE:
*      Disable 63138 PCM Module
*
*  PARAMETERS:
*
*  RETURNS: 0 on SUCCESS
...
/*
*****************************************************************************
** FUNCTION:   pcm63138_initDmaDescriptors
**
...
{
   XDRV_UINT32              i, chnum;
   BCM63138_IUDMA_DESC      *dmaTxDesc;
   BCM63138_IUDMA_DESC      *dmaRxDesc;
   ...
   rxDescriptorArea = sizeof(BCM63138_IUDMA_DESC) * PCM63138_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM63138_DMA_UBUS_BYTES;
   txDescriptorArea = sizeof(BCM63138_IUDMA_DESC) * PCM63138_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM63138_DMA_UBUS_BYTES;
   ...
   XDRV_LOG_INFO((XDRV_LOG_MOD_PCM, "Sizeof(BCM63138_IUDMA_DESC) %d\n", sizeof(BCM63138_IUDMA_DESC)));

   /* Fill in the global PCM DMA structure with the allocated pointers to descriptors */
   for(chnum = 0; chnum < PCM63138_NUM_DMA_CHANNELS; chnum++)
   {
   ...
   rxBufferArea = sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * PCM63138_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS;
   txBufferArea = sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * PCM63138_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS;
   ...
   for(chnum = 0; chnum < PCM63138_NUM_DMA_CHANNELS; chnum++)
   {
      dmaCtl[chnum].rx[0].bufVirt.addr32 = (dmaRxData.addr32 + chnum * DMA_RX_NUM_BUFFERS * sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32));
      dmaCtl[chnum].rx[1].bufVirt.addr32 = (dmaRxData.addr32 + chnum * DMA_RX_NUM_BUFFERS * sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) + sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32));
      dmaCtl[chnum].rx[0].bufPhy.addr32 = dmaCtl[chnum].rx[0].bufPhy.addr32 + chnum * DMA_RX_NUM_BUFFERS * sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32);
      dmaCtl[chnum].rx[1].bufPhy.addr32 = dmaCtl[chnum].rx[0].bufPhy.addr32 + chnum * DMA_RX_NUM_BUFFERS * sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) + sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32);

      dmaCtl[chnum].tx[0].bufVirt.addr32 = (dmaTxData.addr32 + chnum * DMA_TX_NUM_BUFFERS * sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32));
      dmaCtl[chnum].tx[1].bufVirt.addr32 = (dmaTxData.addr32 + chnum * DMA_TX_NUM_BUFFERS * sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) + sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32));
      dmaCtl[chnum].tx[0].bufPhy.addr32 = dmaCtl[chnum].tx[0].bufPhy.addr32 + chnum * DMA_TX_NUM_BUFFERS * sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32);
      dmaCtl[chnum].tx[1].bufPhy.addr32 = dmaCtl[chnum].tx[0].bufPhy.addr32 + chnum * DMA_TX_NUM_BUFFERS * sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) + sizeof(XDRV_UINT32) * (PCM63138_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32);
   }
   ...
   for(chnum = 0; chnum < PCM63138_NUM_DMA_CHANNELS; chnum++)
   {
      bufferSizeBytes = PCM63138_DMA_MAX_BUFFER_SIZE_BYTES;

      /* Initialize our TX channels */
      for(i = 0; i < DMA_TX_NUM_BUFFERS; i++)
      {
         /* Set buffer status, size, and physical address */
         dmaCtl[chnum].tx[i].descVirt->flagsLengthStatus.status = BCM63138_IUDMA_DESC_STATUS_SOP | BCM63138_IUDMA_DESC_STATUS_EOP;
         ...
         if( i == 0 )
         {
            ...
            dmaCtl[chnum].tx[i].descVirt->flagsLengthStatus.status |= BCM63138_IUDMA_DESC_STATUS_OWN;
         }
         else if( i == (DMA_TX_NUM_BUFFERS - 1) )
         {
            /* Last buffer - set the WRAP bit */
            dmaCtl[chnum].tx[i].descVirt->flagsLengthStatus.status |= BCM63138_IUDMA_DESC_STATUS_WRAP;
         }
      }
      ...
      for(i = 0; i < DMA_RX_NUM_BUFFERS; i++)
      {
         ...
         dmaCtl[chnum].rx[i].descVirt->flagsLengthStatus.status = BCM63138_IUDMA_DESC_STATUS_OWN;
         ...
         {
            dmaCtl[chnum].rx[i].descVirt->flagsLengthStatus.status |= BCM63138_IUDMA_DESC_STATUS_WRAP;
         }
      }
   }
...
static int pcm63138_dmaDescrDeinit( void )
{
   ...
   rxDescriptorArea = sizeof(BCM63138_IUDMA_DESC) * PCM63138_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS;
   txDescriptorArea = sizeof(BCM63138_IUDMA_DESC) * PCM63138_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS;
   ...
   for(i = 0; i < PCM63138_NUM_DMA_CHANNELS; i++)
   {
...
static int pcm63138_iudmaInit( BCM_IUDMA_ADDR_CTL* dmaCtl )
{
   ...
   for(chnum = 0; chnum < PCM63138_NUM_DMA_CHANNELS; chnum++)
   {
      ...
      iuDmap->ctrl[dmaCtl[chnum].dmaChanRx].intMask = BCM63138_IUDMA_INTMASK_BDONE | BCM63138_IUDMA_INTMASK_NOTVLD;
      ...
      iuDmap->ctrl[dmaCtl[chnum].dmaChanTx].intMask = BCM63138_IUDMA_INTMASK_BDONE | BCM63138_IUDMA_INTMASK_NOTVLD;
...
static int pcm63138_iudmaDeinit( void )
{
   ...
   for(chnum = 0; chnum < PCM63138_NUM_DMA_CHANNELS; chnum++)
   {
      ...
      iuDmap->ctrl[gDmaCtl[chnum].dmaChanRx].config = ( BCM63138_IUDMA_CONFIG_BURSTHALT );

      while( (iuDmap->ctrl[gDmaCtl[chnum].dmaChanRx].config & BCM63138_IUDMA_CONFIG_ENDMA) != 0x00000000 )
      {
         /* Set the burstHalt bit while clearing endma bit */
         iuDmap->ctrl[gDmaCtl[chnum].dmaChanRx].config = ( BCM63138_IUDMA_CONFIG_BURSTHALT );
      ...
      /* Set the burstHalt bit while clearing endma bit */
      iuDmap->ctrl[gDmaCtl[chnum].dmaChanTx].config = ( BCM63138_IUDMA_CONFIG_BURSTHALT );

      while( (iuDmap->ctrl[gDmaCtl[chnum].dmaChanTx].config & BCM63138_IUDMA_CONFIG_ENDMA) != 0x00000000 )
      {
         /* Set the burstHalt bit while clearing endma bit */
         iuDmap->ctrl[gDmaCtl[chnum].dmaChanTx].config = ( BCM63138_IUDMA_CONFIG_BURSTHALT );
...
/*****************************************************************************
*
*  FUNCTION: pcm63138_timeslotAlloc
*
*  PURPOSE:
