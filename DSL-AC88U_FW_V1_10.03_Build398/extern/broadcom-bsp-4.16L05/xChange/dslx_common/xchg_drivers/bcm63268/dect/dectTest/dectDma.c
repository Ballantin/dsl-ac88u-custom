...
XDRV_UINT8          rxBufferArea = sizeof(XDRV_UINT32)*(DECT_DMA_NUM_CHAN * DECT_DMA_BUFFER_SIZE_BYTES/4) + (DECT_DMA_ALIGNMENT_OVERHEAD/4);
XDRV_UINT8          txBufferArea = sizeof(XDRV_UINT32)*(DECT_DMA_NUM_CHAN * DECT_DMA_BUFFER_SIZE_BYTES/4) + (DECT_DMA_ALIGNMENT_OVERHEAD/4);
...
volatile XDRV_UINT32 * pDectTxDmaAHBStartAddr[DECT_DMA_NUM_CHAN];
volatile XDRV_UINT32 * pDectRxDmaAHBStartAddr[DECT_DMA_NUM_CHAN];
...
void dectDMATestStart( void )
{
   ...
   gprint("RxDMABuff: 0x%08x\n", (XDRV_UINT32)dectRxDMABuff, 0, 0);
   dectTxDMABuff = dma_alloc_coherent( NULL, txBufferArea, &dmaBufHandleTx, GFP_KERNEL );
   gprint("TxDMABuff: 0x%08x\n", (XDRV_UINT32)dectTxDMABuff, 0, 0);
...
void dectDMAConfig()
{
   ...
   for ( i=0; i < DECT_DMA_NUM_CHAN; i++ )
   {
      if ( i == 0 )
      {
        ...
        // pDectTxDmaDDRStartAddr[i] = (volatile XDRV_UINT32 *) ALIGN_DATA( (XDRV_UINT32) &dectTxDMABuff[0], DMA_UBUS_BYTES );
        // pDectRxDmaDDRStartAddr[i] = (volatile XDRV_UINT32 *) ALIGN_DATA( (XDRV_UINT32) &dectRxDMABuff[0], DMA_UBUS_BYTES );
         pDectTxDmaDDRStartAddr[i] = (volatile XDRV_UINT32 *) ALIGN_DATA( (XDRV_UINT32) dectTxDMABuff, DMA_UBUS_BYTES );
         pDectRxDmaDDRStartAddr[i] = (volatile XDRV_UINT32 *) ALIGN_DATA( (XDRV_UINT32) dectRxDMABuff, DMA_UBUS_BYTES );     
             
         /* AHB addresses */         
         pDectTxDmaAHBStartAddr[i] = (XDRV_UINT32 *)DECT_AHB_SHARED_RAM_BASE;
         pDectRxDmaAHBStartAddr[i] = pDectTxDmaAHBStartAddr[i] + (DECT_DMA_NUM_CHAN*DECT_DMA_BUFFER_SIZE_WORDS );
      }
      else
      {
         /* DDR */
         pDectTxDmaDDRStartAddr[i] = pDectTxDmaDDRStartAddr[i-1] + DECT_DMA_BUFFER_SIZE_WORDS;
         pDectRxDmaDDRStartAddr[i] = pDectRxDmaDDRStartAddr[i-1] + DECT_DMA_BUFFER_SIZE_WORDS;
         
         /* AHB */
         pDectTxDmaAHBStartAddr[i] = pDectTxDmaAHBStartAddr[i-1] + DECT_DMA_BUFFER_SIZE_WORDS;
         pDectRxDmaAHBStartAddr[i] = pDectRxDmaAHBStartAddr[i-1] + DECT_DMA_BUFFER_SIZE_WORDS;
      }
   }
   ...
   DECT_DMA_CTRL->dect_shm_dma_ctrl |= RX_TRIG_FIRST;
   ...
   for ( i=0; i<DECT_DMA_NUM_CHAN; i++ )
   {
      ...
      {
         *(pDectTxDmaDDRStartAddr[i] + j) =  0x01234567;
         *(pDectRxDmaDDRStartAddr[i] + j) =  0x01010101;
         *(pDectTxDmaAHBStartAddr[i] + j) =  0x02020202;
         *(pDectRxDmaAHBStartAddr[i] + j) =  0xFEDCBA98;                  
      }
   }
...
void dectDMATest()
{
   ...
   DECT_DMA_CTRL->dect_shm_dma_ctrl |= ( RX_INT_TRIG_EN | TX_INT_TRIG_EN ); 
   ...
   DECT_DMA_CTRL->dect_shm_dma_ctrl |= ( RX_CNT_TRIG_EN | TX_CNT_TRIG_EN );
#endif
   ...
   DECT_DMA_CTRL->dect_shm_dma_ctrl |= TRIG_CNT_CLK_SEL_PCM;
   DECT_DMA_CTRL->dect_shm_dma_ctrl |= ( RX_CNT_TRIG_EN | TX_CNT_TRIG_EN );
   ...
   DECT_DMA_CTRL->dect_shm_dma_trig_cnt_preset = 0x12FC00; /* 0x12FC00 is 10 milliseconds at DECT clock (12*10.368 MHz = 124.416 MHz) */
   DECT_DMA_CTRL->dect_shm_dma_ctrl |= TRIG_CNT_IRQ_EN;
   DECT_DMA_CTRL->dect_shm_dma_ctrl |= ( RX_CNT_TRIG_EN | TX_CNT_TRIG_EN );
   ...
   gprint("\n###################### TEST COMPLETE ###############################\n", 0, 0, 0);
   gprint("After test: MIPS count 0x%08x, PCM pulse count 0x%08x, DECT pulse count 0x%08x\n", 
           timerGetCountMips(), DECT_CTRL->dect_shm_pcm_clk_cntr, DECT_CTRL->dect_shm_dect_clk_cntr_sh);
   dumpDataMemBytes();
   //dumpDataMemWords();
   gprint("\n********************** POST TEST **********************************\n", 0, 0, 0);
   ...
   for ( i=0; i<DECT_DMA_NUM_CHAN; i++ )
   {
      for ( j=0; j<DECT_DMA_BUFFER_SIZE_WORDS; j++ )
      {
         gprint("%d TxD 0x%08x, TxA 0x%08x, ",  i,
                                                *(pDectTxDmaDDRStartAddr[i] + j),        
                                                *(pDectTxDmaAHBStartAddr[i] + j)       );                                                               
         gprint("RxA 0x%08x, RxD 0x%08x %d\n",  *(pDectRxDmaAHBStartAddr[i] + j),        
                                                *(pDectRxDmaDDRStartAddr[i] + j), j    );                  
      }
      gprint("\n",0,0,0);
   }
}
   ...
   for ( i=0; i<DECT_DMA_NUM_CHAN; i++ )
   {
      for ( j=0; j<DECT_DMA_BUFFER_SIZE_BYTES; j++ )
      {
         gprint("%d TxD 0x%02x, TxA 0x%02x, ",  i,
                                                *( (( unsigned char *)pDectTxDmaDDRStartAddr[i]) + j),
                                                *( (( unsigned char *)pDectTxDmaAHBStartAddr[i]) + j)       );                                                                        
         gprint("RxA 0x%02x, RxD 0x%02x %d\n",  *( (( unsigned char *)pDectRxDmaAHBStartAddr[i]) + j),
                                                *( (( unsigned char *)pDectRxDmaDDRStartAddr[i]) + j), j    );
      }
      gprint("\n",0,0,0);
   }
}
