...
// start of test
int dectip_gauss_data063(void)
{ 
 BYTE j;
// start of test
SetWord (DEBUGGER_REG, 0xff);

SetWord (BMC_CTRL2_REG, 0x0001); 
SetWord (BMC_TX_SFIELDL_REG, 0x0000); 
SetWord (BMC_TX_SFIELDH_REG, 0x00FF); 

SetWord (CLK_PLL_CTRL_REG, 0x000C);
/* BMC RC data */
SetWord (SHARED_RAM_START + DIP_BANK_ADDR + 0x80, 0xFF21); 
SetWord (SHARED_RAM_START + DIP_BANK_ADDR + 0x82, 0xFF00);
...
// 450 mV
   SetWord (BMC_TX_SFIELDL_REG, 0x0015); 
   while(GetWord(DIP_SLOT_NUMBER_REG) != 0x02)
     {}

// 900 mV
   SetWord (BMC_TX_SFIELDL_REG, 0x003F); 
   while(GetWord(DIP_SLOT_NUMBER_REG) != 0x03)
     {}
