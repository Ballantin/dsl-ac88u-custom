                   ...
                   unsigned *pNumTxfr )
{
   ...
   if( pMspiRegs->spiIntMask != 0x00 )
   {
      if( bosSemTimedTake( &mspiIsrSem, 100 ))
      {
   ...
   */
   ...
   while( bytesRemaining > 0 )
   {
      ...
      */
      if( bytesRemaining >= ( BCM_MSPI_TX_QUEUE_SIZE - 2 ))
      {
      ...
      */
...
#endif /* MSPI_USE_INTERRUPTS */
      {
        /* Clear MSPI finished flags and begin transmission */
         ...
         while(!(pMspiRegs->spiIntStatus & MMR_MSPI_INTSTATUS_CMNDDONE));
         /*
         ** Read out rx data
         */
         for( indx=0; indx < numBytesToXfer; indx++ )
