...
static const BCM_MSPI6818_REGS mspi6818DefaultRegs =
{
   ...
   BCM_MSPI6818_CTRL_MODE_INTERRUPT  /* Interrupt mode */
#else
   BCM_MSPI6818_CTRL_MODE_POLLING /*Polling mode*/
#endif /* MSPI6818_USE_INTERRUPTS */
};

/* ---- Private Variables ------------------------------------ */
/* ---- Private Function Prototypes -------------------------- */
static int setMspiRegs( BCM_MSPI6818_DRV *pMspiDrv, BCM_MSPI6818_REGS *pNewMspiRegs );
static int getMspiRegs( BCM_MSPI6818_DRV *pMspiDrv, BCM_MSPI6818_REGS *pNewMspiRegs );
...
*****************************************************************************
*/
int mspiIOctl( BCM_MSPI6818_CMD command, void *cmdp )
{
   int result;
   ...
   switch ( command )
   {
      case BCM_MSPI6818_INITREGS_CMD:
      {
         result = setMspiRegs( pMspiDrv, (BCM_MSPI6818_REGS *)&mspi6818DefaultRegs );
      }
      break;

      case BCM_MSPI6818_SETREGS_CMD:
      {
         result = setMspiRegs( pMspiDrv, cmdp );
      }
      break;

      case BCM_MSPI6818_GETREGS_CMD:
      {
         result = getMspiRegs( pMspiDrv, cmdp );
      }
      break;

      case BCM_MSPI6818_RXCALLBACK_INSTALL_CMD:
      {
      ...
      case BCM_MSPI6818_TXCALLBACK_INSTALL_CMD:
      {
...
*****************************************************************************
*/
static int setMspiRegs( BCM_MSPI6818_DRV *pMspiDrv, 
                        BCM_MSPI6818_REGS *pNewMspiRegs )
{
   volatile SpiControl *pMspiHwRegs = pMspiDrv->p6818SpiRegs;

   /* If MSPI is busy then abort */
   if (( pMspiHwRegs->spiStatus & MMR6818_MSPI_STATUS_SERBUSY ) || ( pMspiHwRegs->spiStatus & MMR6818_MSPI_STATUS_CMDBUSY ))
   {
   ...
   */
   if( pNewMspiRegs->mspiCtrlMode == BCM_MSPI6818_CTRL_MODE_POLLING )
   {
      XDRV_LOG_DBG((XDRV_LOG_MOD_XDRV, "INFO: MSPI POLLING MODE"));

      pMspiHwRegs->spiIntMask = 0x00;
      mspi6818intc_IrqDisable();
   }
#if MSPI6818_USE_INTERRUPTS
   else
   {
      ...
      mspi6818intc_IrqDisable();
      pMspiHwRegs->spiIntStatus |= MMR6818_MSPI_INTSTATUS_MASK;
...
*****************************************************************************
*/
static int getMspiRegs( BCM_MSPI6818_DRV *pMspiDrv, 
                        BCM_MSPI6818_REGS *pNewMspiRegs )
{
   volatile SpiControl *pMspiHwRegs = pMspiDrv->p6818SpiRegs;
   ...
   {
      pNewMspiRegs->mspiCtrlMode = BCM_MSPI6818_CTRL_MODE_INTERRUPT;
   }
   else
   {
      pNewMspiRegs->mspiCtrlMode = BCM_MSPI6818_CTRL_MODE_POLLING;
   }
