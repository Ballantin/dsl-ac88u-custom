...
#define BMU6818_MAX_CURRENT              32000
#define BMU6818_MIN_CURRENT             -32000
...
XDRV_SINT16 linetestReadSignal( int chan, XDRV_UINT32 measureType, TPD_MEASURE_RESULTS *tpdResults )
{
   APM_6818_DRV *pDev = (APM_6818_DRV *)pApmDriver[chan];
...
static XDRV_SINT16 linetestReadSignalSettle( int chan, XDRV_UINT32 measureType, TPD_MEASURE_RESULTS *tpdResults, XDRV_BOOL voltage )
{
   ...
   previousValue = BMU6818_MAX_CURRENT;
...
XDRV_UINT16 linetestDropResFlt(int chan, HSZDROPRSLT_RFLT *rfltRsltp )
{
   ...
   IMTfh = BMU6818_MAX_CURRENT;
   IMTrh = BMU6818_MAX_CURRENT;
   IMTfl = BMU6818_MAX_CURRENT;
   IMTrl = BMU6818_MAX_CURRENT;
   VToh = BMU6818_MAX_CURRENT;
   VTol = BMU6818_MAX_CURRENT;
   ...
   else if (( abs(IMTrl) > ( BMU6818_MAX_CURRENT / 2 ) ) &&
            ( abs(IMTrh) > ( BMU6818_MAX_CURRENT ) ) &&
            ( abs(IMTol) < ( BMU6818_MAX_CURRENT / 200 ) ) &&
            ( abs(IMToh) < ( BMU6818_MAX_CURRENT / 200 ) ) &&
            ( IMTfl < 0 ))
   {
      Rrg = 1000000;
      Rtr = 1000000;
   ...
   else if (( abs(IMTfl) > ( BMU6818_MAX_CURRENT * 5 / 8 ) ) &&
            ( abs(IMTfh) > ( BMU6818_MAX_CURRENT ) ) &&
            ( abs(IMTol) > ( BMU6818_MAX_CURRENT * 3 / 8 ) ) &&
            ( abs(IMToh) > ( BMU6818_MAX_CURRENT / 2 ) ))
   {
      Rtg = 1000000;
      Rtr = 1000000;
      Rrg = 0;
