...
static void slic6818Si3239ModeControl( XDRV_SLIC *pDrv, XDRV_SLIC_MODE mode );
static void slic6818Si3239LedControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static void slic6818Si3239PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slic6818Si3239IsOffhook( XDRV_SLIC *pDrv );
static int slic6818Si3239SetRingParms( XDRV_SLIC *pDrv, int ringFrequency, int ringWaveshape,
                                       int ringVoltage, int ringOffset, int ringOffsetCal );
static int slic6818Si3239GetRingParms( XDRV_SLIC *pDrv, int *ringFrequency, int *ringWaveshape,
                                       int *ringVoltage, int *ringOffset, int *ringOffsetCal );
static int slic6818Si3239SetBoostedLoopCurrent( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static int slic6818Si3239SetPowerSource( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slic6818Si3239GetOverCurrentStatus( XDRV_SLIC *pDrv );
static XDRV_SINT16 slic6818Si3239GetDlp( XDRV_SLIC *pDrv );
static void slic6818Si3239ProcessEvents( XDRV_SLIC *pDrv );
static XDRV_UINT32 slic6818Si3239Probe( XDRV_SLIC *pDrv, XDRV_UINT32 deviceId, XDRV_UINT32 chan, XDRV_UINT32 reg,
                                      XDRV_UINT32 regSize, void* value, XDRV_UINT32 valueSize, XDRV_UINT32 indirect, 
                                      XDRV_UINT8 set );
static int slic6818Si3239GetSlicParms( XDRV_SLIC *pDrv, int *phaseReversal, int *loopCurrent, int *powerSource, int *slicMode );
/*
...
static const XDRV_SLIC_FUNCS slic6818Si3239DrvFuncs =
{
   slic6818Si3239ModeControl,
   slic6818Si3239LedControl,
   slic6818Si3239PhaseReversalControl,
   slic6818Si3239IsOffhook,
   slic6818Si3239SetRingParms,
   slic6818Si3239GetRingParms,
   slic6818Si3239SetBoostedLoopCurrent,
   slic6818Si3239SetPowerSource,
   slic6818Si3239GetOverCurrentStatus,
   slic6818Si3239GetDlp,
   slic6818Si3239ProcessEvents,
   slic6818Si3239Probe,
   slic6818Si3239GetSlicParms
};

/* Private Functions */
static void OpenSlic( SLIC_6818_SI3239_DRV *pDev );
static void CloseSlic( SLIC_6818_SI3239_DRV *pDev );
static void InitSlicRegs( SLIC_6818_SI3239_DRV *pDev );
...
*****************************************************************************
*/
void slic6818Si3239Init
(
   int                   chan,
   SLIC_6818_SI3239_DRV *pDev,
   const SLICCFG_6818_SI3239_INFO *pSlicCfgInfo,
   XDRV_APM             *pApmDrv
)
{
   int status;
   ...
   pDev->chan = chan;
   pDev->pDrvFuncs = &slic6818Si3239DrvFuncs;
...
*****************************************************************************
*/
void slic6818Si3239Deinit( SLIC_6818_SI3239_DRV *pDev )
{
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239IsOffhook
**
...
static XDRV_BOOL slic6818Si3239IsOffhook( XDRV_SLIC *pDrv )
{
   SLIC_6818_SI3239_DRV *pDev = (SLIC_6818_SI3239_DRV *)pDrv;
   int                   chan = pDev->chan;
   XDRV_UINT16           rc;
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239ModeControl
**
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239LedControl
**
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239PhaseReversalControl
**
...
static void slic6818Si3239PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value )
{
   ...
   {
      slic6818Si3239ModeControl( pDrv, pDev->slicMode );
   }
}

/*
*****************************************************************************
** FUNCTION:   slic6818Si3239SetRingParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239GetRingParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239GetSlicParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239SetBoostedLoopCurrent
**
...
/*
******************************************************************************
** FUNCTION:  slic6818Si3239RegisterRead
**
** PURPOSE:   Read from a memory address via MSPI
**
** PARAMETERS:
...
*****************************************************************************
*/
void slic6818Si3239RegisterRead( int chan, XDRV_UINT8 addr, XDRV_UINT16 *data )
{
...
/*
******************************************************************************
** FUNCTION:  slic6818Si3239RegisterWrite
**
** PURPOSE:   Write to a memory address via MSPI
**
** PARAMETERS:
...
*****************************************************************************
*/
void slic6818Si3239RegisterWrite( int chan, XDRV_UINT8 addr, XDRV_UINT16 data )
{
...
/*
******************************************************************************
** FUNCTION:  slic6818Si3239RegisterMaskWrite
**
** PURPOSE:   Write to the masked location of a memory address via MSPI
**            and verify the write if requested to
**
...
*****************************************************************************
*/
void slic6818Si3239RegisterMaskWrite( int chan, XDRV_UINT8 addr, XDRV_UINT16 data, XDRV_UINT16 mask, int verify )
{
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239SetPowerSource
**
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239GetOverCurrentStatus
**
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239SetSpecialMode
**
...
*****************************************************************************
*/
int slic6818Si3239SetSpecialMode( SLIC_6818_SI3239_DRV *pDev, SLIC_6818_SI3239_SPECIAL_MODE mode, XDRV_BOOL enable )
{
   switch( mode )
   {
      case SLIC_6818_SI3239_SPECIAL_DISABLE_RING_TRANISTION_MODE:
      {
...
/*
*****************************************************************************
** FUNCTION:   slic6818Si3239UpdateState
**
...
*****************************************************************************
*/
void slic6818Si3239UpdateState( SLIC_6818_SI3239_DRV *pDev )
{
   if( pDev->pendingSlicTime > 0 )
   {
