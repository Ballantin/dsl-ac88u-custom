...
****************************************************************************
*
*    Description:  APM (Audio Processor Module) device driver for BCM6818
*
****************************************************************************/
...
#define APM6818_RING_FREQ_MAX         75
#define APM6818_RING_FREQ_MIN         15
#define APM6818_RING_FREQ_SCALE       32768 / 3125   /* 2^20/100kHz */

/* Maximum iterations to wait for DPLL lock */
#define DPLL_LOCK_MAX_WAIT            20000000
#define DPLL_RETRY_CNT                2           /* Number of times to retry in case of PLL lock fails*/

/* Program all channels */
#define APM6818_ALL_CHANNELS          0xFF

typedef struct
{
   int clkSource;                      /* APM clock source */
   int sampleRate;                     /* Sampling frequency (in kHz) */
   XDRV_SINT16 eq_rx_delta_offset;     /* Rx Equalizer preceeds the ASRC */
   XDRV_SINT16 asrc_int_delta_offset;  /* Offset in delta value going to horner engine relative to fir engine */
...
typedef struct APM6818_DMA_SIMPLEX_CHANNEL_INFO
{
   int                  iuChanNum;     /* IUDMA channel number assosiated with this APM channel */
   BCM6818_IUDMA_DESC   *dmaDescp;     /* Where are the descriptors for this channel? */
   DMA_HANDLE           dmaDescHandle;
   APM6818_DMA_BUFFER   *dmaDatap;     /* Where are the DMA buffers to store/fetch the data to/from for each channel */
   DMA_HANDLE           dmaBufHandle;
} APM6818_DMA_SIMPLEX_CHANNEL_INFO;

/* DMA Channel info (duplex - both ways) for easy access to DMA channels */
typedef struct APM6818_DMA_DUPLEX_CHANNEL_INFO
{
   APM6818_DMA_SIMPLEX_CHANNEL_INFO rxChan;  /* RX channel */
   APM6818_DMA_SIMPLEX_CHANNEL_INFO txChan;  /* TX channel */
} APM6818_DMA_DUPLEX_CHANNEL_INFO;

/* Ring Gain Settings */
typedef struct APM6818_RING_GAIN_INFO
{
...
} APM6818_RING_GAIN_INFO;

/* asin table struct */
typedef struct APM6818_ASIN
{
   XDRV_SINT32   value;
   XDRV_SINT32   angle;
} APM6818_ASIN;
...
static APM6818_DMA_BUFFER dmaRxData[APM6818_NUM_CHANNELS*DMA_RX_NUM_BUFFERS];
static APM6818_DMA_BUFFER dmaTxData[APM6818_NUM_CHANNELS*DMA_TX_NUM_BUFFERS];
...
static APM6818_DMA_DUPLEX_CHANNEL_INFO gApmDmaInfo[APM6818_NUM_CHANNELS] =
{
   {
      /* RX Channel 0 */
      {
         BCM6818_APM_IUDMA_RX_A,
...
{
   {  0,   8,  0x1D00,  0x2B00,   0x10000000,    0,  0x0A7C5AC4 },
   {  0,  16,  0x3A00,  0x5600,   0x20000000,    0,  0x14F8B589 },
    { -1,  -1,      -1,      -1,         -1,    -1,          -1 }
...
static const APM6818_RING_GAIN_INFO ringGainTable[] =
{
   { 0x3239,  58,  10,  114 },
   { 0x32392, 63,  10,  108 },
   { 0x9530,  18,  50,  150 },
...
static const APM6818_ASIN apmAsinTable[]=
{
...
static int apm6818_dmaInit( volatile BCM6818_APM *apmp, volatile BCM6818_IUDMA *iuDmap, int sampleRate, int packetRate);
static int apm6818_dmaDeinit( volatile BCM6818_APM *apmp, volatile BCM6818_IUDMA *iuDmap);
static int apm6818_regInit(volatile BCM6818_APM *pApm, const APM6818_CFG *pCfg );
static int apm_pllStart(volatile BCM6818_APM *pApm);
static int initIudma( volatile BCM6818_IUDMA *iuDmap);
static int deinitIudma( volatile BCM6818_IUDMA *iuDmap );
...
#endif
static int WriteApmFilterCoeffs( volatile BCM6818_APM *pApm, int chan, APM6818_FILTER_TYPE filter, int *data, int length );
static XDRV_SINT64 divRound( XDRV_SINT64 dividend, XDRV_SINT64 divisor );

/* APM "meta-driver" interface functions */
static void apm6818SetRingParms( XDRV_APM *pDrv, int ringFrequency, int ringWaveshape, int ringVoltage, int ringOffset, int ringOffsetCal );
static void apm6818GetRingParms( XDRV_APM *pDrv, int *ringFrequency, int *ringWaveshape, int *ringVoltage, int *ringOffset, int *ringOffsetCal );
static XDRV_SINT32 apm6818DebugStatusDump( XDRV_APM *pDrv );
static XDRV_SINT16 apm6818GetDlp( XDRV_APM *pDrv);
static XDRV_SINT16 apm6818GetElp( XDRV_APM *pDrv);
static void apm6818PulseSignal( XDRV_APM *pDrv, int endpt, int duration, int period,
                                int repetition, int amplitude, int frequency );
static void apm6818SetHPF( XDRV_APM *pDrv, int endpt, XDRV_BOOL enable );
static int apm6818SetTxRxGains( XDRV_APM *pDrv, int chan, int txGain, int rxGain, int *apmTxGain, int *apmRxGain );
static XDRV_SINT64 apmAsin(XDRV_SINT32 value);

/* APM interface */
static const XDRV_APM_FUNCS bcm6818ApmDrvFuncs =
{
   apm6818SetRingParms,
   apm6818GetRingParms,
   apm6818GetDlp,
   apm6818DebugStatusDump,
   apm6818GetElp,
   hvg6818GetOverCurrentStatus,
   hvg6818GetOverVoltageStatus,
   apm6818PulseSignal,
   hvg6818SetVoltage,
   apm6818SetHPF,
   hvg6818UpdateSlicStatus,
   hvg6818UpdateHookStatus,
   hvg6818UpdateHookStatusEnable,
   apm6818SetTxRxGains
};

/* ---- Public Functions -------------------------------------------------- */
/* ---- Private Functions ------------------------------------------------- */
#define APM6818_USLEEP(cycles) ( {volatile int i; for (i=0; i < (cycles); i++) {}} )

/*****************************************************************************
*
*  FUNCTION: apm6818_init
*
*  PURPOSE:
*      Intialize 6818 APM Driver interface
*
*  PARAMETERS:
*      pDrv      - driver interface
*      pIntCtl   - pointer to 6818's Interrupt Control module
...
int apm6818_init(
   ...
   const APM6818_CFG *cfgp
)
{
   int result;

   /* Set the "meta-driver" interface */
   pDrv->pDrvFuncs = &bcm6818ApmDrvFuncs;

   /* Initialize the driver state variables */
   pDrv->pApm = BCM6818_APM_PTR;
   pDrv->pIuDma = BCM6818_APM_IUDMA_PTR;
   pDrv->pIntCtl = pIntCtl;
   pDrv->Cfg = *cfgp;

   /* Now start the APM module */
   result = apm6818_start(pDrv);
...
/*****************************************************************************
*
*  FUNCTION: apm6818_deinit
*
*  PURPOSE:
*      DeIntialize 6818 APM Driver interface
*
*  PARAMETERS:
*      pDrv       - 6818 driver interface
*
*  RETURNS: 0 on SUCCESS
*
*  NOTES:
*
*****************************************************************************/
int apm6818_deinit( APM_6818_DRV *pDrv )
{
   /* First stop the APM */
   apm6818_stop(pDrv);

   /* Clear the driver state */
   memset(pDrv,0, sizeof(APM_6818_DRV));
...
/*****************************************************************************
*
*  FUNCTION: apm6818_start
*
*  PURPOSE:
*      Initialize and start the actual 6818 Audio Processing Module
*
*  PARAMETERS:
*      pDrv       - 6818 driver interface
*
...
*
*****************************************************************************/
int apm6818_start( APM_6818_DRV  *pDrv )
{
   volatile int i;

   /* Enable APM block and both channels */
   pDrv->pIntCtl->blkEnables |= APM_AUDIO_UBUS_CLKEN |
                                APM_AUDIO_A_CLKEN |
                                APM_AUDIO_B_CLKEN |
                                APM_BMU_CLKEN |
                                APM_AUDIO_COMMON_CLKEN;

   PERF->softResetB &= ~SOFT_RST_APM;
   PERF->softResetB &= ~SOFT_RST_ACP;
   PERF->softResetB &= ~SOFT_RST_HVG;
   PERF->softResetB &= ~SOFT_RST_BMU;
   ...
   */
   if ( apm6818_regInit( pDrv->pApm, &pDrv->Cfg ))
   {
      return ( -1 );
   }
   ...
   */
   if ( apm6818_flexicalcConfig( pDrv->pApm, pDrv->Cfg.pFlexiCalc ))
   {
   ...
   ** Launch the APM DMAs
   */
   if ( apm6818_dmaInit( pDrv->pApm, pDrv->pIuDma, pDrv->Cfg.sampleRate, pDrv->Cfg.packetRate ))
   {
...
/*****************************************************************************
*
*  FUNCTION: apm6818_stop
*
*  PURPOSE:
*      Stop and deinitialize the 6818 Audio Processing Module
*
*  PARAMETERS:
*      pDrv       - 6818 driver interface
*
...
*
*****************************************************************************/
int apm6818_stop( APM_6818_DRV *pDrv )
{
   /*
   ** Stop the APM DMA
   */
   if (apm6818_dmaDeinit( pDrv->pApm, pDrv->pIuDma))
   {
...
/*****************************************************************************
*
*  FUNCTION: apm6818_dmaInit
*
*  PURPOSE:
*
*  PARAMETERS:
*      apmp       - pointer to 6818's APM module
*      iuDmap     - pointer to 6818's APM IUDMA module
...
*
*****************************************************************************/
static int apm6818_dmaInit
(
   volatile BCM6818_APM    *apmp,
   volatile BCM6818_IUDMA  *iuDmap,
   int                     sampleRate,
   int                     packetRate
)
{
...
{
   XDRV_UINT8                       i,chnum;
   volatile BCM6818_IUDMA_DESC*     dmaTxDesc;
   volatile BCM6818_IUDMA_DESC*     dmaRxDesc;
   volatile BCM6818_IUDMA_DESC*     dmaTxDesc_c;
   volatile BCM6818_IUDMA_DESC*     dmaRxDesc_c;
   volatile APM6818_DMA_BUFFER*     dmaRxData;
   volatile APM6818_DMA_BUFFER*     dmaTxData;
   APM6818_DMA_SIMPLEX_CHANNEL_INFO *dmaChanInfop;
   ...
   rxDescriptorArea = sizeof(BCM6818_IUDMA_DESC) * APM6818_NUM_CHANNELS * DMA_RX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6818_DMA_UBUS_BYTES;
   txDescriptorArea = sizeof(BCM6818_IUDMA_DESC) * APM6818_NUM_CHANNELS * DMA_TX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6818_DMA_UBUS_BYTES;
...
#else
   dmaRxDesc_c = (volatile BCM6818_IUDMA_DESC *)dma_alloc_coherent( NULL, rxDescriptorArea, &gApmDmaInfo[0].rxChan.dmaDescHandle, GFP_KERNEL );
   dmaTxDesc_c = (volatile BCM6818_IUDMA_DESC *)dma_alloc_coherent( NULL, txDescriptorArea, &gApmDmaInfo[0].txChan.dmaDescHandle, GFP_KERNEL );
#endif
   ...
   for (i=0; i < APM6818_NUM_CHANNELS; i++)
   {
      ...
      dmaRxDesc_c = (volatile BCM6818_IUDMA_DESC *) ALIGN_DATA( (unsigned int) dmaRxDesc_c, BCM6818_DMA_UBUS_BYTES );
      dmaTxDesc_c = (volatile BCM6818_IUDMA_DESC *) ALIGN_DATA( (unsigned int) dmaTxDesc_c, BCM6818_DMA_UBUS_BYTES );

      gApmDmaInfo[i].rxChan.dmaDescp = (BCM6818_IUDMA_DESC*)(dmaRxDesc_c + i * DMA_RX_NUM_BUFFERS);
      gApmDmaInfo[i].txChan.dmaDescp = (BCM6818_IUDMA_DESC*)(dmaTxDesc_c + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   rxBufferArea = sizeof(XDRV_UINT32) * (APM6818_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * APM6818_NUM_CHANNELS * DMA_RX_NUM_BUFFERS;
   txBufferArea = sizeof(XDRV_UINT32) * (APM6818_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * APM6818_NUM_CHANNELS * DMA_TX_NUM_BUFFERS;
   ...
   for (i=0; i < APM6818_NUM_CHANNELS; i++)
     {
      gApmDmaInfo[i].rxChan.dmaDatap = (APM6818_DMA_BUFFER*)(dmaRxData + i * DMA_RX_NUM_BUFFERS);
      gApmDmaInfo[i].txChan.dmaDatap = (APM6818_DMA_BUFFER*)(dmaTxData + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   for (chnum=0; chnum<APM6818_NUM_CHANNELS; chnum++)
   {
      ...
      dmaTxDesc = (volatile BCM6818_IUDMA_DESC *)CacheToNonCacheVirtAddress((XDRV_UINT32)dmaChanInfop->dmaDescp);
      ...
      for( i = 0; i < DMA_TX_NUM_BUFFERS; i++ )
      {
         ...
         dmaTxDesc[i].flagsLengthStatus.status = BCM6818_IUDMA_DESC_STATUS_SOP |
                                                 BCM6818_IUDMA_DESC_STATUS_EOP;

         if( i == 0 )
         {
            /* First buffer - prime the TX fifo */
            dmaTxDesc[i].flagsLengthStatus.status |= BCM6818_IUDMA_DESC_STATUS_OWN;
            ...
            bufferSizeBytes = (((packetRate+1)*sampleRate)<<1) + APM6818_TX_DMA_BUF_DEPTH_BYTES;
         }

         if( i == ( DMA_TX_NUM_BUFFERS - 1 ) )
         {
            /* Last buffer - set the WRAP bit */
            dmaTxDesc[i].flagsLengthStatus.status |= BCM6818_IUDMA_DESC_STATUS_WRAP;
         }

         /* Check if we can accomodate the desired buffer size */
         if (bufferSizeBytes <= APM6818_DMA_MAX_BUFFER_SIZE_BYTES)
         {
         ...
         else
         {
            ...
            ** You can either increase the compile time buffer size (APM6818_DMA_MAX_BUFFER_SIZE),
         ...
         ** buffers aligned on the cache-line size (16-bytes for the 6818).
      ...
      dmaRxDesc = (volatile BCM6818_IUDMA_DESC *)CacheToNonCacheVirtAddress((XDRV_UINT32)dmaChanInfop->dmaDescp);
      ...
      for( i = 0; i < DMA_RX_NUM_BUFFERS; i++ )
      {
         ...
         dmaRxDesc[i].flagsLengthStatus.status = BCM6818_IUDMA_DESC_STATUS_OWN;

         if( i == ( DMA_RX_NUM_BUFFERS - 1 ) )
         {
            /* Last descriptor - set the wrap bit */
            dmaRxDesc[i].flagsLengthStatus.status |= BCM6818_IUDMA_DESC_STATUS_WRAP;
         }
...
{
   int chnum;
   volatile BCM6818_IUDMA_STRAM *stRamp = iuDmap->stram;
  volatile unsigned short test1 = 0;

   /* Reset entire IUDMA state RAM */
   memset( (void*)stRamp, 0, BCM6818_MAX_APM_PCM_DMA_CHANNELS * sizeof(BCM6818_IUDMA_STRAM) );
   ...
   for (chnum=0; chnum<APM6818_NUM_CHANNELS; chnum++)
   {
      ...
      /* Init RX side */
      iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].maxBurst = 4; /* in 32-bit words */
   ...
   test1 = iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intStat;
   iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intStat = test1;
//      iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intStat |= iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intStat;
   iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intMask = 0;
   iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intMask = 1;
   iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intMask = 4;
   iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intMask = 5;
      iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intMask = 0x000f & (BCM6818_IUDMA_INTMASK_BDONE | BCM6818_IUDMA_INTMASK_NOTVLD);
      ...
      /* Init TX side */
      iuDmap->ctrl[gApmDmaInfo[chnum].txChan.iuChanNum].maxBurst = 4; /* in 32-bit words */
      ...
      iuDmap->ctrl[gApmDmaInfo[chnum].txChan.iuChanNum].intMask = BCM6818_IUDMA_INTMASK_BDONE | BCM6818_IUDMA_INTMASK_NOTVLD;
      iuDmap->regs.gbl_int_mask |= ( 1 << gApmDmaInfo[chnum].txChan.iuChanNum );

   }

   /* Enable the IUDMA master */
   iuDmap->regs.ctrlConfig = BCM6818_IUDMA_REGS_CTRLCONFIG_MASTER_EN;
...
*
*****************************************************************************/
static int apm6818_dmaDeinit
(
   volatile BCM6818_APM *apmp,
   volatile BCM6818_IUDMA *dmap
)
{
   /* Stop the IUDMA */
   deinitIudma( dmap );
...
*
*  PARAMETERS:
*      pApm     - pointer to 6818's APM module
*
...
static int apm_pllStart(volatile BCM6818_APM *pApm)
{
 ...
 retry_lock:
   ...
   volatile XDRV_UINT32 *pSerdesMisc=(volatile XDRV_UINT32 *)GPON_SERDES_MISC;
   *pSerdesMisc= 5;/* 256KHz in GPON mode */
   ...
   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "%s: DPLL Locked, Iteration = %u dpll_status = 0x%08x ", __FUNCTION__, (unsigned int)i, (unsigned int) temp));
...
static int apm6818_regInit(volatile BCM6818_APM *pApm, const APM6818_CFG *pCfg )
{
   ...
   {
      XDRV_LOG_ERROR(( XDRV_LOG_MOD_XDRV, "APM6818 regInit ERROR: Unsupported sample rate (%d) !!! ", pCfg->sampleRate));
      return ( -1 );
   }

   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6818 regInit: Using %d kHz sample rate.", tablep->sampleRate));
   ...
   */
   ...
   pApm->reg_cha_int_config_2 = ((0x720A & ASRC_INT_SCALE) |
                                 ((DEFAULT_ASRC_INT_FIR_SHFT << ASRC_INT_FIR_SHFT_SHIFT) & ASRC_INT_FIR_SHFT) |
   ...
   pApm->reg_cha_asrc_dec_config = (((DEFAULT_ASRC_DEC_SCALE << ASRC_DEC_SCALE_SHIFT) & ASRC_DEC_SCALE) |
                                    ((0x13 << ASRC_DEC_NUM_SECT_SHIFT) & ASRC_DEC_NUM_SECT) |
                                    ((DEFAULT_ASRC_DEC_SHFT << ASRC_DEC_SHFT_SHIFT) & ASRC_DEC_SHFT) |
   ...
   */
   ...
   /* Set ASRC_DEC_COEFF_SEL for low frequency (8/16 kHz sampling ) */
   pApm->reg_chb_asrc_dec_config = (((DEFAULT_ASRC_DEC_SCALE << ASRC_DEC_SCALE_SHIFT) & ASRC_DEC_SCALE) |
                                    ((0x13 << ASRC_DEC_NUM_SECT_SHIFT) & ASRC_DEC_NUM_SECT) |
                                    ((DEFAULT_ASRC_DEC_SHFT << ASRC_DEC_SHFT_SHIFT) & ASRC_DEC_SHFT) |
                                    (0 << ASRC_DEC_SYM_ASYM_SHIFT ) |
                                    ((0 << ASRC_DEC_COEFF_SEL_SHIFT) & ASRC_DEC_COEFF_SEL));
   ...
   */
   ...
   {
      pApm->reg_ring_config_1 = ((0 & RING_OFFSET) |
                                ((0x4900 << RING_SCALE_SHIFT) & RING_SCALE) );
   }
   else
   {
      pApm->reg_ring_config_1 = ((0 & RING_OFFSET) |
   ...
   pApm->reg_ring_config_2 = ((0x36 & RING_OMEGA_NORM_A) |
                             ((0x36 << RING_OMEGA_NORM_B_SHIFT) & RING_OMEGA_NORM_B) |
                             ((0 << RING_CREST_FACTOR_SHIFT) & RING_CREST_FACTOR) |
                             ((1 << RING_GEN_ENABLE_A_SHIFT) & RING_GEN_ENABLE_A) |
                             ((1 << RING_GEN_ENABLE_B_SHIFT) & RING_GEN_ENABLE_B) );

   pApm->reg_ring_config_3 = ((0 & RING_BREAK_PHASE_A) |
                             ((0 << RING_BREAK_PHASE_B_SHIFT) & RING_BREAK_PHASE_B) |
                             ((0 << RING_BREAK_PHASE_SIGN_A_SHIFT) & RING_BREAK_PHASE_SIGN_A) |
                             ((0 << RING_BREAK_PHASE_SIGN_B_SHIFT) & RING_BREAK_PHASE_SIGN_B) |
                             ((0 << RING_START_CADENCE_A_SHIFT) & RING_START_CADENCE_A) |
                             ((0 << RING_START_CADENCE_B_SHIFT) & RING_START_CADENCE_B) );

   pApm->reg_ring_config_4 = ((0 & RING_STOP_IMMED_A) |
                             ((0 << RING_STOP_IMMED_B_SHIFT) & RING_STOP_IMMED_B) |
                             ((0 << RING_STOP_NEXT_BREAK_A_SHIFT) & RING_STOP_NEXT_BREAK_A) |
                             ((0 << RING_STOP_NEXT_BREAK_B_SHIFT) & RING_STOP_NEXT_BREAK_B) );
   ...
   pApm->apm_dev_irq_pend = 0xffffffff;
   ...
   pApm->reg_codec_config_2 = 0x20000000;
   pApm->reg_codec_config_3 = 0x20000000;
   pApm->reg_codec_config_4 = 0x00000000;
   pApm->reg_codec_config_5 = 0x00000000;
...
/*****************************************************************************
*
*  FUNCTION: apm6818_flexicalcConfig
*
*  PURPOSE:
...
*
*****************************************************************************/
int apm6818_flexicalcConfig
(
   volatile BCM6818_APM *pApm,
   const APM6818_FLEXICALC_CFG *flexicalcp
)
{
   ...
   if ( WriteApmFilterCoeffs( pApm, APM6818_ALL_CHANNELS, APM6818_FILTER_HISPD_HYBRID_BAL, (int *)&flexicalcp->hybal_audio_fir[0], APM6818_HYBAL_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load High-speed Hybrid Balance coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6818_ALL_CHANNELS, APM6818_FILTER_LOSPD_HYBRID_BAL, (int *)&flexicalcp->hybal_audio_fir[0], APM6818_HYBAL_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load Y-filter coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6818_ALL_CHANNELS, APM6818_FILTER_Y_FILTER, (int *)&flexicalcp->yfltr_iir2[0], APM6818_YFLTR_BLOCK_SIZE ))
   {
      return ( 1 );
   }

   /* Load RX equalizer coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6818_ALL_CHANNELS, APM6818_FILTER_RX_EQ, (int *)&flexicalcp->eq_rx_filter[0], APM6818_RX_EQ_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load TX equalizer coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6818_ALL_CHANNELS, APM6818_FILTER_TX_EQ, (int *)&flexicalcp->eq_tx_filter[0], APM6818_TX_EQ_NUM_TAPS ))
   {
      return ( 1 );
   }
   ...
   pApm->reg_cha_hybal_config |= ((( flexicalcp->hybal_shft << HISPD_HYBAL_SHFT_SHIFT ) & HISPD_HYBAL_SHFT ) |
   ...
   pApm->reg_cha_int_config_2 |= ( flexicalcp->asrc_int_scale & ASRC_INT_SCALE );

   pApm->reg_cha_asrc_dec_config &= ~( ASRC_DEC_SCALE );
   pApm->reg_cha_asrc_dec_config |= ((( flexicalcp->asrc_dec_scale * ASRC_DEC_SCALE_FACTOR ) << ASRC_DEC_SCALE_SHIFT ) & ASRC_DEC_SCALE );
   ...
   pApm->reg_chb_hybal_config |= ((( flexicalcp->hybal_shft << HISPD_HYBAL_SHFT_SHIFT ) & HISPD_HYBAL_SHFT ) |
                                  ( flexicalcp->hybal_en << HISPD_HYBAL_EN_SHIFT ) |
                                  (( flexicalcp->hybal_smpl_offset << HISPD_HYBAL_SMPL_OFFSET_SHIFT ) & HISPD_HYBAL_SMPL_OFFSET ) |
                                  ( flexicalcp->yfltr_en << YFLTR_EN_SHIFT ) |
                                  ( flexicalcp->hpf_en << HPF_EN_SHIFT ));
   ...
   pApm->reg_chb_int_config_2 |= ( flexicalcp->asrc_int_scale & ASRC_INT_SCALE );

   pApm->reg_chb_asrc_dec_config &= ~( ASRC_DEC_SCALE );
   pApm->reg_chb_asrc_dec_config |= ((( flexicalcp->asrc_dec_scale * ASRC_DEC_SCALE_FACTOR ) << ASRC_DEC_SCALE_SHIFT ) & ASRC_DEC_SCALE );

   /* Reconfigure codec registers with custom values */
   pApm->reg_codec_config_2 &= ~( PGA_CTL_VADC );
   pApm->reg_codec_config_2 |= (( flexicalcp->vtx_pg << PGA_CTL_VADC_SHIFT ) & PGA_CTL_VADC );

   pApm->reg_codec_config_3 &= ~( PGA_CTL_VADC );
   pApm->reg_codec_config_3 |= (( flexicalcp->vtx_pg << PGA_CTL_VADC_SHIFT ) & PGA_CTL_VADC );

   pApm->reg_codec_config_4 &= ~( PGA_GAIN_CTL_VDAC_A | PGA_GAIN_CTL_VDAC_B );
   pApm->reg_codec_config_4 |= ((( flexicalcp->vrx_pg << PGA_GAIN_CTL_VDAC_A_SHIFT ) & PGA_GAIN_CTL_VDAC_A ) |
...
/*
*****************************************************************************
** FUNCTION:   apm6818DebugStatusDump
**
...
static XDRV_SINT32 apm6818DebugStatusDump( XDRV_APM *pDrv )
{
   APM_6818_DRV            *pDev  = (APM_6818_DRV *)pDrv;
   volatile BCM6818_APM    *pApm;
   volatile BCM6818_IUDMA  *iuDmap;
   volatile PerfControl     *pIntCtl;
   int                      chnum;
   ...
   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "CHIPCTL Periph status info ..."));
   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "  irqMask   0x%016llx;   irqStatus   0x%016llx; ",
   (unsigned long long)(pIntCtl->IrqControl[0].IrqMask), (unsigned long long)(pIntCtl->IrqControl[0].IrqStatus)));
   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "  irqMask1   0x%016llx;   irqStatus1   0x%016llx; ",
   (unsigned long long)(pIntCtl->IrqControl[1].IrqMask), (unsigned long long)(pIntCtl->IrqControl[1].IrqStatus)));

   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, ""));
   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM reg status info ..."));
   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "  devIrq    0x%08x;   devIrqPend  0x%08lx; ",
   pApm->apm_dev_irq_status, pApm->apm_dev_irq_pend));
...
static XDRV_SINT16 apm6818GetDlp( XDRV_APM *pDrv )
{
   ...
   /* The 6818 currently only supports DLP in the [-12dB,0dB] range */
...
static XDRV_SINT16 apm6818GetElp( XDRV_APM *pDrv )
{
   ...
   /* The 6818 currently only supports ELP in the [-12dB,0dB] range */
...
static int apm6818SetTxRxGains( XDRV_APM *pDrv, int chan, int txGain, int rxGain, int *apmTxGain, int *apmRxGain )
{
   ...
   if ( pDev )
   {
      /* Grab the values from flexicalc */
      *apmRxGain = apmAbs(pDev->Cfg.pFlexiCalc->dBdlp);
      *apmTxGain = apmAbs(pDev->Cfg.pFlexiCalc->dBdlp);
   }
...
*****************************************************************************
*/
static void apm6818SetRingParms( XDRV_APM *pDrv, int ringFrequency, int ringWaveshape,
                                 int ringVoltage, int ringOffset, int ringOffsetCal )
{
   ...
   const APM6818_RING_GAIN_INFO *ringGainTablep;
   XDRV_SINT64 ring_gain, crest_factor, peak_ac, peak_ring;
   XDRV_SINT64 ring_amph, ring_offh, ring_offhcal;
   ...
   XDRV_SINT64 vreg_ref_gain, vmax_hvg_slic;
#endif
   XDRV_SINT64 ring_stop = 48000;
   XDRV_SINT64 ring_break_res = 256;
   XDRV_SINT64 ring_stoph, ring_break_asin, ring_break_phase, ring_amp;
   XDRV_UINT32 ring_break_sign;
   ...
   switch ( ringWaveshape )
   {
      ...
      {
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6818 reprogramming ring registers to trapezoidal waveform."));
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "Frequency %d, Voltage %d, Offset %d, Type %d",
                         ringFrequency, ringVoltage, ringOffset, ringWaveshape ));
      ...
      {
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6818 reprogramming ring registers to sinusoidal waveform."));
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "Frequency %d, Voltage %d, Offset %d, Type %d",
                         ringFrequency, ringVoltage, ringOffset, ringWaveshape ));
   ...
   switch( ringGainTablep->slicType )
   {
      case 0x3239:
      {
         ...
         vres = 75;                           /* Increased by factor of 1000 */
         ringFS = 1250;                       /* Increased by factor of 1000 */
         voh_min = 6000;                          /* Increased by factor of 1000 */
         //vbat_min = apmDiv(peak_ring , 2) + voh_min * 2;  /* Increased by factor of 1000 */
#if XDRV_CFG_BALANCED_RINGING
         vreg_ref_gain = 47;
         vbat_min1 = apmDiv(peak_ring, 2) + apmDiv(voh_min * 4,  3);           /* Increased by factor of 1000 */
         vbat_min  = vbat_min1 + apmDiv(apmAbs( ringOffsetCal ), 2);   /* Increased to account for ringOffsetCal */
         vmax_hvg_slic = apmDiv( (vbat_min1 - voh_min),  ( vreg_ref_gain ) );
         max_hvg_slic = divRound(( vmax_hvg_slic * 100 * 4096 + 58000 * 4096 ), ( 330000 ));
#else
         vbat_min1 = 8000;
         vbat_min = 8000;
         max_hvg_slic = 0;
      ...
      case 0x32392:
      {
         ...
         voh_min = 6000;                          /* Increased by factor of 1000 */
#if XDRV_CFG_BALANCED_RINGING
         vreg_ref_gain = 95;                      /* Increased by factor of 2 */
         vbat_min1 = apmDiv(peak_ring , 2 )+ apmDiv(voh_min * 4 , 3 );           /* Increased by factor of 1000 */
         vbat_min = vbat_min1 + apmDiv(apmAbs( ringOffsetCal ) , 2);   /* Increased to account for ringOffsetCal */
         vmax_hvg_slic = apmDiv(( vbat_min1 - voh_min ) * 2 , ( vreg_ref_gain ));
         max_hvg_slic = divRound(( vmax_hvg_slic * 100 * 4096 ), ( 330000 )); /* + 58000 * 4096 deleted because buffer removed*/
#else
         vbat_min1 = 12000;
         vbat_min = 12000;
         max_hvg_slic = 0;
      ...
      case 0x9541:
      {
         ...
         fsamp = 390625;                      /* Increased by factor of 4 */
         vres = 75;                           /* Increased by factor of 1000 */
         ringFS = -1037;                      /* Increased by factor of 1000 */
   ...
   ring_freqh = divRound(( 1048576 * ringFrequency ), ( fsamp ));
   ring_amph = divRound(( 128 * RING_SCALE_FACTOR * peak_ac ), ( ring_gain * apmAbs( ringFS )));
   ring_offh = divRound(( 128 * RING_SCALE_FACTOR * ringOffset ), ( ring_gain * ringFS ));
   ring_offhcal = divRound(( 128 * RING_SCALE_FACTOR * ringOffsetCal ), ( ring_gain * ringFS ));
   ring_offh = ring_offh + ring_offhcal;

   /* Determine HVG Gain */
   hvg_ring_gain = divRound((( peak_ring + voh_min - vbat_min1 ) * 256 * RING_SCALE_FACTOR ), ( vres * ( ring_amph + apmAbs( ring_offh ))));
   hvg_ring_ovhd = divRound( vbat_min, vres );
   ...
   ring_stoph = divRound(( 32768 * ( ring_stop - ringOffset )),
                         ( ring_gain * ringFS ));
   ring_break_asin  = apmAsin(apmDiv(ring_stoph*1000,ring_amp));
   ring_break_phase = divRound( ( 2 * ring_break_res * ring_break_asin ), 3142 * RING_PHASE_FACTOR );

   /* Determine sign based on phase */
   if( ring_break_phase < 0 )
   {
      ring_break_sign = 1;
   }
   else
   ...
   ring_break_phase = apmAbs( ring_break_phase );

   /* Update appropriate registers */
   pApm->reg_ring_config_1 = ((( ring_offh << RING_OFFSET_SHIFT ) & RING_OFFSET ) |
                              (( ring_amph << RING_SCALE_SHIFT ) & RING_SCALE ));

   pApm->reg_ring_config_2 = ((( ring_freqh << RING_OMEGA_NORM_A_SHIFT ) & RING_OMEGA_NORM_A ) |
                              (( ring_freqh << RING_OMEGA_NORM_B_SHIFT ) & RING_OMEGA_NORM_B ) |
                              (( ring_crest << RING_CREST_FACTOR_SHIFT ) & RING_CREST_FACTOR ) |
                              (( 1 << RING_GEN_ENABLE_A_SHIFT ) & RING_GEN_ENABLE_A ) |
                              (( 1 << RING_GEN_ENABLE_B_SHIFT ) & RING_GEN_ENABLE_B ));

   pApm->reg_ring_config_3 = ((( ring_break_phase << RING_BREAK_PHASE_A_SHIFT ) & RING_BREAK_PHASE_A ) |
                              (( ring_break_phase << RING_BREAK_PHASE_B_SHIFT ) & RING_BREAK_PHASE_B ) |
                              (( ring_break_sign << RING_BREAK_PHASE_SIGN_A_SHIFT ) & RING_BREAK_PHASE_SIGN_A ) |
                              (( ring_break_sign << RING_BREAK_PHASE_SIGN_B_SHIFT ) & RING_BREAK_PHASE_SIGN_B ) |
                              (( 0 << RING_START_CADENCE_A_SHIFT ) & RING_START_CADENCE_A ) |
                              (( 0 << RING_START_CADENCE_B_SHIFT ) & RING_START_CADENCE_B ));

   pHvg->reg_hvg_ring = ((( hvg_ring_gain << RING_GAIN_SHIFT ) & RING_GAIN ) |
...
*****************************************************************************
*/
static void apm6818GetRingParms( XDRV_APM *pDrv, int *ringFrequency, int *ringWaveshape,
                                 int *ringVoltage, int *ringOffset, int *ringOffsetCal )
{
   APM_6818_DRV *pDev  = (APM_6818_DRV *)pDrv;
...
*****************************************************************************
*/
void apm6818SetClkSource( XDRV_APM *pDrv, int clkSource )
{
}
...
**
** PARAMETERS: pApm   - pointer to 6818's APM module
...
static int WriteApmFilterCoeffs( volatile BCM6818_APM *pApm, int chan, APM6818_FILTER_TYPE filter, int *data, int length )
{
   ...
   if (( chan < 0 || chan > APM6818_NUM_CHANNELS ) && ( chan != APM6818_ALL_CHANNELS ))
   {
   ...
   switch (filter)
   {
      case APM6818_FILTER_TX_EQ:
      {
         if( chan == APM6818_ALL_CHANNELS )
         {
      ...
      case APM6818_FILTER_HISPD_HYBRID_BAL:
      {
         if( chan == APM6818_ALL_CHANNELS )
         {
      ...
      case APM6818_FILTER_LOSPD_HYBRID_BAL:
      {
         if( chan == APM6818_ALL_CHANNELS )
         {
...
static XDRV_SINT64 divRound( XDRV_SINT64 dividend, XDRV_SINT64 divisor )
{
   ...
   if( divisor == 0 )
   {
      XDRV_LOG_ERROR(( XDRV_LOG_MOD_XDRV, "ERROR: APM6818 - Divisor of zero entered!"));
      return( 1 );
   }
...
static XDRV_SINT64 apmAsin(XDRV_SINT32 value)
{
   ...
   if( value<apmAsinTable[0].value || value> apmAsinTable[sizeof(apmAsinTable)/sizeof(APM6818_ASIN)-1].value)
   {
     return 0;
   }
