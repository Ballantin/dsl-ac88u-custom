...
****************************************************************************
*
*    Description:  PCM device driver for BCM6328
*
****************************************************************************/
...
BCM6328_IUDMA_DESC *dmaRxDesc_c[PCM6328_NUM_DMA_CHANNELS*DMA_RX_NUM_BUFFERS];
BCM6328_IUDMA_DESC *dmaTxDesc_c[PCM6328_NUM_DMA_CHANNELS*DMA_TX_NUM_BUFFERS];

PCM6328_DMA_BUFFER dmaRxData[PCM6328_NUM_DMA_CHANNELS*DMA_RX_NUM_BUFFERS];
PCM6328_DMA_BUFFER dmaTxData[PCM6328_NUM_DMA_CHANNELS*DMA_TX_NUM_BUFFERS];

static XDRV_UINT8 PCM_SPECIAL_MODE;

static int  pcm6328_PLLInit( void );
static int  pcm6328_PLLDeInit( void );
static int  pcm6328_iudmaInit(void);
static int  pcm6328_iudmaDeinit(void);
static int  pcm6328_dmaDescrInit(void);
static int  pcm6328_dmaDescrDeinit(void);
static int  pcm6328_regInit( PCM_CHANNEL_TIMESLOT_MAP* chanTsMap );
static int  pcm6328_regDeinit( void );
static void pcm6328_timeslotAlloc( int chnum, int ts);
...
typedef struct PCM6328_DMA_SIMPLEX_CHANNEL_INFO
{
   int                  iuChanNum;     /* IUDMA channel number assosiated with this APM channel */
   BCM6328_IUDMA_DESC   *dmaDescp;     /* Where are the descriptors for this channel? */
   DMA_HANDLE           dmaDescHandle;
   PCM6328_DMA_BUFFER   *dmaDatap;     /* Where are the DMA buffers to store/fetch the data to/from for each channel */
   DMA_HANDLE           dmaBufHandle;
}  PCM6328_DMA_SIMPLEX_CHANNEL_INFO;

/* DMA Channel info (duplex - both ways) for easy access to DMA channels */
typedef struct PCM6328_DMA_DUPLEX_CHANNEL_INFO
{
   PCM6328_DMA_SIMPLEX_CHANNEL_INFO rxChan;  /* RX channel */
   PCM6328_DMA_SIMPLEX_CHANNEL_INFO txChan;  /* TX channel */
} PCM6328_DMA_DUPLEX_CHANNEL_INFO;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)
extern struct dma_pool *pcm_dma_pool;
#endif
/* Initialized DMA channel info */
static PCM6328_DMA_DUPLEX_CHANNEL_INFO gPcmDmaInfo[PCM6328_NUM_DMA_CHANNELS] =
{
   {
      /* RX Channel 0 */
      {
...
/*****************************************************************************
*
*  FUNCTION: pcm6328_init
*
*  PURPOSE:
*      Intialize 6328 PCM Module
*
...
int pcm6328_init( PCM_CHANNEL_TIMESLOT_MAP* chanTsMap, XDRV_UINT32 channelMask, XDRV_UINT8 supportMode )
{
   ...
   pcm6328_regInit( chanTsMap );

   pcm6328_dmaDescrInit();
...
/*****************************************************************************
*
*  FUNCTION: pcm6328_deinit
*
*  PURPOSE:
*      Deintialize 6328 PCM Module
*
*  PARAMETERS:
*
*  RETURNS: 0 on SUCCESS
...
*
*****************************************************************************/
int pcm6328_deinit( void )
{
   /* this block of code was commented out for some reason,
    * and caused compiler warning. Uncommented. */
   PCM->pcm_chan_ctrl = 0;
...
static int pcm6328_PLLInit( void )
{
 ...
 retry_lock:
   ...
   else  /* Settings without ISI */
   {
      PCM->pcm_pll_ctrl1   = 0x1C80147D;
...
/*****************************************************************************
*
*  FUNCTION: pcm6328_regInit
*
*  PURPOSE:
*      Intialize 6328 PCM Module Registers
*
*  PARAMETERS:
*
*  RETURNS: 0 on SUCCESS
...
#  endif
   ...
   for ( i = 0; i < PCM_MAX_CHANNELS; i++ )
   {
      ...
      pcm6328_timeslotAlloc( i, chanTsMap[i].txTimeslot );
   }
...
{
   XDRV_UINT8                       i,chnum;
   volatile BCM6328_IUDMA_DESC      *dmaTxDesc;
   volatile BCM6328_IUDMA_DESC      *dmaRxDesc;
   volatile BCM6328_IUDMA_DESC      *dmaTxDesc_c;
   volatile BCM6328_IUDMA_DESC      *dmaRxDesc_c;
   volatile PCM6328_DMA_BUFFER      *dmaRxData;
   volatile PCM6328_DMA_BUFFER      *dmaTxData;
   PCM6328_DMA_SIMPLEX_CHANNEL_INFO *dmaChanInfop;
   ...
   rxDescriptorArea = sizeof(BCM6328_IUDMA_DESC) * PCM6328_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6328_DMA_UBUS_BYTES;
   txDescriptorArea = sizeof(BCM6328_IUDMA_DESC) * PCM6328_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6328_DMA_UBUS_BYTES;
   ...
   for (i=0; i < PCM6328_NUM_DMA_CHANNELS; i++)
   {
      /* Align DMA descriptior to 64Bit Ubus */
      dmaRxDesc_c = (volatile BCM6328_IUDMA_DESC *) ALIGN_DATA( (unsigned int) dmaRxDesc_c, BCM6328_DMA_UBUS_BYTES );
      dmaTxDesc_c = (volatile BCM6328_IUDMA_DESC *) ALIGN_DATA( (unsigned int) dmaTxDesc_c, BCM6328_DMA_UBUS_BYTES );

      gPcmDmaInfo[i].rxChan.dmaDescp = (BCM6328_IUDMA_DESC*)(dmaRxDesc_c + i * DMA_RX_NUM_BUFFERS);
      gPcmDmaInfo[i].txChan.dmaDescp = (BCM6328_IUDMA_DESC*)(dmaTxDesc_c + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   rxBufferArea = sizeof(XDRV_UINT32) * (PCM6328_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * PCM6328_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS;
   txBufferArea = sizeof(XDRV_UINT32) * (PCM6328_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * PCM6328_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS;
   ...
   for (i=0; i < PCM6328_NUM_DMA_CHANNELS; i++)
     {
      gPcmDmaInfo[i].rxChan.dmaDatap = (PCM6328_DMA_BUFFER*)(dmaRxData + i * DMA_RX_NUM_BUFFERS);
      gPcmDmaInfo[i].txChan.dmaDatap = (PCM6328_DMA_BUFFER*)(dmaTxData + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   for (chnum=0; chnum<PCM6328_NUM_DMA_CHANNELS; chnum++)
   {
      ...
      dmaTxDesc = (volatile BCM6328_IUDMA_DESC *)CacheToNonCacheVirtAddress((XDRV_UINT32)dmaChanInfop->dmaDescp);
      ...
      for (i=0; i<DMA_TX_NUM_BUFFERS; i++)
      {
         ...
         bufferSizeBytes = PCM6328_DMA_MAX_BUFFER_SIZE_BYTES;

         /* Each buffer is a complete packet */
         dmaTxDesc[i].flagsLengthStatus.status = BCM6328_IUDMA_DESC_STATUS_SOP |
                                                 BCM6328_IUDMA_DESC_STATUS_EOP;

         if( i == 0 )
         {
            /* First buffer - prime the TX fifo */
            dmaTxDesc[i].flagsLengthStatus.status |= BCM6328_IUDMA_DESC_STATUS_OWN;
         ...
         if( i == ( DMA_TX_NUM_BUFFERS - 1 ) )
         {
            ...
            dmaTxDesc[i].flagsLengthStatus.status |= BCM6328_IUDMA_DESC_STATUS_WRAP;
         }

         /* Check if we can accomodate the desired buffer size */
         if (bufferSizeBytes <= PCM6328_DMA_MAX_BUFFER_SIZE_BYTES)
         {
         ...
         else
         {
            ...
            ** You can either increase the compile time buffer size (PCM6328_DMA_MAX_BUFFER_SIZE),
      ...
      dmaRxDesc = (volatile BCM6328_IUDMA_DESC *)CacheToNonCacheVirtAddress((XDRV_UINT32)dmaChanInfop->dmaDescp);
      ...
      for( i = 0; i < DMA_RX_NUM_BUFFERS; i++ )
      {
         ...
         dmaRxDesc[i].flagsLengthStatus.status = BCM6328_IUDMA_DESC_STATUS_OWN;

         if( i == ( DMA_RX_NUM_BUFFERS - 1 ) )
         {
            /* Last descriptor - set the wrap bit */
            dmaRxDesc[i].flagsLengthStatus.status |= BCM6328_IUDMA_DESC_STATUS_WRAP;
         }
         ...
         ** buffers before and after DMA operations. Since cache operations
         ** operate on the entire cache-line, it is necessary to make the
         ** buffers aligned on the cache-line size (16-bytes for the 6328).
         ** Failure to do so may result in corruption of data that shares the
         ** same cache-line as the DMA buffer. */
         dmaDatap = dmaChanInfop->dmaDatap[i].dmaBuff;
         dmaDatap = (volatile XDRV_UINT32 *) ALIGN_DATA( (unsigned int) dmaDatap, DCACHE_LINE_SIZE_BYTES );
...
static int pcm6328_dmaDescrDeinit( void )
{
   ...
   rxDescriptorArea = sizeof(BCM6328_IUDMA_DESC) * PCM6328_NUM_DMA_CHANNELS * DMA_RX_NUM_BUFFERS;
   txDescriptorArea = sizeof(BCM6328_IUDMA_DESC) * PCM6328_NUM_DMA_CHANNELS * DMA_TX_NUM_BUFFERS;
...
static int pcm6328_iudmaInit( void)
{
   ...
   memset( (void*)stRamp, 0, BCM6328_MAX_PCM_DMA_CHANNELS * sizeof(PcmIudmaStateRam) );
   ...
   /* Init RX side */
      ...
      iuDmap->ctrl[gPcmDmaInfo[chnum].rxChan.iuChanNum].intMask = BCM6328_IUDMA_INTMASK_BDONE | BCM6328_IUDMA_INTMASK_NOTVLD;
      ...
      iuDmap->ctrl[gPcmDmaInfo[chnum].txChan.iuChanNum].intMask = BCM6328_IUDMA_INTMASK_BDONE | BCM6328_IUDMA_INTMASK_NOTVLD;
   }

   /* Enable the IUDMA master */
   iuDmap->regs.ctrlConfig = BCM6328_IUDMA_REGS_CTRLCONFIG_MASTER_EN;
...
static int pcm6328_iudmaDeinit( void)
{
   ...
   for (chnum=0; chnum<PCM6328_NUM_DMA_CHANNELS; chnum++)
   {
      ...
      iuDmap->ctrl[gPcmDmaInfo[chnum].rxChan.iuChanNum].config = ( BCM6328_IUDMA_CONFIG_BURSTHALT );

      while( (iuDmap->ctrl[gPcmDmaInfo[chnum].rxChan.iuChanNum].config & BCM6328_IUDMA_CONFIG_ENDMA) != 0x00000000 )
      {
         /* Set the burstHalt bit while clearing endma bit */
         iuDmap->ctrl[gPcmDmaInfo[chnum].rxChan.iuChanNum].config = ( BCM6328_IUDMA_CONFIG_BURSTHALT );
      ...
      /* Set the burstHalt bit while clearing endma bit */
      iuDmap->ctrl[gPcmDmaInfo[chnum].txChan.iuChanNum].config = ( BCM6328_IUDMA_CONFIG_BURSTHALT );

      while( (iuDmap->ctrl[gPcmDmaInfo[chnum].txChan.iuChanNum].config & BCM6328_IUDMA_CONFIG_ENDMA) != 0x00000000 )
      {
         /* Set the burstHalt bit while clearing endma bit */
         iuDmap->ctrl[gPcmDmaInfo[chnum].txChan.iuChanNum].config = ( BCM6328_IUDMA_CONFIG_BURSTHALT );
...
/*****************************************************************************
*
*  FUNCTION: pcm6328_timeslotAlloc
*
*  PURPOSE:
...
static void pcm6328_timeslotAlloc( int chnum, int ts)
{
   ...
   */
   ...
   XDRV_LOG_DBG((XDRV_LOG_MOD_PCM, "%s 0x%08X: 0x%08X", __FUNCTION__, (unsigned int)&PCM->pcm_slot_alloc_tbl[ts_register_offset], (unsigned int)entry ));
