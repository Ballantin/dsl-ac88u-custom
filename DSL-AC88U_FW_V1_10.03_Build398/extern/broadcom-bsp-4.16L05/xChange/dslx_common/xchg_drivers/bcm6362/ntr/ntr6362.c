...
****************************************************************************
*
*    Description:  NTR device driver for BCM6362
*
****************************************************************************/
...
#define  GPIO_MODE_NTR_ENABLE               (1<<11) /* Chip specific GPIO */

static XDRV_UINT8 gpio11InitFlag = 0;

static void ntr6362_init()
{
   if ( !gpio11InitFlag )
   {
      gpio11InitFlag = 1;
#ifdef NTR_SUPPORT_6362_COMP
      /* Using PCM feedback to NTR counter */
      PCM->pcm_ctrl  |= PCM_NTR_ENABLE;
      GPIO->GPIODir  &= ~(1<<11);        /* Chip specific GPIO */
      GPIO->GPIOMode |= (1<<11);
#endif      
#ifdef NTR_SUPPORT_6362
      /* Using PCM feedback to DSL PHY */
      GPIO->GPIODir  &= ~(1<<31);        /* Chip specific GPIO */
      //GPIO->GPIOMode |= (1<<31);
#endif
      bosSleep(10);
   }
...
void ntr6362_getCounts( XDRV_UINT32 * localCount, XDRV_UINT32 * ntrCount )
{
   /* Macro that accesses the mips counter (200MHz on 6362, 150MHz on 6358) */
...
void ntr6362_getNdiv( XDRV_UINT32 * ndivInt, XDRV_UINT32 * ndivFrac )
{
...
)
{
   ...
   NTR_LOGDEBUG(("\n\nBeginning of ntr6362_setFeedback.  Dispaying unmodified registers."));
...
void ntr6362_print( XDRV_BOOL numSamples, XDRV_SINT32 sampleIntervalMs)
{
