...
**
** $Id: proslic.c 4851 2015-04-01 00:34:33Z nizajerk $
**
** Distributed by: 
...
#define PROSLIC_REG_PATCH_JMPTBL_START_ADDR 82
#define PROSLIC_REG_PATCH_JMPTBL_HI_ADDR 1597
...
static BOOLEAN ProSLIC_VerifyPatchJMPLow(proslicChanType *pProslic, const uInt16 *data)
{
    ...
    for(address = PATCH_JMPTBL_START_ADDR; address < (PATCH_JMPTBL_START_ADDR+PATCH_NUM_LOW_ENTRIES); address++)
    {
        if(*data)
        {
...
static BOOLEAN ProSLIC_VerifyPatchJMPHigh(proslicChanType *pProslic, const uInt16 *data)
{
    ...
    for(address = PATCH_JMPTBL_HIGH_ADDR; address < (PATCH_JMPTBL_HIGH_ADDR+8); address++)
    {
        if(*data)
        {
...
static void ProSLIC_LoadJMPHigh(proslicChanType *pProslic, uInt8 channel, uInt16 *data)
{
    uInt16 loop;
...
{
    uInt8 channel;
    const uInt16 jmp_disable[PATCH_NUM_ENTRIES] = {0,0,0,0,0,0,0,0};
#ifdef SI3226X
    BOOLEAN hasJmpTableHigh = FALSE;
    const uInt16 jmphigh_disable[PATCH_NUM_ENTRIES] = {0UL, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL, 0UL};
#endif
	...
	if( (pProslic->deviceId->chipType >= SI32260 && pProslic->deviceId->chipType <= SI32269) )
    {
        ...
        ProSLIC_LoadJMPHigh(pProslic, channel, (uInt16 *)jmphigh_disable);
    }
#endif
    ...
    {
        ProSLIC_LoadJMPHigh(pProslic, channel, (uInt16 *)&(pPatch->patchEntries[PATCH_NUM_ENTRIES]));
    }
#endif        
#if defined(SI322X) || defined(SI3217X) || defined(SI3226X)
...
#endif /* Si3226, Si3217x, Si3226x */
	return RC_COMPLETE_NO_ERR;
}

int ProSLIC_VerifyPatch (proslicChanType_ptr hProslic,proslicPatch *pPatch){
		if(hProslic->channelType != PROSLIC) return RC_IGNORE;
#if defined(SI3217X) || defined(SI3226X) || defined(SI324X) || defined (SI322X)
    if (0 
#ifdef SI324X
    ...
    )
    {
	    ...
	    if(hProslic->deviceId->chipType >= SI32260 && hProslic->deviceId->chipType <= SI32269)
        {
            if( ProSLIC_VerifyPatchJMPHigh(hProslic, &(pPatch->patchEntries[PATCH_NUM_ENTRIES])) == FALSE)
            {
#ifdef ENABLE_DEBUG
            if (hProslic->debugMode)
...
#if defined(SI322X) || defined(SI3226X) || defined (SI324X)
...
#endif
#if defined (SI322X) || defined (SI3226X)
    if( 0
#ifdef SI322X
	    || (hProslic->deviceId->chipType >= SI3226 && hProslic->deviceId->chipType <= SI3227)
#endif
...
#endif
#if defined (SI3217X) || defined (SI3226X) || defined (SI324X)
  if(0
#ifdef SI3226X
	  || (hProslic->deviceId->chipType >= SI32260 && hProslic->deviceId->chipType <= SI32269)
#endif
...
**
*/
#define REG_LINEFEED 30
#define RAM_RDC_SUM  1499
...
int ProSLIC_isReinitRequired(proslicChanType *pProslic)
{
 ...
 if( (pProslic->channelType != PROSLIC) ||
	...
	lf = ReadReg(pProslic, pProslic->channel,REG_LINEFEED);
	rkdc_sum = ReadRAM(pProslic,pProslic->channel,RAM_RDC_SUM);
#ifdef ENABLE_DEBUG
	if(pProslic->debugMode)
    {
