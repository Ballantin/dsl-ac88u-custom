...
*
****************************************************************************
*    Filename: slicSi3215.c
*
****************************************************************************
*    Description:
*
*      This file contains functions related to the control of the
*      SI3215 Subscriber Line Interface Circuits (SLIC).
*
****************************************************************************/
...
#include "slicSi3215.h"
#include "slicSi3215Locale.h"
#include <proslic.h>
...
/* SI3215 SLIC states */
#define SI3215_STATE_DISCONNECT        LF_OPEN        /* Disconnect */
#define SI3215_STATE_FWD_ACTIVE        LF_FWD_ACTIVE  /* Forward active */
#define SI3215_STATE_FWD_ONHOOK_TX     LF_FWD_OHT     /* On hook transmission, forward */
#define SI3215_STATE_TIP_OPEN          LF_TIP_OPEN    /* Tip Open mode */
#define SI3215_STATE_RINGING           LF_RINGING     /* Ringing */
#define SI3215_STATE_REV_ACTIVE        LF_REV_ACTIVE  /* Reverse active */
#define SI3215_STATE_REV_ONHOOK_TX     LF_REV_OHT     /* On hook transmission, reverse */
#define SI3215_STATE_RING_OPEN         LF_RING_OPEN   /* Ring Open mode */
                                                            

/* SLIC initial/shutdown states */
#define SI3215_STATE_INIT              SI3215_STATE_DISCONNECT
#define SI3215_STATE_DEINIT            SI3215_STATE_DISCONNECT

/* private functions */
static int mapVrgLocaleToSi321x ( VRG_COUNTRY vrgLocale );

/* SLIC "meta-driver" interface functions */
static void slicSi3215ModeControl( XDRV_SLIC *pDrv, XDRV_SLIC_MODE mode );
static void slicSi3215LedControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static void slicSi3215PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slicSi3215IsOffhook( XDRV_SLIC *pDrv );
static int slicSi3215SetRingParms( XDRV_SLIC *pDrv, int ringFrequency, int ringWaveshape,
                                      int ringVoltage, int ringOffset, int ringOffsetCal );
static int slicSi3215GetRingParms( XDRV_SLIC *pDrv, int *ringFrequency, int *ringWaveshape,
                                      int *ringVoltage, int *ringOffset, int *ringOffsetCal );
static int slicSi3215SetBoostedLoopCurrent( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static int slicSi3215SetPowerSource( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slicSi3215GetOverCurrentStatus( XDRV_SLIC *pDrv );
static XDRV_SINT16 slicSi3215GetDlp( XDRV_SLIC *pDrv );
...
static const XDRV_SLIC_FUNCS  slicSi3215DrvFuncs =
{
   slicSi3215ModeControl,
   slicSi3215LedControl,
   slicSi3215PhaseReversalControl,
   slicSi3215IsOffhook,
   slicSi3215SetRingParms,
   slicSi3215GetRingParms,
   slicSi3215SetBoostedLoopCurrent,
   slicSi3215SetPowerSource,
   slicSi3215GetOverCurrentStatus,
   slicSi3215GetDlp,
   slicProcessEvents,
   slicProbe
};
...
*****************************************************************************
*/
void slicSi3215Init( int chan, SLIC_SI3215_DRV *pDev )
{
   int si321xLocale;
   ...
   pDev->chan = chan;
   pDev->pDrvFuncs = &slicSi3215DrvFuncs;

   /* Driver enabled */
   pDev->bDrvEnabled = XDRV_TRUE;
    
   /* Assign mspi device id to spireset object per channel */
   ...
   ProSLIC_SetLinefeedStatus (pProSlicChanPtr[pDev->chan], SI3215_STATE_INIT );
...
void slicSi3215Deinit( SLIC_SI3215_DRV *pDev )
{
   ...
   ProSLIC_SetLinefeedStatus (pProSlicChanPtr[pDev->chan], SI3215_STATE_DEINIT );
...
static XDRV_BOOL slicSi3215IsOffhook( XDRV_SLIC *pDrv )
{
   SLIC_SI3215_DRV *pDev = (SLIC_SI3215_DRV *)pDrv;
...
/*
*****************************************************************************
** FUNCTION:   slicSi3215ModeControl
**
...
static void slicSi3215ModeControl( XDRV_SLIC *pDrv, XDRV_SLIC_MODE mode )
{
   ...
   switch( mode )
   {
      ...
      {
         ProSLIC_SetLinefeedStatus (pProSlicChanPtr[pDev->chan], SI3215_STATE_DISCONNECT );
      }
      break;
      ...
      {
         ProSLIC_SetLinefeedStatus (pProSlicChanPtr[pDev->chan], SI3215_STATE_FWD_ACTIVE );
      }
      break;
      ...
      {
         ProSLIC_SetLinefeedStatus (pProSlicChanPtr[pDev->chan], SI3215_STATE_FWD_ONHOOK_TX );
      }
      break;
      ...
      {
         ProSLIC_SetLinefeedStatus (pProSlicChanPtr[pDev->chan], SI3215_STATE_REV_ONHOOK_TX );
      }
      break;
      ...
      {
         ProSLIC_SetLinefeedStatus (pProSlicChanPtr[pDev->chan], SI3215_STATE_REV_ACTIVE );
      }
      break;
...
static XDRV_SINT16 slicSi3215GetDlp( XDRV_SLIC *pDrv )
{
   SLIC_SI3215_DRV *pDev    = NULL;
   VRG_COUNTRY      country = VRG_COUNTRY_MAX;

   pDev = (SLIC_SI3215_DRV *)pDrv;
