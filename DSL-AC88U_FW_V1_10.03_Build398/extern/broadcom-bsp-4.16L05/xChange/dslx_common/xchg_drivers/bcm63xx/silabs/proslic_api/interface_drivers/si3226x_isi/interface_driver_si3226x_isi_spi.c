...
#include <xdrvLog.h>
#include <bosSleep.h>
#include "si3226x_isi_reset.h"
#include "bcmSpiRes.h"
#include "interface_driver_si_common.h"


/* Spi Control Interface */
#define SI3226x_ISI_SPI_BRDCST_CMD        0x80
#define SI3226x_ISI_SPI_READ_CMD          0x40
#define SI3226x_ISI_SPI_WRITE_CMD         0x00
#define SI3226x_ISI_SPI_REG_CMD           0x20
#define SI3226x_ISI_SPI_RAM_CMD           0x00
#define SI3226x_ISI_SPI_CID_MSK           0x1F

/* Indirect register access */
#define SI3226x_ISI_REG_UAM               126
#define SI3226x_ISI_UAM_BYTE_0            0x02
#define SI3226x_ISI_UAM_BYTE_1            0x08
#define SI3226x_ISI_UAM_BYTE_2            0x0E
#define SI3226x_ISI_UAM_BYTE_3            0x00
#define SI3226x_ISI_REG_RAMSTAT              4
#define SI3226x_ISI_REG_RAM_ADDR_HI          5
#define SI3226x_ISI_RAM_ADDR_HI_SHFT         8
#define SI3226x_ISI_RAM_ADDR_HI_MSK     0x0700
#define SI3226x_ISI_RAM_ADDR_10_MSK       0x80
#define SI3226x_ISI_REG_RAM_ADDR_LO         10
#define SI3226x_ISI_RAM_ADDR_LO_MSK     0x00FF
#define SI3226x_ISI_REG_RAM_DATA_B0          6
#define SI3226x_ISI_RAM_DATA_B0_RD_MSK    0xF8
#define SI3226x_ISI_REG_RAM_DATA_B1          7
#define SI3226x_ISI_REG_RAM_DATA_B2          8
#define SI3226x_ISI_REG_RAM_DATA_B3          9
#define SI3226x_ISI_RAM_DATA_B0_WR_MSK       0x0000001F
#define SI3226x_ISI_RAM_DATA_B1_WR_MSK       0x00001FE0
#define SI3226x_ISI_RAM_DATA_B2_WR_MSK       0x001FE000
#define SI3226x_ISI_RAM_DATA_B3_WR_MSK       0x1FE00000
...
*****************************************************************************
*/
int SPI_3226x_ISI_Init (ctrl_S * hSpi)
{
	return 0;
}
...
/*
******************************************************************************
** FUNCTION:  ctrl_3226x_ISI_ResetWrapper
**
** PURPOSE:   Reset ProSLIC device
**
** PARAMETERS: hSpi   - pointer to spi reset object
**             status - 1 to go into reset, 0 to go out of reset
**
** RETURNS: None
**
*****************************************************************************
*/
int ctrl_3226x_ISI_ResetWrapper (void * hSpi, int status)
{
	if (status)
	{
		Si3226x_ISI_Reset(1);
...
/*
******************************************************************************
** FUNCTION:  ctrl_3226x_ISI_ReadRegisterWrapper
**
...
*****************************************************************************
*/
XDRV_UINT8 ctrl_3226x_ISI_ReadRegisterWrapper (void * hSpi, XDRV_UINT8 channel,XDRV_UINT8 regAddr)
{
...
/*
******************************************************************************
** FUNCTION:  ctrl_3226x_ISI_WriteRegisterWrapper
**
...
*****************************************************************************
*/
int ctrl_3226x_ISI_WriteRegisterWrapper (void * hSpi, XDRV_UINT8 channel,  XDRV_UINT8 regAddr, XDRV_UINT8 data)
{
   bosMutexAcquire( &spiSiMutex );   
...
/*
******************************************************************************
** FUNCTION:  ctrl_3226x_ISI_ReadRAMWrapper
**
...
*****************************************************************************
*/
XDRV_UINT32 ctrl_3226x_ISI_ReadRAMWrapper (void * hSpi, XDRV_UINT8 channel, XDRV_UINT16 ramAddr)
{
...
/*
******************************************************************************
** FUNCTION:  ctrl_3226x_ISI_WriteRAMWrapper
**
...
*****************************************************************************
*/
int ctrl_3226x_ISI_WriteRAMWrapper (void * hSpi, XDRV_UINT8 channel, XDRV_UINT16 ramAddr, XDRV_UINT32 data)
{
   bosMutexAcquire( &spiSiMutex ); 
...
/* Debug */
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC,"Proslic: MSPI_Read: mspiDevId %d, channel %d, addr 0x%08x ", mspiDevId, channel, addr));  

#if 0
   temp_cid = channel & SI3226x_ISI_SPI_CID_MSK;
   /* Bits in SI3226x_ISI SPI Control byte are reversed for CID field.  ie: CID[4:0] must be bit reversed */
   ...
   SPICmdControlByte = SI3226x_ISI_SPI_READ_CMD | SI3226x_ISI_SPI_REG_CMD | chan_cid;
   ...
   for ( i = 0; i < 3; i++ )
   {
       if( BcmSpiSyncTrans(&txBuffer[i], &rxBuffer[i], 0, 1, HS_SPI_BUS_NUM, mspiDevId) )
       {
          XDRV_LOG_ERROR((XDRV_LOG_MOD_SLIC,"PROSlic api interface MSPI_Read Error"));
       }
   }  

//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "Proslic: MSPI_Read: rxBuffer: 0x%02x ", rxBuffer[2]&0xFF ));  
...
/* Debug */
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "Proslic: MSPI_Write: mspiDevId %d, channel %d, addr 0x%08x, data 0x%08x ", mspiDevId, channel, addr, data)); 
   ...
   if( channel == 0xFF )
   {
      SPICmdControlByte = SI3226x_ISI_SPI_BRDCST_CMD | SI3226x_ISI_SPI_WRITE_CMD | SI3226x_ISI_SPI_REG_CMD | chan_cid;
   }
   else
   {
      SPICmdControlByte = SI3226x_ISI_SPI_WRITE_CMD | SI3226x_ISI_SPI_REG_CMD | chan_cid;
   }
   ...
   for ( i = 0; i < 3; i++ )
   {
       if( BcmSpiSyncTrans(&txBuffer[i], &dummy, 0, 1, HS_SPI_BUS_NUM, mspiDevId) )
       {
          XDRV_LOG_ERROR((XDRV_LOG_MOD_SLIC, "PROSlic api interface MSPI_Read Error"));
       }
   }  
...
//channel, addr));   
   ...
   if( ram_addr_hi_10_required )
   {
      ...
      uam_status = (MSPI_Read( mspiDevId, channel, SI3226x_ISI_REG_UAM) & 0x1);
   
      /* If not in UAM mode, then set to UAM to be able to use RAM_ADDR_HI[10] bit. */
      if( uam_status != 0x1 )
      {
         MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_UAM, SI3226x_ISI_UAM_BYTE_0 );
         MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_UAM, SI3226x_ISI_UAM_BYTE_1 );
         MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_UAM, SI3226x_ISI_UAM_BYTE_2 );
         MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_UAM, SI3226x_ISI_UAM_BYTE_3 );
   ...
   MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_RAM_ADDR_HI, hi_addr );
   
   /* Program RAM_ADDR_LO register */
   MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_RAM_ADDR_LO, lo_addr );
   
   /* Wait for RAMSTAT bit */
   while( MSPI_Read( mspiDevId, channel, SI3226x_ISI_REG_RAMSTAT ) & 0x1 );
  
   if( ram_addr_hi_10_required )
   {
      /* Clear RAM_ADDR_HI [10] bit */
      MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_RAM_ADDR_HI, ~SI3226x_ISI_RAM_ADDR_10_MSK&hi_addr );
 #if 0 /* Already done in ProSlic driver */ 
   ...
   /* Read RAM_DATA_B0 */
   b0_data = MSPI_Read( mspiDevId, channel, SI3226x_ISI_REG_RAM_DATA_B0 );
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "Proslic: MSPI_Indirect_Read: b0_data 0x%08x", b0_data)); 
   data = (b0_data & SI3226x_ISI_RAM_DATA_B0_RD_MSK) >> 3;

   /* Read RAM_DATA_B1 */ 
   b1_data = MSPI_Read( mspiDevId, channel, SI3226x_ISI_REG_RAM_DATA_B1 );
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "Proslic: MSPI_Indirect_Read: b1_data 0x%08x", b1_data));
   data |= b1_data << 5;    

   /* Read RAM_DATA_B2 */ 
   b2_data = MSPI_Read( mspiDevId, channel, SI3226x_ISI_REG_RAM_DATA_B2 );
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "Proslic: MSPI_Indirect_Read: b2_data 0x%08x", b2_data));   
   data |= b2_data << 13;

   /* Read RAM_DATA_B3 */
   b3_data = MSPI_Read( mspiDevId, channel, SI3226x_ISI_REG_RAM_DATA_B3 );
...
//channel, addr, data));
   ...
   /* Write RAM_DATA_B0 */ 
   b0_data = ((data & SI3226x_ISI_RAM_DATA_B0_WR_MSK) << 3);
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC,"Proslic: MSPI_Indirect_Write: b0_data 0x%08x", b0_data));   
   MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_RAM_DATA_B0, b0_data );
   /* Write RAM_DATA_B1 */
   b1_data = ((data & SI3226x_ISI_RAM_DATA_B1_WR_MSK) >> 5); 
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "Proslic: MSPI_Indirect_Write: b1_data 0x%08x", b1_data));   
   MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_RAM_DATA_B1, b1_data );
   /* Write RAM_DATA_B2 */ 
   b2_data = ((data & SI3226x_ISI_RAM_DATA_B2_WR_MSK) >> 13); 
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "Proslic: MSPI_Indirect_Write: b2_data 0x%08x", b2_data));   
   MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_RAM_DATA_B2, b2_data );
   /* Write RAM_DATA_B3 */ 
   b3_data = ((data & SI3226x_ISI_RAM_DATA_B3_WR_MSK) >> 21);
//XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "Proslic: MSPI_Indirect_Write: b3_data 0x%08x", b3_data));    
   MSPI_Write( mspiDevId, channel, SI3226x_ISI_REG_RAM_DATA_B3, b3_data ); 
...
void MSPI_dump_all_3226x_ISI(void)
{
   int i;
   XDRV_UINT8 data;
   XDRV_UINT32 ramdata;
