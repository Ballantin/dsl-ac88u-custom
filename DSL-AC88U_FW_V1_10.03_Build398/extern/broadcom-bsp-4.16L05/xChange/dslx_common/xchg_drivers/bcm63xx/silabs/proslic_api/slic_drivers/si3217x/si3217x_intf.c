...
**
** $Id: si3217x_intf.c 4835 2015-03-31 16:28:13Z nizajerk $
**
** SI3217X_Intf.c
    ...
    */
    for (k=0;k<size;k++)
    {
      if ((pProslic[k]->channelEnable)&&(pProslic[k]->channelType == PROSLIC))
      {
        ...
        if(data == 1)
        {
          data = (uInt8)Si3217x_VerifyPatch(pProslic[k],(proslicPatch *)patch);
          if (data)
          {
            pProslic[k]->channelEnable=0;
...
int i;
    ...
    switch(pPSTNCheck->pState.stage) 
    {
        ...
        case 3: 
            ...
            if (pProslic->debugMode)
            {
                LOGPRINT("%sDiff PSTN : Vopen = %d mV\n", LOGPRINT_PREFIX, (int)pPSTNCheck->vdiff_open);
            }
#endif
        ...
        case 12:
            ...
            if (pProslic->debugMode)
            {
                LOGPRINT("%sDiff PSTN: Vdiff1[%d] = %d mV\n", LOGPRINT_PREFIX,
                    (int)pPSTNCheck->pState.sampleIterations,(int)pPSTNCheck->vdiff1[pPSTNCheck->pState.sampleIterations]);
                LOGPRINT("%sDiff PSTN: Iloop1[%d] = %d uA\n", LOGPRINT_PREFIX, 
                    (int)pPSTNCheck->pState.sampleIterations,(int)pPSTNCheck->iloop1[pPSTNCheck->pState.sampleIterations]);
            }
#endif
        ...
        case 14:
            ...
            if (pProslic->debugMode)
            {
                LOGPRINT("%sDiff PSTN: Vdiff2[%d] = %d mV\n", LOGPRINT_PREFIX,
                    (int)pPSTNCheck->pState.sampleIterations,(int)pPSTNCheck->vdiff2[pPSTNCheck->pState.sampleIterations]);
                LOGPRINT("%sDiff PSTN: Iloop2[%d] = %d uA\n", LOGPRINT_PREFIX,
                    (int)pPSTNCheck->pState.sampleIterations,(int)pPSTNCheck->iloop2[pPSTNCheck->pState.sampleIterations]);
            }
#endif
            pPSTNCheck->pState.sampleIterations++;
            if(pPSTNCheck->pState.sampleIterations >= pPSTNCheck->samples)
