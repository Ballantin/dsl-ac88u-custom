    ...
    void *pResult)
{
    ...
    char *pOut = outbuf;
	  ...
	  strncpy(pOut, "results={",strlen("results={")); //1
    pOut += strlen("results={");
		
		
		//char *strTmp="results={";
		//char *pOut = outbuf + sprintf(outbuf, "%s",strTmp);	
    ...
    if (
        ...
        pOut += sprintf(pOut, "loadChecksum=0x%8.8X ", (unsigned)pChkSum->loadChecksum);
	      ...
	      //strncpy(pOut, "vInfo={"); //2
	      strncpy(pOut, "vInfo={",strlen("vInfo={")); //2
        pOut += strlen("vInfo={");
        ...
        /* To do */
	      //strncpy(pOut, "(test result not displayed)"); //3
	      strncpy(pOut, "(test result not displayed)",strlen("(test result not displayed)")); //3	      
        pOut += strlen("(test result not displayed)");
        ...
        switch (pEvent->eventId) {
            ...
            case VP_LINE_EVID_RD_OPTION:
                /* To do */
	              //strncpy(pOut, "(option value not displayed)"); //4
	              strncpy(pOut, "(option value not displayed)",strlen("(option value not displayed)")); //4	              
                pOut += strlen("(option value not displayed)");
                break;
            ...
            case VP_DEV_EVID_IO_ACCESS_CMP:
                /* We can't distinguish between VpDeviceIoAccessDataType and
	              ...
	              //strncpy(pOut, "(cannot display result)"); //5
	              strncpy(pOut, "(cannot display result)",strlen("(cannot display result)")); //5	              
                pOut += strlen("(cannot display result)");
                break;
            case VP_LINE_EVID_LINE_IO_RD_CMP: {
                ...
                pOut += sprintf(pOut, "direction=%d ", pLineIoAccess->direction);
	              //strncpy(pOut, "ioBits={"); //6
	              strncpy(pOut, "ioBits={",strlen("ioBits={")); //6	              
                pOut += strlen("ioBits={");
            ...
            case VP_LINE_EVID_QUERY_CMP: {
                ...
                switch (pEvent->eventData) {
                    ...
                    case VP_QUERY_ID_SEAL_CUR:
						            //strncpy(pOut, "sealCur={"); //7
						            strncpy(pOut, "sealCur={",strlen("sealCur={")); //7						            
                        pOut += strlen("sealCur={");
                        ...
                        pOut += sprintf(pOut, "minCurrent=%u ", pQueryResults->sealCur.minCurrent);
						            //strncpy(pOut, "sealMaskArray[]={"); //8
						            strncpy(pOut, "sealMaskArray[]={",strlen("sealMaskArray[]={")); //8						            
                        pOut += strlen("sealMaskArray[]={");
                        for (i = 0; i < VP_SEAL_CURRENT_ARRAY_SIZE; i++) {
                            pOut += sprintf(pOut, "0x%4.4X ", pQueryResults->sealCur.sealMaskArray[i]);
                        }
						            ////strncpy(pOut, "} }"); //9
						            strncpy(pOut, "} }",strlen("} }")); //9						            
                        pOut += strlen("} }");
                        break;
