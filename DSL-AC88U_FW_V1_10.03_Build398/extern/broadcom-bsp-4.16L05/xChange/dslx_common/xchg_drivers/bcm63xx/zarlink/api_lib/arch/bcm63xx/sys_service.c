...
static BOS_SPINLOCK bosSpinCriticalLock;
 ...
 */
    ...
    bosSpinlockAcquire(&bosSpinCriticalLock, &flags);
    ...
    if( (gSysCriticalSec[criticalSecType].pid == -1) ||
        ...
        if(gSysCriticalSec[criticalSecType].pid == -1) {
            ...
            bosSpinlockRelease( &bosSpinCriticalLock, flags );
    ...
    VpDeviceIdType deviceId)
{
    ...
    if (deviceId & 0x8000) {
       ...
       /*changed, BRCM does not use them */
        uint32 interruptReg = 0;
        uint32 interruptMaskReg = 0;
