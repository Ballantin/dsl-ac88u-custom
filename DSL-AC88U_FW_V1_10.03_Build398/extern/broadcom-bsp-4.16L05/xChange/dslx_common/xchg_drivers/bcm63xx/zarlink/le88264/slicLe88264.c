...
****************************************************************************
*    Description: 
*      This is the Legerity Le88264 - Single Channel Digital SLIC Device
...
/*
*****************************************************************************
** FUNCTION:   slicLe88264Init
**
** PURPOSE:    Initialize and open the Zarlink 88264 device (FXSx2)
**
...
*****************************************************************************
*/
XDRV_SLIC_STATUS slicLe88264Init( SLIC_DEVICE_DRV *pDev, XDRV_UINT16 resetGpioPin )
{
   XDRV_SLIC_STATUS retStatus = XDRV_SLIC_STATUS_SUCESS;
   
   XDRV_LOG_INFO((XDRV_LOG_MOD_SLIC, "%s: Starting %s device and line initializion...\n", LOG_MODULE_NAME, LOG_DEV_NAME ));
   ...
   {
      XDRV_LOG_DBG((XDRV_LOG_MOD_SLIC, "%s: Skipping SLIC chip reset\n", LOG_MODULE_NAME ));
   }   

   if( pDev->deviceProfile == ZARLINK_FLYBACK_THALASSA )
   {
      zarlinkConfig.pDevProfile = DEV_VE880_88264_PROFILE_FLYBACK_THALASSA;   
   }
   else if( pDev->deviceProfile == ZARLINK_FLYBACK )
   {
      zarlinkConfig.pDevProfile = DEV_VE880_88264_PROFILE_FLYBACK_ZSI;   
   }
   
   /* Setup Configuration Structure */
   zarlinkConfig.pDev              = pDev;
   zarlinkConfig.numFxsPerDevice   = LE88264_FXS_LINES;
   zarlinkConfig.numFxoPerDevice   = 0;
   zarlinkConfig.deviceType        = VP_DEV_880_SERIES;
   zarlinkConfig.fxsTermType       = VP_TERM_FXS_GENERIC;
   zarlinkConfig.fxsTermTypeSecondValid = 1;                /* Used only when 2nd line termination type is different */
   zarlinkConfig.fxsTermTypeSecond = VP_TERM_FXS_SPLITTER;  /* Used only when 2nd line termination type is different */
   ...
   zarlinkConfig.pDcProfile        = DC_22MA_CC;
   ...
   {
      XDRV_LOG_ERROR((XDRV_LOG_MOD_SLIC, "%s: %s device initializion FAILED !.\n", LOG_MODULE_NAME, LOG_DEV_NAME ));
      return ( retStatus );
   }
   ...
   if ( retStatus != XDRV_SLIC_STATUS_SUCESS )
   {
      XDRV_LOG_ERROR((XDRV_LOG_MOD_SLIC, "%s: %s FXS line initializion FAILED !.\n", LOG_MODULE_NAME, LOG_DEV_NAME ));
      return ( retStatus );
   }

   XDRV_LOG_NOTICE((XDRV_LOG_MOD_SLIC, "%s: %s device %d and FXS initializion COMPLETED OK !\n", LOG_MODULE_NAME, LOG_DEV_NAME, pDev->deviceId ));
...
/*
*****************************************************************************
** FUNCTION:   slicLe88264Deinit
**
** PURPOSE:    Shutdown the LE88264 SLIC
**
** PARAMETERS: 
**
** RETURNS:    XDRV_SLIC_STATUS
...
*****************************************************************************
*/
XDRV_SLIC_STATUS slicLe88264Deinit( SLIC_DEVICE_DRV *pDev )
{
   XDRV_SLIC_STATUS retStatus = XDRV_SLIC_STATUS_SUCESS;
...
*
*****************************************************************************/
XDRV_SLIC_STATUS slicLe88264Shutdown( XDRV_UINT16 rstGpioPin )
{
