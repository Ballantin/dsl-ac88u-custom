...
static const BCM_MSPI6816_REGS mspi6816DefaultRegs =
{
   ...
   BCM_MSPI6816_CTRL_MODE_INTERRUPT  /* Interrupt mode */
#else
   BCM_MSPI6816_CTRL_MODE_POLLING /*Polling mode*/
#endif /* MSPI6816_USE_INTERRUPTS */
};

/* ---- Private Variables ------------------------------------ */
/* ---- Private Function Prototypes -------------------------- */
static int setMspiRegs( BCM_MSPI6816_DRV *pMspiDrv, BCM_MSPI6816_REGS *pNewMspiRegs );
static int getMspiRegs( BCM_MSPI6816_DRV *pMspiDrv, BCM_MSPI6816_REGS *pNewMspiRegs );
...
*****************************************************************************
*/
int mspiIOctl( BCM_MSPI6816_CMD command, void *cmdp )
{
   int result;
   ...
   switch ( command )
   {
      case BCM_MSPI6816_INITREGS_CMD:
      {
         result = setMspiRegs( pMspiDrv, (BCM_MSPI6816_REGS *)&mspi6816DefaultRegs );
      }
      break;

      case BCM_MSPI6816_SETREGS_CMD:
      {
         result = setMspiRegs( pMspiDrv, cmdp );
      }
      break;

      case BCM_MSPI6816_GETREGS_CMD:
      {
         result = getMspiRegs( pMspiDrv, cmdp );
      }
      break;

      case BCM_MSPI6816_RXCALLBACK_INSTALL_CMD:
      {
      ...
      case BCM_MSPI6816_TXCALLBACK_INSTALL_CMD:
      {
...
*****************************************************************************
*/
static int setMspiRegs( BCM_MSPI6816_DRV *pMspiDrv, 
                        BCM_MSPI6816_REGS *pNewMspiRegs )
{
   volatile SpiControl *pMspiHwRegs = pMspiDrv->p6816SpiRegs;

   /* If MSPI is busy then abort */
   if (( pMspiHwRegs->spiStatus & MMR6816_MSPI_STATUS_SERBUSY ) || ( pMspiHwRegs->spiStatus & MMR6816_MSPI_STATUS_CMDBUSY ))
   {
   ...
   */
   if( pNewMspiRegs->mspiCtrlMode == BCM_MSPI6816_CTRL_MODE_POLLING )
   {
      XDRV_LOG_DBG((XDRV_LOG_MOD_XDRV, "INFO: MSPI POLLING MODE"));

      pMspiHwRegs->spiIntMask = 0x00;
      mspi6816intc_IrqDisable();
   }
#if MSPI6816_USE_INTERRUPTS
   else
   {
...
*****************************************************************************
*/
static int getMspiRegs( BCM_MSPI6816_DRV *pMspiDrv, 
                        BCM_MSPI6816_REGS *pNewMspiRegs )
{
   volatile SpiControl *pMspiHwRegs = pMspiDrv->p6816SpiRegs;
   ...
   {
      pNewMspiRegs->mspiCtrlMode = BCM_MSPI6816_CTRL_MODE_INTERRUPT;
   }
   else
   {
      pNewMspiRegs->mspiCtrlMode = BCM_MSPI6816_CTRL_MODE_POLLING;
   }
