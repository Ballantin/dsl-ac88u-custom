...
/* 
** This is the 6816 custom register table with values customized for
** specific HW layouts with specific SLICs.
*/
static BCM6816_HVG_CUSTOM regCustomTable[XDRV_APM_HVG_DESIGN_NUM_TYPES] =
{
...
#define HVG_OVERVOLTAGE_MAX_COUNT 2

static int hvg6816RegSetup( XDRV_APM *pApmDrv );
static int hvg6816UpdateHvgParms( XDRV_APM *pApmDrv, int chan );
static XDRV_APM_HVG_VOLTAGE hvg6816GetVoltage( XDRV_UINT16 mode, XDRV_UINT16 hookState, int battery );
static XDRV_SINT32 hvgExtend21to32( XDRV_UINT32 value );

/*****************************************************************************
**
**  FUNCTION:   hvg6816RegCheck
**
...
static int hvg6816RegSetup( XDRV_APM *pApmDrv )
{
   ...
   hvgp->reg_hvg_scale_vinput  = HVG_6816_INPUT_SCALE;  /* Reg 2 - Lower 16 Bits */
   ...
   hvgp->reg_hvg_turns_ratio   = 0x00000200;            /* Reg 7 */ 
   hvgp->reg_hvg_duty_cycle    = 0x991A01A0;            /* Reg 8 */
   hvgp->reg_hvg_off_hook      =  0x0001E0A0;      /* Reg 10 */  
   hvgp->reg_hvg_bg            = 0x00000000;            /* Reg 11 */
   hvgp->reg_hvg_vbat          = 0x00000EEA;            /* Reg 12 */

   /* Retreive OTP trim value from APM and update HVG value */
   otpTrimValue                  = (( pApm->reg_otp_0 & APM_OTP_TRIM ) >> APM_OTP_TRIM_SHIFT );
   hvgp->reg_hvg_bg             |= (( otpTrimValue << BG_PTAT_SHIFT ) & BG_PTAT );
   hvgp->reg_hvg_shift          |= BG_TRIM_SOURCE;   

   /* HVG Channel A Settings */
   hvgp->reg_hvg_cha_window_ctrl       =  0x07777337;      /* Reg 13 */
   hvgp->reg_hvg_cha_misc              =  0x0000E884;      /* Reg 15 */
   hvgp->reg_hvg_cha_spare             =  0x00000000;      /* Reg 16 */

   /* HVG Channel B Settings */
   hvgp->reg_hvg_chb_window_ctrl       =  0x07777337;      /* Reg 17 */
   hvgp->reg_hvg_chb_misc              =  0x0000E884;      /* Reg 19 */
   hvgp->reg_hvg_chb_spare             =  0x00000000;      /* Reg 20 */

   /* Platform Specific Settings */
   /* HVG Shared Settings */
   hvgp->reg_hvg_track_max             =  regCustomTable[pDev->Cfg.hvgType].reg_hvg_track_max;         /* Reg 7 */ 
   hvgp->reg_hvg_ring                  =  regCustomTable[pDev->Cfg.hvgType].reg_hvg_ring;              /* Reg 9 */
   hvgp->reg_hvg_cha_max_hvg_slic      =  regCustomTable[pDev->Cfg.hvgType].reg_hvg_max_hvg_slic;      /* Reg 14 - Upper 16 Bits */
   hvgp->reg_hvg_off_hook              =  regCustomTable[pDev->Cfg.hvgType].reg_hvg_off_hook |         /* Reg 10 */
                                          0x0001E000;

   /* HVG Channel A Settings */
   hvgp->reg_hvg_cha_max_hvg_slic      =  regCustomTable[pDev->Cfg.hvgType].reg_hvg_max_hvg_slic;      /* Reg 14 - Upper 16 Bits */
   hvgp->reg_hvg_cha_const_volt_goal   =  regCustomTable[pDev->Cfg.hvgType].reg_hvg_const_volt_goal;   /* Reg 14 - Lower 16 Bits */

   /* HVG Channel B Settings */
   hvgp->reg_hvg_chb_max_hvg_slic      =  regCustomTable[pDev->Cfg.hvgType].reg_hvg_max_hvg_slic;      /* Reg 18 - Upper 16 Bits */
   hvgp->reg_hvg_chb_const_volt_goal   =  regCustomTable[pDev->Cfg.hvgType].reg_hvg_const_volt_goal;   /* Reg 18 - Lower 16 Bits */
...
int hvg6816SetVoltage( XDRV_APM *pApmDrv, int chan, int voltage )
{
   ...
   switch (voltage)
   {
      case XDRV_APM_HVG_VOLTAGE_OFFHOOK:
      {
         if ( chan == 0 )
         {
            /* Clear HVG mode and set to offhook tracking mode */
            hvgRegData = hvgp->reg_hvg_cha_misc;
            hvgRegData &= ~HVG_MODE;
            hvgRegData |= HVG_MODE_OFFHOOK_TRACKING;
...
/*
*****************************************************************************
** FUNCTION:   hvg6816UpdateSlicStatus
**
                             ...
                             XDRV_BOOL ringStop )
{
   ...
   volatile BCM6816_APM *pApm = pDev->pApm;
   volatile BCM6816_HVG *hvgp = BCM6816_HVG_PTR;
   const APM6816_FLEXICALC_CFG *flexicalcp = pDev->Cfg.pFlexiCalc;
   ...
   index = chan % APM6816_NUM_CHANNELS;
   ...
   else
   {
      hvgRegData = hvgp->reg_hvg_duty_cycle;
      hvgRegData &= ~MAX_DUTY_CYCLE;
      hvgRegData |= 0x000000A0;
      hvgp->reg_hvg_duty_cycle = hvgRegData;
   }

   /* Determine the battery voltage associated with the current state */
   voltage = hvg6816GetVoltage( pDev->slicState[index], pDev->hookState[index], pDev->battery[index] );

   /* Set the desired battery voltage */
   hvg6816SetVoltage( pApmDrv, chan, voltage );

   /* Update HVG switching parameters */
   hvg6816UpdateHvgParms( pApmDrv, chan );
...
/*
*****************************************************************************
** FUNCTION:   hvg6816UpdateHookStatus
**
...
/*
*****************************************************************************
** FUNCTION:   hvg6816UpdateHvgParms
**
...
static int hvg6816UpdateHvgParms( XDRV_APM *pApmDrv, int chan )
{
   ...
   hvgTrackMax = regCustomTable[pDev->Cfg.hvgType].reg_hvg_track_max;
...
/*
*****************************************************************************
** FUNCTION:   hvg6816GetOverCurrentStatus
**
...
/*
*****************************************************************************
** FUNCTION:   hvg6816GetOverVoltageStatus
**
...
void hvg6816GetOverVoltageStatus( XDRV_APM *pDrv, int chan )
{
   int mode;
   int voltage = 0;
   int voltageExceeded = XDRV_FALSE;
   volatile BCM6816_HVG *hvgp = BCM6816_HVG_PTR;
   ...
   if( chan == 0 )
   {
      mode = hvgp->reg_hvg_cha_misc & HVG_MODE;
   }
   else
   {
      mode = hvgp->reg_hvg_chb_misc & HVG_MODE;
   }

   /* Determine if tracking voltage exceeds acceptable threshhold */
     if(( mode == HVG_MODE_OFFHOOK_TRACKING ) || ( mode == HVG_MODE_OFFHOOK_FASD ))
   {
      /* Retrieve HVG voltage */
      if( chan == 0 )
      {
         voltage = hvgp->reg_hvg_status8 & HVG_MEAN;
      }
      else
      {
         voltage = hvgp->reg_hvg_status9 & HVG_MEAN;
      }

      /* Sign extend voltage from 21 to 32 bits */
      if( voltage & 0x00100000 )
      {
         voltage |= 0xFFE00000;
      }

         /* Determine if HVG voltage */
         if( voltage < -191200 )
         {
            pDev->highVoltage[chan]++;
         }
         else
         {
            pDev->highVoltage[chan] = 0;
         }

         if( pDev->highVoltage[chan] > HVG_OVERVOLTAGE_MAX_COUNT )
         {
            voltageExceeded = XDRV_TRUE;
         }
      }
      else
      {
      ...
      /* Adjust voltage level if necessary */
      if( voltageExceeded )
      {
         if( chan == 0 )
         {
            /* Adjust settings for constant 25V */
            hvgp->reg_hvg_cha_const_volt_goal = 0x215;
         ...
         {
            /* Adjust settings for constant 25V */
            hvgp->reg_hvg_chb_const_volt_goal = 0x215;
...
/*
*****************************************************************************
** FUNCTION:   hvg6816UpdateHookStatusEnable
**
