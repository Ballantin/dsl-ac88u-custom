...
****************************************************************************
*
*    Description:  APM (Audio Processor Module) device driver for BCM6816
*
****************************************************************************/
...
#define RING_SCALE_FACTOR           1
#define RING_PHASE_FACTOR           4
#define ASRC_DEC_SCALE_FACTOR       1
#define DEFAULT_ASRC_DEC_SCALE      0xA3A
#define DEFAULT_ASRC_DEC_SHFT       5
#define DEFAULT_ASRC_INT_FIR_SHFT   5

/* Ring Frequency Ranges and Scaling Factors */
#define APM6816_RING_FREQ_MAX         75
#define APM6816_RING_FREQ_MIN         15
#define APM6816_RING_FREQ_SCALE       32768 / 3125   /* 2^20/100kHz */
...
#define APM6816_ALL_CHANNELS          0xFF
...
typedef struct APM6816_DMA_SIMPLEX_CHANNEL_INFO
{
   ...
   APM6816_DMA_BUFFER   *dmaDatap;     /* Where are the DMA buffers to store/fetch the data to/from for each channel */
   DMA_HANDLE           dmaBufHandle;
} APM6816_DMA_SIMPLEX_CHANNEL_INFO;

/* DMA Channel info (duplex - both ways) for easy access to DMA channels */
typedef struct APM6816_DMA_DUPLEX_CHANNEL_INFO
{
   APM6816_DMA_SIMPLEX_CHANNEL_INFO rxChan;  /* RX channel */
   APM6816_DMA_SIMPLEX_CHANNEL_INFO txChan;  /* TX channel */
} APM6816_DMA_DUPLEX_CHANNEL_INFO;

/* Ring Gain Settings */
typedef struct APM6816_RING_GAIN_INFO
{
...
} APM6816_RING_GAIN_INFO;

/* asin table struct */
typedef struct APM6816_ASIN
{
   XDRV_SINT32   value;
   XDRV_SINT32   angle;
} APM6816_ASIN;
...
static APM6816_DMA_BUFFER dmaRxData[APM6816_NUM_CHANNELS*DMA_RX_NUM_BUFFERS];
static APM6816_DMA_BUFFER dmaTxData[APM6816_NUM_CHANNELS*DMA_TX_NUM_BUFFERS];

/* Initialized DMA channel info */
static APM6816_DMA_DUPLEX_CHANNEL_INFO gApmDmaInfo[APM6816_NUM_CHANNELS] =
{
   {
      /* RX Channel 0 */
      {
         BCM6816_APM_IUDMA_RX_A,
...
{
   {  0,   8,  0x10000000,    0,  0x0A7C5AC4 },
   {  0,  16,  0x20000000,    0,  0x14F8B589 },
   { -1,  -1,          -1,   -1,          -1 }
};

/* Table for mapping ring gains factors to slic types */
static const APM6816_RING_GAIN_INFO ringGainTable[] =
{
   { 0x3239,   0,  10,  110 },
   { 0x9530,  18,  50,  150 },
   { 0x0000,   0,  50,    0 }
...
static const APM6816_ASIN apmAsinTable[]=
{
...
static int apm6816_dmaInit( volatile BCM6816_APM *apmp, volatile BCM6816_IUDMA *iuDmap, int sampleRate, int packetRate);
static int apm6816_dmaDeinit( volatile BCM6816_APM *apmp, volatile BCM6816_IUDMA *iuDmap);
static int apm6816_regInit(volatile BCM6816_APM *pAmp, const APM6816_CFG *pCfg );
//static int apm6816_regDeinit( );
static int apm_pllStart(volatile BCM6816_APM *pApm);
static int initIudma( volatile BCM6816_IUDMA *iuDmap);
static int deinitIudma( volatile BCM6816_IUDMA *iuDmap );
...
#endif
static int WriteApmFilterCoeffs( volatile BCM6816_APM *pApm, int chan, APM6816_FILTER_TYPE filter, int *data, int length );
static XDRV_SINT64 divRound( XDRV_SINT64 dividend, XDRV_SINT64 divisor );

/* APM "meta-driver" interface functions */
static void apm6816SetRingParms( XDRV_APM *pDrv, int ringFrequency, int ringWaveshape, int ringVoltage, int ringOffset, int ringOffsetCal );
static void apm6816GetRingParms( XDRV_APM *pDrv, int *ringFrequency, int *ringWaveshape, int *ringVoltage, int *ringOffset, int *ringOffsetCal );
static XDRV_SINT32 apm6816DebugStatusDump( XDRV_APM *pDrv );
static XDRV_SINT16 apm6816GetDlp( XDRV_APM *pDrv);
static XDRV_SINT16 apm6816GetElp( XDRV_APM *pDrv);
static void apm6816PulseSignal( XDRV_APM *pDrv, int endpt, int duration, int period,
                                int repetition, int amplitude, int frequency );
static void apm6816SetHPF( XDRV_APM *pDrv, int endpt, XDRV_BOOL enable );
static int apm6816SetTxRxGains( XDRV_APM *pDrv, int chan, int txGain, int rxGain, int *apmTxGain, int *apmRxGain );
static XDRV_SINT64 apmAsin(XDRV_SINT32 value);
/* APM interface */
static const XDRV_APM_FUNCS bcm6816ApmDrvFuncs =
{
   apm6816SetRingParms,
   apm6816GetRingParms,
   apm6816GetDlp,
   apm6816DebugStatusDump,
   apm6816GetElp,
   hvg6816GetOverCurrentStatus,
   hvg6816GetOverVoltageStatus,
   apm6816PulseSignal,
   hvg6816SetVoltage,
   apm6816SetHPF,
   hvg6816UpdateSlicStatus,
   hvg6816UpdateHookStatus,
   hvg6816UpdateHookStatusEnable,
   apm6816SetTxRxGains
};

/* ---- Public Functions -------------------------------------------------- */
/* ---- Private Functions ------------------------------------------------- */
#define APM6816_USLEEP(cycles) ( {volatile int i; for (i=0; i < (cycles); i++) {}} )

/*****************************************************************************
*
*  FUNCTION: apm6816_init
*
*  PURPOSE:
*      Intialize 6816 APM Driver interface
*
*  PARAMETERS:
*      pDrv      - driver interface
*      pIntCtl   - pointer to 6816's Interrupt Control module
...
int apm6816_init(
   ...
   const APM6816_CFG *cfgp
)
{
   int result;

   /* Set the "meta-driver" interface */
   pDrv->pDrvFuncs = &bcm6816ApmDrvFuncs;

   /* Initialize the driver state variables */
   pDrv->pApm = BCM6816_APM_PTR;
   pDrv->pIuDma = BCM6816_APM_IUDMA_PTR;
   pDrv->pIntCtl = pIntCtl;
   pDrv->Cfg = *cfgp;

   /* Now start the APM module */
   result = apm6816_start(pDrv);
...
/*****************************************************************************
*
*  FUNCTION: apm6816_deinit
*
*  PURPOSE:
*      DeIntialize 6816 APM Driver interface
*
*  PARAMETERS:
*      pDrv       - 6816 driver interface
*
*  RETURNS: 0 on SUCCESS
*
*  NOTES:
*
*****************************************************************************/
int apm6816_deinit( APM_6816_DRV *pDrv )
{
   /* First stop the APM */
   apm6816_stop(pDrv);

   /* Clear the driver state */
   memset(pDrv,0, sizeof(APM_6816_DRV));
...
/*****************************************************************************
*
*  FUNCTION: apm6816_start
*
*  PURPOSE:
*      Initialize and start the actual 6816 Audio Processing Module
*
*  PARAMETERS:
*      pDrv       - 6816 driver interface
*
...
*
*****************************************************************************/
int apm6816_start( APM_6816_DRV  *pDrv )
{
   volatile int i;
...
#if 0 //VM TC block doesn't exist on 6816. Reference clock should be provided by NTR
   /*
   ** Make sure Timing Control Module (TC) is enabled.
   ** This is needed for APM clocking.
   */
   if (!( pDrv->pIntCtl->blkEnables & TC_CLK_EN ))
   {
      XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "XDRV_APM: Warning - TC clocks disabled. !!!"));
   }
#endif
   ...
   */
   if ( apm6816_regInit( pDrv->pApm, &pDrv->Cfg ))
   {
      return ( -1 );
   }
   ...
   */
   if ( apm6816_flexicalcConfig( pDrv->pApm, pDrv->Cfg.pFlexiCalc ))
   {
   ...
   ** Launch the APM DMAs
   */
   if ( apm6816_dmaInit( pDrv->pApm, pDrv->pIuDma, pDrv->Cfg.sampleRate, pDrv->Cfg.packetRate ))
   {
   ...
   for( i = 0; i < APM6816_NUM_CHANNELS; i++ )
   {
...
/*****************************************************************************
*
*  FUNCTION: apm6816_stop
*
*  PURPOSE:
*      Stop and deinitialize the 6816 Audio Processing Module
*
*  PARAMETERS:
*      pDrv       - 6816 driver interface
*
...
*
*****************************************************************************/
int apm6816_stop( APM_6816_DRV *pDrv )
{
   /*
   ** Stop the APM DMA
   */
   if (apm6816_dmaDeinit( pDrv->pApm, pDrv->pIuDma))
   {
...
/*****************************************************************************
*
*  FUNCTION: apm6816_dmaInit
*
*  PURPOSE:
*
*  PARAMETERS:
*      apmp       - pointer to 6816's APM module
*      iuDmap     - pointer to 6816's APM IUDMA module
...
*
*****************************************************************************/
static int apm6816_dmaInit
(
   volatile BCM6816_APM    *apmp,
   volatile BCM6816_IUDMA  *iuDmap,
   int                     sampleRate,
   int                     packetRate
)
{
...
static int initDmaDescriptors( int sampleRate, int packetRate )
{
   ...
   volatile APM6816_DMA_BUFFER*     dmaRxData;
   volatile APM6816_DMA_BUFFER*     dmaTxData;
   APM6816_DMA_SIMPLEX_CHANNEL_INFO *dmaChanInfop;
   ...
   rxDescriptorArea = sizeof(BCM6816_IUDMA_DESC) * APM6816_NUM_CHANNELS * DMA_RX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6816_DMA_UBUS_BYTES;
   txDescriptorArea = sizeof(BCM6816_IUDMA_DESC) * APM6816_NUM_CHANNELS * DMA_TX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6816_DMA_UBUS_BYTES;

   /* Allocate receive and transmit descriptors */

   dmaRxDesc_c = (volatile BCM6816_IUDMA_DESC *)dma_alloc_coherent( NULL, rxDescriptorArea, &gApmDmaInfo[0].rxChan.dmaDescHandle, GFP_KERNEL );
   dmaTxDesc_c = (volatile BCM6816_IUDMA_DESC *)dma_alloc_coherent( NULL, txDescriptorArea, &gApmDmaInfo[0].txChan.dmaDescHandle, GFP_KERNEL );
   ...
   for (i=0; i < APM6816_NUM_CHANNELS; i++)
   {
      ...
      gApmDmaInfo[i].rxChan.dmaDescp = (BCM6816_IUDMA_DESC*)(dmaRxDesc_c + i * DMA_RX_NUM_BUFFERS);
      gApmDmaInfo[i].txChan.dmaDescp = (BCM6816_IUDMA_DESC*)(dmaTxDesc_c + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   rxBufferArea = sizeof(XDRV_UINT32) * (APM6816_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * APM6816_NUM_CHANNELS * DMA_RX_NUM_BUFFERS;
   txBufferArea = sizeof(XDRV_UINT32) * (APM6816_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * APM6816_NUM_CHANNELS * DMA_TX_NUM_BUFFERS;
   ...
   for (i=0; i < APM6816_NUM_CHANNELS; i++)
     {
      gApmDmaInfo[i].rxChan.dmaDatap = (APM6816_DMA_BUFFER*)(dmaRxData + i * DMA_RX_NUM_BUFFERS);
      gApmDmaInfo[i].txChan.dmaDatap = (APM6816_DMA_BUFFER*)(dmaTxData + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   for (chnum=0; chnum<APM6816_NUM_CHANNELS; chnum++)
   {
      ...
      for( i = 0; i < DMA_TX_NUM_BUFFERS; i++ )
      {
         ...
         if( i == 0 )
         {
            ...
            bufferSizeBytes = (((packetRate+1)*sampleRate)<<1) + APM6816_TX_DMA_BUF_DEPTH_BYTES; 
         }
         ...
         if (bufferSizeBytes <= APM6816_DMA_MAX_BUFFER_SIZE_BYTES)
         {
         ...
         else
         {
            ...
            ** You can either increase the compile time buffer size (APM6816_DMA_MAX_BUFFER_SIZE),
...
{
   int chnum;
   volatile BCM6816_IUDMA_STRAM *stRamp = iuDmap->stram;

   /* Reset entire IUDMA state RAM */
   memset( (void*)stRamp, 0, BCM6816_IUDMA_MAX_CHANNELS * sizeof(BCM6816_IUDMA_STRAM) );
   ...
   for (chnum=0; chnum<APM6816_NUM_CHANNELS; chnum++)
   {
      ...
      iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intMask = BCM6816_IUDMA_INTMASK_BDONE | BCM6816_IUDMA_INTMASK_NOTVLD;
      iuDmap->regs.intMask |= ( 1 << gApmDmaInfo[chnum].rxChan.iuChanNum );
      ...
      iuDmap->ctrl[gApmDmaInfo[chnum].txChan.iuChanNum].intMask = BCM6816_IUDMA_INTMASK_BDONE | BCM6816_IUDMA_INTMASK_NOTVLD;
      iuDmap->regs.intMask |= ( 1 << gApmDmaInfo[chnum].txChan.iuChanNum );
   }
...
*
*****************************************************************************/
static int apm6816_dmaDeinit
(
   volatile BCM6816_APM *apmp,
   volatile BCM6816_IUDMA *dmap
)
{
   /* Stop the IUDMA */
   deinitIudma( dmap );
...
*
*  PARAMETERS:
*      pApm     - pointer to 6816's APM module
*
...
static int apm_pllStart(volatile BCM6816_APM *pApm)
{
 ...
 retry_lock:
   ...
   XDRV_LOG_ERROR(( XDRV_LOG_MOD_XDRV, "%s: DPLL Locked, Iteration = %u dpll_status = 0x%08x", __FUNCTION__, (unsigned int)i, (unsigned int) temp));
...
*
*****************************************************************************/
static int apm6816_regInit(volatile BCM6816_APM *pApm, const APM6816_CFG *pCfg )
{
   ...
   {
      XDRV_LOG_ERROR(( XDRV_LOG_MOD_XDRV, "APM6816 regInit ERROR: Unsupported sample rate (%d) !!! ", pCfg->sampleRate));
      return ( -1 );
   }

   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6816 regInit: Using %d kHz sample rate.", tablep->sampleRate));
   ...
   */
   ...
   {
      pApm->reg_ring_config_1 = ((0 & RING_OFFSET_A) |
                                ((0x49 << RING_SCALE_A_SHIFT) & RING_SCALE_A) |
                                ((0 << RING_OFFSET_B_SHIFT) & RING_OFFSET_B) |
                                ((0x49 << RING_SCALE_B_SHIFT) & RING_SCALE_B) );
   }
   else
   {
      pApm->reg_ring_config_1 = ((0 & RING_OFFSET_A) |
                                ((0x46 << RING_SCALE_A_SHIFT) & RING_SCALE_A) |
                                ((0 << RING_OFFSET_B_SHIFT) & RING_OFFSET_B) |
                                ((0x46 << RING_SCALE_B_SHIFT) & RING_SCALE_B) );
   }

   pApm->reg_ring_config_2 = ((0x7FFF & RING_CLIP_A) |
                             ((1 << RING_GEN_ENABLE_A_SHIFT) & RING_GEN_ENABLE_A) |
                             ((0x7FFF << RING_CLIP_B_SHIFT) & RING_CLIP_B) |
                             ((1 << RING_GEN_ENABLE_B_SHIFT) & RING_GEN_ENABLE_B) );

   pApm->reg_ring_config_3 = ((0x36 & RING_OMEGA_NORM_A) |
                             ((0x36 << RING_OMEGA_NORM_B_SHIFT) & RING_OMEGA_NORM_B) |
                             ((0 << RING_BREAK_PHASE_A_SHIFT) & RING_BREAK_PHASE_A) |
...
/*****************************************************************************
*
*  FUNCTION: apm6816_flexicalcConfig
*
*  PURPOSE:
...
*
*****************************************************************************/
int apm6816_flexicalcConfig
(
   volatile BCM6816_APM *pApm,
   const APM6816_FLEXICALC_CFG *flexicalcp
)
{
   ...
   if ( WriteApmFilterCoeffs( pApm, APM6816_ALL_CHANNELS, APM6816_FILTER_HISPD_HYBRID_BAL, (int *)&flexicalcp->hybal_audio_fir[0], APM6816_HYBAL_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load High-speed Hybrid Balance coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6816_ALL_CHANNELS, APM6816_FILTER_LOSPD_HYBRID_BAL, (int *)&flexicalcp->hybal_audio_fir[0], APM6816_HYBAL_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load Y-filter coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6816_ALL_CHANNELS, APM6816_FILTER_Y_FILTER, (int *)&flexicalcp->yfltr_iir2[0], APM6816_YFLTR_BLOCK_SIZE ))
   {
      return ( 1 );
   }

   /* Load RX equalizer coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6816_ALL_CHANNELS, APM6816_FILTER_RX_EQ, (int *)&flexicalcp->eq_rx_filter[0], APM6816_RX_EQ_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load TX equalizer coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6816_ALL_CHANNELS, APM6816_FILTER_TX_EQ, (int *)&flexicalcp->eq_tx_filter[0], APM6816_TX_EQ_NUM_TAPS ))
   {
      return ( 1 );
   }
...
/*
*****************************************************************************
** FUNCTION:   apm6816DebugStatusDump
**
...
static XDRV_SINT32 apm6816DebugStatusDump( XDRV_APM *pDrv )
{
   APM_6816_DRV            *pDev  = (APM_6816_DRV *)pDrv;
   volatile BCM6816_APM    *pApm;
   volatile BCM6816_IUDMA  *iuDmap;
   volatile PerfControl     *pIntCtl;
   int                      chnum;
...
static XDRV_SINT16 apm6816GetDlp( XDRV_APM *pDrv )
{
   ...
   /* The 6816 currently only supports DLP in the [-12dB,0dB] range */
...
static XDRV_SINT16 apm6816GetElp( XDRV_APM *pDrv )
{
   ...
   /* The 6816 currently only supports ELP in the [-12dB,0dB] range */
                                 ...
                                 int ringVoltage, int ringOffset, int ringOffsetCal )
{
   ...
   switch ( ringWaveshape )
   {
      ...
      {
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6816 reprogramming ring registers to trapezoidal waveform."));
      ...
      {
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6816 reprogramming ring registers to sinusoidal waveform."));
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "Frequency %d, Voltage %d, Offset %d, Type %d",
                         ringFrequency, ringVoltage, ringOffset, ringWaveshape ));
      ...
      default:
      {
         XDRV_LOG_WARN(( XDRV_LOG_MOD_XDRV, "Unknown ringwaveform type in APM ring configuration."));
         return;
      }
      break;
   }
   ...
   switch( ringGainTablep->slicType )
   {
      case 0x3239:
      {
         ...
         max_hvg_slic = 0;
#endif
         max_hvg_target = apmDiv(( vmax_hvg_slic - 1200 ) * 15900, 1000);
      }
      break;
   ...
   if( hvg_ring_gain >= 4095 )
   {
      hvg_ring_ovhd = ((apmDiv(( peak_ring + voh_min ), vres )) -
                       apmDiv(( ring_amph + apmAbs( ring_offh )) * 4095 , 256 * RING_SCALE_FACTOR));
   }
   else
   {
   ...
   if( ringWaveshape == XDRV_RINGWAVEFORM_SINE )
   {
      ring_clip = ring_sin_amp;
   }
   else
   {
      ring_clip = divRound(( 536870912 ), ( ring_sin_amp ));
      ring_amph = divRound(( ring_sin_amp * peak_ac ), ( ring_gain * apmAbs( ringFS ) * 128 ));
   }
   ...
   pApm->reg_ring_config_1 = ((( ring_offh << RING_OFFSET_A_SHIFT ) & RING_OFFSET_A ) |
                              (( ring_amph << RING_SCALE_A_SHIFT ) & RING_SCALE_A ) |
                              (( ring_offh << RING_OFFSET_B_SHIFT ) & RING_OFFSET_B ) |
                              (( ring_amph << RING_SCALE_B_SHIFT ) & RING_SCALE_B ));

   pApm->reg_ring_config_2 = ((( ring_clip << RING_CLIP_A_SHIFT ) & RING_CLIP_A ) |
                              (( 1 << RING_GEN_ENABLE_A_SHIFT ) & RING_GEN_ENABLE_A ) |
                              (( ring_clip << RING_CLIP_B_SHIFT ) & RING_CLIP_B ) |
                              (( 1 << RING_GEN_ENABLE_B_SHIFT ) & RING_GEN_ENABLE_B ));

   pApm->reg_ring_config_3 = ((( ring_freqh << RING_OMEGA_NORM_A_SHIFT ) & RING_OMEGA_NORM_A ) |
                              (( ring_freqh << RING_OMEGA_NORM_B_SHIFT ) & RING_OMEGA_NORM_B ) |
                              (( ring_break_phase << RING_BREAK_PHASE_A_SHIFT ) & RING_BREAK_PHASE_A ) |
...
*****************************************************************************
*/
static void apm6816GetRingParms( XDRV_APM *pDrv, int *ringFrequency, int *ringWaveshape,
                                 int *ringVoltage, int *ringOffset, int *ringOffsetCal )
{
   APM_6816_DRV *pDev  = (APM_6816_DRV *)pDrv;
...
*****************************************************************************
*/
void apm6816SetClkSource( XDRV_APM *pDrv, int clkSource )
{
}
...
**
** PARAMETERS: pApm   - pointer to 6816's APM module
...
static int WriteApmFilterCoeffs( volatile BCM6816_APM *pApm, int chan, APM6816_FILTER_TYPE filter, int *data, int length )
{
   ...
   if (( chan < 0 || chan > APM6816_NUM_CHANNELS ) && ( chan != APM6816_ALL_CHANNELS ))
   {
   ...
   switch (filter)
   {
      case APM6816_FILTER_TX_EQ:
      {
         if( chan == APM6816_ALL_CHANNELS )
         {
      ...
      case APM6816_FILTER_HISPD_HYBRID_BAL:
      {
         if( chan == APM6816_ALL_CHANNELS )
         {
      ...
      case APM6816_FILTER_LOSPD_HYBRID_BAL:
      {
         if( chan == APM6816_ALL_CHANNELS )
         {
...
static XDRV_SINT64 divRound( XDRV_SINT64 dividend, XDRV_SINT64 divisor )
{
   ...
   if( divisor == 0 )
   {
      XDRV_LOG_ERROR(( XDRV_LOG_MOD_XDRV, "ERROR: APM6816 - Divisor of zero entered!"));
      return( 1 );
   }
...
static XDRV_SINT64 apmAsin(XDRV_SINT32 value)
{
   ...
   if( value<apmAsinTable[0].value || value> apmAsinTable[sizeof(apmAsinTable)/sizeof(APM6816_ASIN)-1].value)
   {
     return 0;
   }
