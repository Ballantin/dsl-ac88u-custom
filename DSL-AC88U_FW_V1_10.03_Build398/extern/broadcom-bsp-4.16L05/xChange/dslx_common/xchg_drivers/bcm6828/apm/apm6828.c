...
****************************************************************************
*
*    Description:  APM (Audio Processor Module) device driver for BCM6828
*
****************************************************************************/
...
#define APM6828_RING_FREQ_MAX         75
#define APM6828_RING_FREQ_MIN         15
#define APM6828_RING_FREQ_SCALE       32768 / 3125   /* 2^20/100kHz */
...
#define APM6828_ALL_CHANNELS          0xFF
...
typedef struct APM6828_DMA_SIMPLEX_CHANNEL_INFO
{
   int                  iuChanNum;     /* IUDMA channel number assosiated with this APM channel */
   BCM6828_IUDMA_DESC   *dmaDescp;     /* Where are the descriptors for this channel? */
   DMA_HANDLE           dmaDescHandle;
   APM6828_DMA_BUFFER   *dmaDatap;     /* Where are the DMA buffers to store/fetch the data to/from for each channel */
   DMA_HANDLE           dmaBufHandle;
} APM6828_DMA_SIMPLEX_CHANNEL_INFO;

/* DMA Channel info (duplex - both ways) for easy access to DMA channels */
typedef struct APM6828_DMA_DUPLEX_CHANNEL_INFO
{
   APM6828_DMA_SIMPLEX_CHANNEL_INFO rxChan;  /* RX channel */
   APM6828_DMA_SIMPLEX_CHANNEL_INFO txChan;  /* TX channel */
} APM6828_DMA_DUPLEX_CHANNEL_INFO;

/* Ring Gain Settings */
typedef struct APM6828_RING_GAIN_INFO
{
...
} APM6828_RING_GAIN_INFO;

/* asin table struct */
typedef struct APM6828_ASIN
{
   XDRV_SINT32   value;
   XDRV_SINT32   angle;
} APM6828_ASIN;
...
static APM6828_DMA_BUFFER dmaRxData[APM6828_NUM_CHANNELS*DMA_RX_NUM_BUFFERS];
static APM6828_DMA_BUFFER dmaTxData[APM6828_NUM_CHANNELS*DMA_TX_NUM_BUFFERS];

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)
extern struct dma_pool *pcm_dma_pool;
#endif
/* Initialized DMA channel info */
static APM6828_DMA_DUPLEX_CHANNEL_INFO gApmDmaInfo[APM6828_NUM_CHANNELS] =
{
   {
      /* RX Channel 0 */
      {
         BCM6828_APM_IUDMA_RX_A,
...
static const APM6828_RING_GAIN_INFO ringGainTable[] =
{
   { 0x3239,  58,  10,  114 },
   { 0x32392, 63,  10,  108 },
   { 0x9530,  18,  50,  150 },
...
static const APM6828_ASIN apmAsinTable[]=
{
...
static int apm6828_dmaInit( volatile BCM6828_APM *apmp, volatile BCM6828_IUDMA *iuDmap, int sampleRate, int packetRate);
static int apm6828_dmaDeinit( volatile BCM6828_APM *apmp, volatile BCM6828_IUDMA *iuDmap);
static int apm6828_regInit(volatile BCM6828_APM *pApm, const APM6828_CFG *pCfg );
static int apm_pllStart(volatile BCM6828_APM *pApm);
static int initIudma( volatile BCM6828_IUDMA *iuDmap);
static int deinitIudma( volatile BCM6828_IUDMA *iuDmap );
...
#endif
static int WriteApmFilterCoeffs( volatile BCM6828_APM *pApm, int chan, APM6828_FILTER_TYPE filter, int *data, int length );
static XDRV_SINT64 divRound( XDRV_SINT64 dividend, XDRV_SINT64 divisor );

/* APM "meta-driver" interface functions */
static void apm6828SetRingParms( XDRV_APM *pDrv, int ringFrequency, int ringWaveshape, int ringVoltage, int ringOffset, int ringOffsetCal );
static void apm6828GetRingParms( XDRV_APM *pDrv, int *ringFrequency, int *ringWaveshape, int *ringVoltage, int *ringOffset, int *ringOffsetCal );
static XDRV_SINT32 apm6828DebugStatusDump( XDRV_APM *pDrv );
static XDRV_SINT16 apm6828GetDlp( XDRV_APM *pDrv);
static XDRV_SINT16 apm6828GetElp( XDRV_APM *pDrv);
static void apm6828PulseSignal( XDRV_APM *pDrv, int endpt, int duration, int period,
                                int repetition, int amplitude, int frequency );
static void apm6828SetHPF( XDRV_APM *pDrv, int endpt, XDRV_BOOL enable );
static int apm6828SetTxRxGains( XDRV_APM *pDrv, int chan, int txGain, int rxGain, int *apmTxGain, int *apmRxGain );
static XDRV_SINT64 apmAsin(XDRV_SINT32 value);

/* APM interface */
static const XDRV_APM_FUNCS bcm6828ApmDrvFuncs =
{
   apm6828SetRingParms,
   apm6828GetRingParms,
   apm6828GetDlp,
   apm6828DebugStatusDump,
   apm6828GetElp,
   hvg6828GetOverCurrentStatus,
   hvg6828GetOverVoltageStatus,
   apm6828PulseSignal,
   hvg6828SetVoltage,
   apm6828SetHPF,   
   hvg6828UpdateSlicStatus,
   hvg6828UpdateHookStatus,
   hvg6828UpdateHookStatusEnable,
   apm6828SetTxRxGains   
};

/* ---- Public Functions -------------------------------------------------- */
/* ---- Private Functions ------------------------------------------------- */
#define APM6828_USLEEP(cycles) ( {volatile int i; for (i=0; i < (cycles); i++) {}} )
...
)
{
   ...
   pDrv->pDrvFuncs = &bcm6828ApmDrvFuncs;

   /* Initialize the driver state variables */
   pDrv->pApm = BCM6828_APM_PTR;
   pDrv->pIuDma = BCM6828_APM_IUDMA_PTR;
   pDrv->pIntCtl = pIntCtl;
   pDrv->Cfg = *cfgp;

   /* Now start the APM module */
   result = apm6828_start(pDrv);
...
int apm6828_deinit( APM_6828_DRV *pDrv )
{
   ...
   memset(pDrv,0, sizeof(APM_6828_DRV));
...
int apm6828_start( APM_6828_DRV  *pDrv )
{
   ...
   PERF->softResetB &= ~SOFT_RST_APM_HARD_SOFT;
   PERF->softResetB &= ~SOFT_RST_APM_AUDIO_SOFT;
   PERF->softResetB &= ~SOFT_RST_HVG_A;
   PERF->softResetB &= ~SOFT_RST_HVG_B;
   ...
   */
   if ( apm6828_regInit( pDrv->pApm, &pDrv->Cfg ))
   {
      return ( -1 );
   }
   ...
   */
   if ( apm6828_flexicalcConfig( pDrv->pApm, pDrv->Cfg.pFlexiCalc ))
   {
   ...
   ** Launch the APM DMAs
   */
   if ( apm6828_dmaInit( pDrv->pApm, pDrv->pIuDma, pDrv->Cfg.sampleRate, pDrv->Cfg.packetRate ))
   {
   ...
   for( i = 0; i < APM6828_NUM_CHANNELS; i++ )
   {
...
int apm6828_stop( APM_6828_DRV *pDrv )
{
   ...
   ** Stop the APM DMA
   */
   if (apm6828_dmaDeinit( pDrv->pApm, pDrv->pIuDma))
   {
...
/*****************************************************************************
*
*  FUNCTION: apm6828_dmaInit
*
*  PURPOSE:
*
*  PARAMETERS:
*      apmp       - pointer to 6828's APM module
*      iuDmap     - pointer to 6828's APM IUDMA module
...
*
*****************************************************************************/
static int apm6828_dmaInit
(
   volatile BCM6828_APM    *apmp,
   volatile BCM6828_IUDMA  *iuDmap,
   int                     sampleRate,
   int                     packetRate
)
{
...
{
   XDRV_UINT8                       i,chnum;
   volatile BCM6828_IUDMA_DESC*     dmaTxDesc;
   volatile BCM6828_IUDMA_DESC*     dmaRxDesc;
   volatile BCM6828_IUDMA_DESC*     dmaTxDesc_c;
   volatile BCM6828_IUDMA_DESC*     dmaRxDesc_c;
   volatile APM6828_DMA_BUFFER*     dmaRxData;
   volatile APM6828_DMA_BUFFER*     dmaTxData;
   APM6828_DMA_SIMPLEX_CHANNEL_INFO *dmaChanInfop;
   ...
   rxDescriptorArea = sizeof(BCM6828_IUDMA_DESC) * APM6828_NUM_CHANNELS * DMA_RX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6828_DMA_UBUS_BYTES;
   txDescriptorArea = sizeof(BCM6828_IUDMA_DESC) * APM6828_NUM_CHANNELS * DMA_TX_NUM_BUFFERS + sizeof(XDRV_UINT8) * BCM6828_DMA_UBUS_BYTES;
...
#else
   dmaRxDesc_c = (volatile BCM6828_IUDMA_DESC *)dma_alloc_coherent( NULL, rxDescriptorArea, &gApmDmaInfo[0].rxChan.dmaDescHandle, GFP_KERNEL );
   dmaTxDesc_c = (volatile BCM6828_IUDMA_DESC *)dma_alloc_coherent( NULL, txDescriptorArea, &gApmDmaInfo[0].txChan.dmaDescHandle, GFP_KERNEL );
#endif
   ...
   for (i=0; i < APM6828_NUM_CHANNELS; i++)
   {
      ...
      dmaRxDesc_c = (volatile BCM6828_IUDMA_DESC *) ALIGN_DATA( (unsigned int) dmaRxDesc_c, BCM6828_DMA_UBUS_BYTES );
      dmaTxDesc_c = (volatile BCM6828_IUDMA_DESC *) ALIGN_DATA( (unsigned int) dmaTxDesc_c, BCM6828_DMA_UBUS_BYTES );

      gApmDmaInfo[i].rxChan.dmaDescp = (BCM6828_IUDMA_DESC*)(dmaRxDesc_c + i * DMA_RX_NUM_BUFFERS);
      gApmDmaInfo[i].txChan.dmaDescp = (BCM6828_IUDMA_DESC*)(dmaTxDesc_c + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   rxBufferArea = sizeof(XDRV_UINT32) * (APM6828_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * APM6828_NUM_CHANNELS * DMA_RX_NUM_BUFFERS;
   txBufferArea = sizeof(XDRV_UINT32) * (APM6828_DMA_MAX_BUFFER_SIZE + DMA_PADDING_WORD32) * APM6828_NUM_CHANNELS * DMA_TX_NUM_BUFFERS;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)
   ...
   for (i=0; i < APM6828_NUM_CHANNELS; i++)
     {
      gApmDmaInfo[i].rxChan.dmaDatap = (APM6828_DMA_BUFFER*)(dmaRxData + i * DMA_RX_NUM_BUFFERS);
      gApmDmaInfo[i].txChan.dmaDatap = (APM6828_DMA_BUFFER*)(dmaTxData + i * DMA_TX_NUM_BUFFERS);
   }
   ...
   for (chnum=0; chnum<APM6828_NUM_CHANNELS; chnum++)
   {
      ...
      dmaTxDesc = (volatile BCM6828_IUDMA_DESC *)CacheToNonCacheVirtAddress((XDRV_UINT32)dmaChanInfop->dmaDescp);
      ...
      for( i = 0; i < DMA_TX_NUM_BUFFERS; i++ )
      {
         ...
         dmaTxDesc[i].flagsLengthStatus.status = BCM6828_IUDMA_DESC_STATUS_SOP |
                                                 BCM6828_IUDMA_DESC_STATUS_EOP;

         if( i == 0 )
         {
            /* First buffer - prime the TX fifo */
            dmaTxDesc[i].flagsLengthStatus.status |= BCM6828_IUDMA_DESC_STATUS_OWN;
            ...
            bufferSizeBytes = (((packetRate+1)*sampleRate)<<1) + APM6828_TX_DMA_BUF_DEPTH_BYTES;
         }

         if( i == ( DMA_TX_NUM_BUFFERS - 1 ) )
         {
            /* Last buffer - set the WRAP bit */
            dmaTxDesc[i].flagsLengthStatus.status |= BCM6828_IUDMA_DESC_STATUS_WRAP;
         }

         /* Check if we can accomodate the desired buffer size */
         if (bufferSizeBytes <= APM6828_DMA_MAX_BUFFER_SIZE_BYTES)
         {
         ...
         else
         {
            ...
            ** You can either increase the compile time buffer size (APM6828_DMA_MAX_BUFFER_SIZE),
         ...
         ** buffers aligned on the cache-line size (16-bytes for the 6828).
      ...
      dmaRxDesc = (volatile BCM6828_IUDMA_DESC *)CacheToNonCacheVirtAddress((XDRV_UINT32)dmaChanInfop->dmaDescp);
      ...
      for( i = 0; i < DMA_RX_NUM_BUFFERS; i++ )
      {
         ...
         dmaRxDesc[i].flagsLengthStatus.status = BCM6828_IUDMA_DESC_STATUS_OWN;

         if( i == ( DMA_RX_NUM_BUFFERS - 1 ) )
         {
            /* Last descriptor - set the wrap bit */
            dmaRxDesc[i].flagsLengthStatus.status |= BCM6828_IUDMA_DESC_STATUS_WRAP;
         }
...
{
   int chnum;
   volatile BCM6828_IUDMA_STRAM *stRamp = iuDmap->stram;
  volatile unsigned short test1 = 0;

   /* Reset entire IUDMA state RAM */
   memset( (void*)stRamp, 0, BCM6828_MAX_APM_PCM_DMA_CHANNELS * sizeof(BCM6828_IUDMA_STRAM) );
...
//      iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intStat |= iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intStat;
      ...
      iuDmap->ctrl[gApmDmaInfo[chnum].rxChan.iuChanNum].intMask = 0x000f & (BCM6828_IUDMA_INTMASK_BDONE | BCM6828_IUDMA_INTMASK_NOTVLD);
      ...
      iuDmap->ctrl[gApmDmaInfo[chnum].txChan.iuChanNum].intMask = BCM6828_IUDMA_INTMASK_BDONE | BCM6828_IUDMA_INTMASK_NOTVLD;
      iuDmap->regs.gbl_int_mask |= ( 1 << gApmDmaInfo[chnum].txChan.iuChanNum );

   }

   /* Enable the IUDMA master */
   iuDmap->regs.ctrlConfig = BCM6828_IUDMA_REGS_CTRLCONFIG_MASTER_EN;
...
*
*****************************************************************************/
static int apm6828_dmaDeinit
(
   volatile BCM6828_APM *apmp,
   volatile BCM6828_IUDMA *dmap
)
{
   /* Stop the IUDMA */
   deinitIudma( dmap );
...
*
*  PARAMETERS:
*      pApm     - pointer to 6828's APM module
*
...
static int apm_pllStart(volatile BCM6828_APM *pApm)
{
 ...
 retry_lock:
...
#if CONFIG_EXTERNAL_NTR_IN
   GPIO->GPIOMode |= ( 1 << 28 ); // This code enables NTR IN ON GPIO 28
   MISC->miscApm_ntr_ctrl = 0x80000000; // This code enables NTR IN ON GPIO 28
#endif /* CONFIG_EXTERNAL_NTR_IN */
   ...
   pApm->dpll_div = (4 << DPLL_REF_DIV_SHIFT) | 8191;     /*  Ref clk divider set to give us 8Khz from input NTR from EPON MAC of 40Khz */
...
static int apm6828_regInit(volatile BCM6828_APM *pApm, const APM6828_CFG *pCfg )
{
   ...
   {
      XDRV_LOG_ERROR(( XDRV_LOG_MOD_XDRV, "APM6828 regInit ERROR: Unsupported sample rate (%d) !!! ", pCfg->sampleRate));
      return ( -1 );
   }

   XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6828 regInit: Using %d kHz sample rate.", tablep->sampleRate));
   ...
   */
   ...
   pApm->reg_codec_config_1 = 0x00220022;
...
*
*****************************************************************************/
int apm6828_flexicalcConfig
(
   volatile BCM6828_APM *pApm,
   const APM6828_FLEXICALC_CFG *flexicalcp
)
{
   ...
   if ( WriteApmFilterCoeffs( pApm, APM6828_ALL_CHANNELS, APM6828_FILTER_HISPD_HYBRID_BAL, (int *)&flexicalcp->hybal_audio_fir[0], APM6828_HYBAL_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load High-speed Hybrid Balance coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6828_ALL_CHANNELS, APM6828_FILTER_LOSPD_HYBRID_BAL, (int *)&flexicalcp->hybal_audio_fir[0], APM6828_HYBAL_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load Y-filter coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6828_ALL_CHANNELS, APM6828_FILTER_Y_FILTER, (int *)&flexicalcp->yfltr_iir2[0], APM6828_YFLTR_BLOCK_SIZE ))
   {
      return ( 1 );
   }

   /* Load RX equalizer coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6828_ALL_CHANNELS, APM6828_FILTER_RX_EQ, (int *)&flexicalcp->eq_rx_filter[0], APM6828_RX_EQ_NUM_TAPS ))
   {
      return ( 1 );
   }

   /* Load TX equalizer coefficients for both channels */
   if ( WriteApmFilterCoeffs( pApm, APM6828_ALL_CHANNELS, APM6828_FILTER_TX_EQ, (int *)&flexicalcp->eq_tx_filter[0], APM6828_TX_EQ_NUM_TAPS ))
   {
      return ( 1 );
   }
...
/*
*****************************************************************************
** FUNCTION:   apm6828DebugStatusDump
**
...
static XDRV_SINT32 apm6828DebugStatusDump( XDRV_APM *pDrv )
{
   APM_6828_DRV            *pDev  = (APM_6828_DRV *)pDrv;
   volatile BCM6828_APM    *pApm;
   volatile BCM6828_IUDMA  *iuDmap;
   volatile PerfControl     *pIntCtl;
   int                      chnum;
...
static XDRV_SINT16 apm6828GetDlp( XDRV_APM *pDrv )
{
   ...
   /* The 6828 currently only supports DLP in the [-12dB,0dB] range */
...
static XDRV_SINT16 apm6828GetElp( XDRV_APM *pDrv )
{
   ...
   /* The 6828 currently only supports ELP in the [-12dB,0dB] range */
...
*****************************************************************************
*/
static void apm6828SetRingParms( XDRV_APM *pDrv, int ringFrequency, int ringWaveshape,
                                 int ringVoltage, int ringOffset, int ringOffsetCal )
{
   APM_6828_DRV *pDev  = (APM_6828_DRV *)pDrv;
   volatile BCM6828_APM *pApm = pDev->pApm;
   volatile BCM6828_HVG *pHvg = BCM6828_HVG_PTR;
   const APM6828_RING_GAIN_INFO *ringGainTablep;
   ...
   switch ( ringWaveshape )
   {
      ...
      {
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6828 reprogramming ring registers to trapezoidal waveform."));
      ...
      {
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "APM6828 reprogramming ring registers to sinusoidal waveform."));
         XDRV_LOG_INFO(( XDRV_LOG_MOD_XDRV, "Frequency %d, Voltage %d, Offset %d, Type %d",
                         ringFrequency, ringVoltage, ringOffset, ringWaveshape ));
...
*****************************************************************************
*/
static void apm6828GetRingParms( XDRV_APM *pDrv, int *ringFrequency, int *ringWaveshape,
                                 int *ringVoltage, int *ringOffset, int *ringOffsetCal )
{
   APM_6828_DRV *pDev  = (APM_6828_DRV *)pDrv;
...
**
** PARAMETERS: pApm   - pointer to 6828's APM module
...
static int WriteApmFilterCoeffs( volatile BCM6828_APM *pApm, int chan, APM6828_FILTER_TYPE filter, int *data, int length )
{
   ...
   if (( chan < 0 || chan > APM6828_NUM_CHANNELS ) && ( chan != APM6828_ALL_CHANNELS ))
   {
   ...
   switch (filter)
   {
      case APM6828_FILTER_TX_EQ:
      {
         if( chan == APM6828_ALL_CHANNELS )
         {
      ...
      case APM6828_FILTER_HISPD_HYBRID_BAL:
      {
         if( chan == APM6828_ALL_CHANNELS )
         {
      ...
      case APM6828_FILTER_LOSPD_HYBRID_BAL:
      {
         if( chan == APM6828_ALL_CHANNELS )
         {
...
static XDRV_SINT64 divRound( XDRV_SINT64 dividend, XDRV_SINT64 divisor )
{
   ...
   if( divisor == 0 )
   {
      XDRV_LOG_ERROR(( XDRV_LOG_MOD_XDRV, "ERROR: APM6828 - Divisor of zero entered!"));
      return( 1 );
   }
...
static XDRV_SINT64 apmAsin(XDRV_SINT32 value)
{
   ...
   if( value<apmAsinTable[0].value || value> apmAsinTable[sizeof(apmAsinTable)/sizeof(APM6828_ASIN)-1].value)
   {
     return 0;
   }
