...
****************************************************************************
*
*    Description: HVG (high voltage generateor) device driver for BCM6828
*
****************************************************************************/
...
#include "hvg6828.h"
#include "apm6828.h"
#include <hvg6828Cfg.h>

typedef struct BCM6828_HVG_CUSTOM
{
   /* HVG Shared Settings */
...
/*
** This is the 6828 custom register table with values customized for
** specific HW layouts with specific SLICs.
*/
static BCM6828_HVG_CUSTOM regCustomTable[XDRV_APM_HVG_DESIGN_NUM_TYPES] =
{
...
static int hvg6828RegSetup( XDRV_APM *pApmDrv );
static int hvg6828UpdateHvgParms( XDRV_APM *pApmDrv, int chan );
static XDRV_APM_HVG_VOLTAGE hvg6828GetVoltage( XDRV_UINT16 mode, XDRV_UINT16 hookState, int battery );
static XDRV_SINT32 hvgExtend21to32( XDRV_UINT32 value );

/*****************************************************************************
**
**  FUNCTION:   hvg6828RegSetup
**
...
static int hvg6828RegSetup( XDRV_APM *pApmDrv )
{
   APM_6828_DRV *pDev = (APM_6828_DRV *)pApmDrv;
   volatile BCM6828_HVG *hvgp = BCM6828_HVG_PTR;
   ...
   {
      hvgp->reg_hvg_scale_vinput  = HVG_6828_INPUT_SCALE_12V;  /* Reg 2 - Lower 16 Bits */
      hvgp->reg_hvg_vbat          = HVG_6828_VBAT_SCALE_12V;   /* Reg 12 */
   }
   else
   {
      hvgp->reg_hvg_scale_vinput  = HVG_6828_INPUT_SCALE_15V;  /* Reg 2 - Lower 16 Bits */
      hvgp->reg_hvg_vbat          = HVG_6828_VBAT_SCALE_15V;   /* Reg 12 */
   }
...
**
*****************************************************************************/
int hvg6828Init( XDRV_APM *pApmDrv )
{
   volatile BCM6828_HVG *hvgp = BCM6828_HVG_PTR;
   APM_6828_DRV *pDev = (APM_6828_DRV *)pApmDrv;
   int i;
...
*****************************************************************************
*/
int hvg6828SetVoltage( XDRV_APM *pApmDrv, int chan, int voltage )
{
   volatile BCM6828_HVG *hvgp = BCM6828_HVG_PTR;
   APM_6828_DRV *pDev = (APM_6828_DRV *)pApmDrv;
...
/*
*****************************************************************************
** FUNCTION:   hvg6828UpdateSlicStatus
**
...
*****************************************************************************
*/
int hvg6828UpdateSlicStatus( XDRV_APM *pApmDrv, int chan, int slicState,
                             ...
                             XDRV_BOOL ringStop )
{
   ...
   const APM6828_FLEXICALC_CFG *flexicalcp = pDev->Cfg.pFlexiCalc;
   ...
   index = chan % APM6828_NUM_CHANNELS;
   ...
   voltage = hvg6828GetVoltage( pDev->slicState[index], pDev->hookState[index], pDev->battery[index] );
   /* Set the desired battery voltage */
   hvg6828SetVoltage( pApmDrv, chan, voltage );
   /* Update HVG switching parameters */
   hvg6828UpdateHvgParms( pApmDrv, chan );
   /* Platform specific settings */
...
/*
*****************************************************************************
** FUNCTION:   hvg6828UpdateHookStatus
**
...
*****************************************************************************
*/
int hvg6828UpdateHookStatus( XDRV_APM *pApmDrv, int chan, int hookState )
{
   APM_6828_DRV *pDev = (APM_6828_DRV *)pApmDrv;
   XDRV_APM_HVG_VOLTAGE voltage;
   int index;
...
/*
*****************************************************************************
** FUNCTION:   hvg6828UpdateHvgParms
**
...
/*
*****************************************************************************
** FUNCTION:   hvg6828GetOverCurrentStatus
**
...
*****************************************************************************
*/
XDRV_BOOL hvg6828GetOverCurrentStatus( XDRV_APM *pDrv, int chan )
{
   volatile BCM6828_HVG *hvgp = BCM6828_HVG_PTR;
...
/*
*****************************************************************************
** FUNCTION:   hvg6828GetOverVoltageStatus
**
...
*****************************************************************************
*/
void hvg6828GetOverVoltageStatus( XDRV_APM *pDrv, int chan )
{
   /* Nothing to do here */
   return;
}
...
/*
*****************************************************************************
** FUNCTION:   hvg6828UpdateHookStatusEnable
**
...
*****************************************************************************
*/
int hvg6828UpdateHookStatusEnable( XDRV_APM *pApmDrv, int chan, XDRV_BOOL update )
{
   APM_6828_DRV *pDev = (APM_6828_DRV *)pApmDrv;
   int index;
