...
unsigned int mspi6828Isr( XDRV_INT_CTRL_ISR_PARM contextp )
{
   ...
   */
   ...
   if( !( pMspiRegs->spiIntStatus & MMR6828_MSPI_INTSTATUS_CMNDDONE ))
   {
      XCHG_ASSERT( 0 );
   }
   ...
   */
   pMspiRegs->spiIntStatus |= MMR6828_MSPI_INTSTATUS_MASK;
   ...
   */
   ...
   else
   {
      ...
      */
      if( pMspiDrv->bytesRemaining >= ( BCM6828_MSPI_TX_QUEUE_SIZE - 2 ))
      {
         numBytesToXfer = ( BCM6828_MSPI_TX_QUEUE_SIZE - 2 );
      }
      else
      {
      ...
      */
      ...
      pMspiRegs->spiIntStatus |= MMR6828_MSPI_INTSTATUS_MASK;
      pMspiRegs->spiCmd |= MMR6828_MSPI_START_IMMEDIATE;
   }
