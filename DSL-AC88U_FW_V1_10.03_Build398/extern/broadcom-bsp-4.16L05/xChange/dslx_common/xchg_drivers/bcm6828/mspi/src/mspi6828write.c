                   ...
                   unsigned *pNumTxfr )
{
   ...
   if( length > MSPI6828_MAX_RX_SIZE )
   {
   ...
   if( pMspiRegs->spiIntMask != 0x00 )
   {
      if( bosSemTimedTake( &mspi6828IsrSem, 100 ))
      {
   ...
   */
   ...
   while( bytesRemaining > 0 )
   {
      ...
      */
      if( bytesRemaining >= ( BCM6828_MSPI_TX_QUEUE_SIZE - 2 ))
      {
      ...
      */
...
#endif /* MSPI6828_USE_INTERRUPTS */
      {
        /* Clear MSPI finished flags and begin transmission */
         ...
         while(!(pMspiRegs->spiIntStatus & MMR6828_MSPI_INTSTATUS_CMNDDONE));
         /*
         ** Read out rx data
         */
         for( indx=0; indx < numBytesToXfer; indx++ )
