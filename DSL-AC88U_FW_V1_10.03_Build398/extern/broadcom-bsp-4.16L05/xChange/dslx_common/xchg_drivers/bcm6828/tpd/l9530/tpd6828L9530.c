...
*      SLIC on the 6828 reference design
*
****************************************************************************/
...
#define BMU6828_MAX_CURRENT              32000
#define BMU6828_MIN_CURRENT             -32000
...
void linetestSelfHvg(int chan, XDRV_SINT32 *selfHvgPassed);
void linetestSelfApm(int chan, XDRV_SINT32 *selfApmPassed);
void linetestSelfCap(int chan, XDRV_SINT32 *selfCap);
void linetestSelfRing(int chan, XDRV_SINT32 *selfRing);
void linetestSelfFeed(int chan, XDRV_SINT32 *voltDc, XDRV_SINT32 *currentDc);

XDRV_UINT16 linetestDropRingMeasure(int chan, XDRV_SINT32 *voltageAc, XDRV_SINT32 *currentAc);
...
XDRV_SINT16 linetestReadSignal( int chan, XDRV_UINT32 measureType, TPD_MEASURE_RESULTS *tpdResults )
{
   APM_6828_DRV *pDev = (APM_6828_DRV *)pApmDriver[chan];
   ...
   if( chan == 0 )
   {
      ...
      /* Utilize audio ADC for current measurements on 3380 parts */
...
static XDRV_SINT16 linetestReadSignalSettle( int chan, XDRV_UINT32 measureType, TPD_MEASURE_RESULTS *tpdResults, XDRV_BOOL voltage )
{
   ...
   previousValue = BMU6828_MAX_CURRENT;
   ...
   while( delta >= threshold )
   {
      ...
      XDRV_LOG_DBG(( XDRV_LOG_MOD_XDRV, "[%lu]: tipVoltDC=%.8ld, iLoopDC=%.8ld", 
         attempts, tpdResults->tipVoltDC, tpdResults->iLoopDC ));
...
XDRV_UINT16 linetestDropResFlt(int chan, HSZDROPRSLT_RFLT *rfltRsltp )
{
   ...
   IMTfh = BMU6828_MAX_CURRENT;
   IMTrh = BMU6828_MAX_CURRENT;
   IMTfl = BMU6828_MAX_CURRENT;
   IMTrl = BMU6828_MAX_CURRENT;
   VToh = BMU6828_MAX_CURRENT;
   VTol = BMU6828_MAX_CURRENT;
   ...
   linetestReadSignalSettle( chan, testMode, &tpdResults, XDRV_FALSE );
   ...
   /* Wait for current to settle due to capacitive loading */
   linetestReadSignalSettle( chan, testMode, &tpdResults, XDRV_TRUE );
   ...
   Rdiv = ( 2 * ( XDRV_SINT64 )( IMTrh - IMTrl + 0 ) * ( XDRV_SINT64 )(( VRol - VRoh ) - ( VTol - VToh )));
   if ( Rdiv == 0 )
   {
      Rdiv = 1;
   }
   ...
   Rdiv = ( 2 * ( XDRV_SINT64 )( IMTrh - IMTrl + 0 ) * apmDiv(Rtg, 1000) - ( XDRV_SINT64 )( VTrh - VTrl ));
   if ( Rdiv == 0 )
   {
      Rdiv = 1;
   }
   ...
   Rdiv = ( 2 * ( XDRV_SINT64 )( IMTfh - IMTfl + 4 ) * apmDiv(Rtr, 1000) - 2 * ( XDRV_SINT64 )( VRfl - VRfh ));
   if ( Rdiv == 0 )
   {
      Rdiv = 1;
   }
   ...
   if(( IMTrh < BMU6828_MIN_CURRENT ) &&
      ( IMTfh > BMU6828_MAX_CURRENT ))
   {
      /* Calculate tip-ring impedance, including offset adjustment */
      Rdiv = ( 2 * ( XDRV_SINT64 )( IMTrh - IMTrl ) * apmDiv(Rtg, 1000) - ( XDRV_SINT64 )(( VTrh - VTrl ) - ( VRrh - VRrl )));
      Rtr = apmDiv(( 2 * ( XDRV_SINT64 )(( VTrh - VTrl ) - ( VRrh - VRrl )) * Rtg ), Rdiv);
      Rtg = 1000000;
      Rrg = 1000000;
   }
   else if (( abs(IMTrl) > ( BMU6828_MAX_CURRENT / 2 ) ) &&
            ( abs(IMTrh) > ( BMU6828_MAX_CURRENT ) ) &&
            ( abs(IMTol) < ( BMU6828_MAX_CURRENT / 200 ) ) &&
            ( abs(IMToh) < ( BMU6828_MAX_CURRENT / 200 ) ) &&
            ( IMTfl < 0 ))
   {
      Rrg = 1000000;
      Rtr = 1000000;
   ...
   else if (( abs(IMTfl) > ( BMU6828_MAX_CURRENT * 5 / 8 ) ) &&
            ( abs(IMTfh) > ( BMU6828_MAX_CURRENT ) ) &&
            ( abs(IMTol) > ( BMU6828_MAX_CURRENT * 3 / 8 ) ) &&
            ( abs(IMToh) > ( BMU6828_MAX_CURRENT / 2 ) ))
   {
      Rtg = 1000000;
      Rtr = 1000000;
      Rrg = 0;
...
XDRV_UINT16 linetestDropRing(int chan, HSZDROPRSLT_RNG *rngRsltp)
{
   ...
   linetestDropRingMeasure(chan, &tipRingVoltAC, &AcCurrent);
...
/*
*****************************************************************************
** FUNCTION:   linetestDropRingMeasure
**
** PURPOSE:    Helper method takes SLIC through mode changes and samples APM
**             for underlying measurements.
**
** PARAMETERS: chan    - channel to measure
...
XDRV_UINT16 linetestDropRingMeasure(int chan, XDRV_SINT32 *voltageAc, XDRV_SINT32 *currentAc)
{
   ...
   /* Save desired results */
   *voltageAc = tpdResults.tipRingVoltAC;
   *currentAc = tpdResults.iLoopAC;

   XDRV_LOG_DBG(( XDRV_LOG_MOD_XDRV, "****************************************************************** "));
   XDRV_LOG_DBG(( XDRV_LOG_MOD_XDRV, "Summary of RING readings ... "));

   XDRV_LOG_DBG(( XDRV_LOG_MOD_XDRV, "ring: trVac = %.8ld ", *voltageAc ));
   XDRV_LOG_DBG(( XDRV_LOG_MOD_XDRV, "ring: Iac  = %.8ld ", *currentAc ));
...
**             selfHvgPassed - pointer to results structure
**
** RETURNS: Nothing
*****************************************************************************
*/
...
**
** PARAMETERS: chan          - channel under test
**             selfApmPassed - pointer to results structure
**
** RETURNS: Nothing
*****************************************************************************
*/
...
void linetestSelfRing(int chan, XDRV_SINT32 *selfRing)
{
   XDRV_SINT32 tipRingVoltAc, currentAc;

   linetestDropRingMeasure(chan, &tipRingVoltAc, &currentAc);

   /* Should already be in mVAC */
   *selfRing = tipRingVoltAc;
}
...
*****************************************************************************
*/
int tpd6828InitChannel( int chan, XDRV_SLIC *slicDrv, XDRV_APM *apmDrv, XDRV_DEBUG *debugDrv )
{
