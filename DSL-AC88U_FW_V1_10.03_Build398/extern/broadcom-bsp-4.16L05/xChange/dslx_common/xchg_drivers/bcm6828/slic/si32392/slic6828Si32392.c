...
#include "slic6828Si32392.h"
#include "slic6828Si32392Defs.h"
...
static void slic6828Si32392ModeControl( XDRV_SLIC *pDrv, XDRV_SLIC_MODE mode );
static void slic6828Si32392LedControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static void slic6828Si32392PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slic6828Si32392IsOffhook( XDRV_SLIC *pDrv );
static int slic6828Si32392SetRingParms( XDRV_SLIC *pDrv, int ringFrequency, int ringWaveshape,
                                       int ringVoltage, int ringOffset, int ringOffsetCal );
static int slic6828Si32392GetRingParms( XDRV_SLIC *pDrv, int *ringFrequency, int *ringWaveshape,
                                       int *ringVoltage, int *ringOffset, int *ringOffsetCal );
static int slic6828Si32392SetBoostedLoopCurrent( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static int slic6828Si32392SetPowerSource( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slic6828Si32392GetOverCurrentStatus( XDRV_SLIC *pDrv );
static XDRV_SINT16 slic6828Si32392GetDlp( XDRV_SLIC *pDrv );
static void slic6828Si32392ProcessEvents( XDRV_SLIC *pDrv );
static XDRV_UINT32 slic6828Si32392Probe( XDRV_SLIC *pDrv, XDRV_UINT32 deviceId, XDRV_UINT32 chan, XDRV_UINT32 reg,
                                      XDRV_UINT32 regSize, void* value, XDRV_UINT32 valueSize, XDRV_UINT32 indirect, 
                                      XDRV_UINT8 set );
static int slic6828Si32392GetSlicParms( XDRV_SLIC *pDrv, int *phaseReversal, int *loopCurrent, int *powerSource, int *slicMode );
/*
...
{
   { SI32392_STATE_DISCONNECT,    XDRV_APM_HVG_VOLTAGE_ONHOOK  },
   { SI32392_STATE_STANDBY,       XDRV_APM_HVG_VOLTAGE_ONHOOK  },
   { SI32392_STATE_FWD_ACTIVE,    XDRV_APM_HVG_VOLTAGE_OFFHOOK  },
   { SI32392_STATE_REV_ACTIVE,    XDRV_APM_HVG_VOLTAGE_OFFHOOK  },
   { SI32392_STATE_RING_OPEN,     XDRV_APM_HVG_VOLTAGE_ONHOOK  },
   { SI32392_STATE_TIP_OPEN,      XDRV_APM_HVG_VOLTAGE_ONHOOK },
   { SI32392_STATE_AC_RING_TRIP,  XDRV_APM_HVG_VOLTAGE_RING    },
   { SI32392_STATE_DC_RING_TRIP,  XDRV_APM_HVG_VOLTAGE_RING    },
   { SI32392_STATE_NULL,          XDRV_APM_HVG_VOLTAGE_NONE    }
}; 

/* SLIC "meta-driver" interface */
static const XDRV_SLIC_FUNCS slic6828Si32392DrvFuncs =
{
   slic6828Si32392ModeControl,
   slic6828Si32392LedControl,
   slic6828Si32392PhaseReversalControl,
   slic6828Si32392IsOffhook,
   slic6828Si32392SetRingParms,
   slic6828Si32392GetRingParms,
   slic6828Si32392SetBoostedLoopCurrent,
   slic6828Si32392SetPowerSource,
   slic6828Si32392GetOverCurrentStatus,
   slic6828Si32392GetDlp,
   slic6828Si32392ProcessEvents,
   slic6828Si32392Probe,
   slic6828Si32392GetSlicParms
};

/* Private Functions */
static void OpenSlic( SLIC_6828_SI32392_DRV *pDev );
static void CloseSlic( SLIC_6828_SI32392_DRV *pDev );
static void InitSlicRegs( SLIC_6828_SI32392_DRV *pDev );
...
*****************************************************************************
*/
void slic6828Si32392Init
(
   int                   chan,
   SLIC_6828_SI32392_DRV *pDev,
   const SLICCFG_6828_SI32392_INFO *pSlicCfgInfo,
   XDRV_APM             *pApmDrv
)
{
   int status;
   ...
   memset( pDev, 0, sizeof( SLIC_6828_SI32392_DRV ) );

   /* Initialize the SLIC information for each channel */
   pDev->chan = chan;
   pDev->pDrvFuncs = &slic6828Si32392DrvFuncs;
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392Deinit
**
...
*****************************************************************************
*/
void slic6828Si32392Deinit( SLIC_6828_SI32392_DRV *pDev )
{
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392IsOffhook
**
...
static XDRV_BOOL slic6828Si32392IsOffhook( XDRV_SLIC *pDrv )
{
   SLIC_6828_SI32392_DRV *pDev = (SLIC_6828_SI32392_DRV *)pDrv;
   int                   chan = pDev->chan;
   XDRV_UINT16           rc;
   ...
   rc = MSPI_Read( chan, SI32392_REG_DEVICE_STATUS );

   if ( rc & (( chan == 0 ) ? SI32392_CHAN0_HOOKSWITCH_STAT_OFF:SI32392_CHAN1_HOOKSWITCH_STAT_OFF ) )
   {
      return ( XDRV_TRUE );
   }
   else
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392ModeControl
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392LedControl
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392PhaseReversalControl
**
...
static void slic6828Si32392PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value )
{
   ...
   {
      slic6828Si32392ModeControl( pDrv, pDev->slicMode );
   }
}

/*
*****************************************************************************
** FUNCTION:   slic6828Si32392SetRingParms
**
                                      ...
                                      int ringVoltage, int ringOffset, int ringOffsetCal )
{
   ...
   slicControlReg = MSPI_Read( pDev->chan, SI32392_REG_LINEFEED_CONFIG_2 );
   slicControlReg &= ~SI32392_RSPOL_MASK;
   if ( ringOffset < 0 )
   {
      slicControlReg |= SI32392_RSPOL_REVERSED;
   }
   MSPI_Write( pDev->chan, SI32392_REG_LINEFEED_CONFIG_2, slicControlReg );
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392GetRingParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392GetSlicParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392SetBoostedLoopCurrent
**
...
static int slic6828Si32392SetBoostedLoopCurrent( XDRV_SLIC *pDrv, XDRV_UINT16 value )
{
   ...
   */
   if ( value )
   {
      MSPI_Mask_Write( chan, SI32392_REG_LINEFEED_CONFIG_2,
...
static void OpenSlic( SLIC_6828_SI32392_DRV *pDev )
{
   ...
   MSPI_Mask_Write( chan, SI32392_REG_LINEFEED_CONFIG_1,
...
static void InitSlicRegs( SLIC_6828_SI32392_DRV *pDev )
{
   ...
   MSPI_Write( chan, SI32392_REG_DEVICE_STATUS, 0x0000 );
   ...
   */
   if(( pDev->chan % 2 ) == 0 )
   {
      MSPI_Write( chan, SI32392_REG_LINEFEED_CONFIG_1, 0x0000 );
   }
   MSPI_Write( chan, SI32392_REG_LINEFEED_CONFIG_2, SI32392_RINGTRIP_DETECT_THRESH_70 );
   MSPI_Write( chan, SI32392_REG_LINEFEED_CONFIG_3, 0x0000 );
}
...
static void setSlicState( int chan, XDRV_SLIC_MODE mode )
{
   ...
   /* Retrieve old data */
   slicStateReg = MSPI_Read( chan, SI32392_REG_LINEFEED_CONFIG_1 );
   slicControlReg = MSPI_Read( chan, SI32392_REG_LINEFEED_CONFIG_3 );
   ...
   MSPI_Write( chan, SI32392_REG_LINEFEED_CONFIG_1, slicStateReg );

   /* If necessary update the temperature control data to the SLIC */
   if( requiredTemp != currentTemp )
   {
      MSPI_Write( chan, SI32392_REG_LINEFEED_CONFIG_3, slicControlReg );
   }
}
#endif 
/*
...
/*
******************************************************************************
** FUNCTION:  slic6828Si32392RegisterRead
**
** PURPOSE:   Read from a memory address via MSPI
**
** PARAMETERS:
...
*****************************************************************************
*/
void slic6828Si32392RegisterRead( int chan, XDRV_UINT8 addr, XDRV_UINT16 *data )
{
...
/*
******************************************************************************
** FUNCTION:  slic6828Si32392RegisterWrite
**
** PURPOSE:   Write to a memory address via MSPI
**
** PARAMETERS:
...
*****************************************************************************
*/
void slic6828Si32392RegisterWrite( int chan, XDRV_UINT8 addr, XDRV_UINT16 data )
{
...
/*
******************************************************************************
** FUNCTION:  slic6828Si32392RegisterMaskWrite
**
** PURPOSE:   Write to the masked location of a memory address via MSPI
**            and verify the write if requested to
**
...
*****************************************************************************
*/
void slic6828Si32392RegisterMaskWrite( int chan, XDRV_UINT8 addr, XDRV_UINT16 data, XDRV_UINT16 mask, int verify )
{
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392SetPowerSource
**
...
static int slic6828Si32392SetPowerSource( XDRV_SLIC *pDrv, XDRV_UINT16 value )
{
   ...
   if ( pDev->bEnhancedControl )
   {
      if ( value & 0xF0 )
      {
         configRegister = MSPI_Read( chan, SI32392_REG_LINEFEED_CONFIG_2 );
         configRegister |= ( SI32392_CHAN0_ENHANCED_CONTROL_ON | SI32392_CHAN1_ENHANCED_CONTROL_ON );
         MSPI_Write( chan, SI32392_REG_LINEFEED_CONFIG_2, configRegister );
      }
      else if ( value == 0 )
      {
         configRegister = MSPI_Read( chan, SI32392_REG_LINEFEED_CONFIG_2 );
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392GetOverCurrentStatus
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392SetSpecialMode
**
...
*****************************************************************************
*/
int slic6828Si32392SetSpecialMode( SLIC_6828_SI32392_DRV *pDev, SLIC_6828_SI32392_SPECIAL_MODE mode, XDRV_BOOL enable )
{
   switch( mode )
   {
      case SLIC_6828_SI32392_SPECIAL_DISABLE_RING_TRANISTION_MODE:
      {
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si32392UpdateState
**
...
*****************************************************************************
*/
void slic6828Si32392UpdateState( SLIC_6828_SI32392_DRV *pDev )
{
   if( pDev->pendingSlicTime > 0 )
   {
