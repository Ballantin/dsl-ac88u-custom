...
#include "slic6828Si3239.h"
#include "slic6828Si3239Defs.h"
...
static void slic6828Si3239ModeControl( XDRV_SLIC *pDrv, XDRV_SLIC_MODE mode );
static void slic6828Si3239LedControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static void slic6828Si3239PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slic6828Si3239IsOffhook( XDRV_SLIC *pDrv );
static int slic6828Si3239SetRingParms( XDRV_SLIC *pDrv, int ringFrequency, int ringWaveshape,
                                       int ringVoltage, int ringOffset, int ringOffsetCal );
static int slic6828Si3239GetRingParms( XDRV_SLIC *pDrv, int *ringFrequency, int *ringWaveshape,
                                       int *ringVoltage, int *ringOffset, int *ringOffsetCal );
static int slic6828Si3239SetBoostedLoopCurrent( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static int slic6828Si3239SetPowerSource( XDRV_SLIC *pDrv, XDRV_UINT16 value );
static XDRV_BOOL slic6828Si3239GetOverCurrentStatus( XDRV_SLIC *pDrv );
static XDRV_SINT16 slic6828Si3239GetDlp( XDRV_SLIC *pDrv );
static void slic6828Si3239ProcessEvents( XDRV_SLIC *pDrv );
static XDRV_UINT32 slic6828Si3239Probe( XDRV_SLIC *pDrv, XDRV_UINT32 deviceId, XDRV_UINT32 chan, XDRV_UINT32 reg,
                                      XDRV_UINT32 regSize, void* value, XDRV_UINT32 valueSize, XDRV_UINT32 indirect, 
                                      XDRV_UINT8 set );
static int slic6828Si3239GetSlicParms( XDRV_SLIC *pDrv, int *phaseReversal, int *loopCurrent, int *powerSource, int *slicMode );
/*
...
static const XDRV_SLIC_FUNCS slic6828Si3239DrvFuncs =
{
   slic6828Si3239ModeControl,
   slic6828Si3239LedControl,
   slic6828Si3239PhaseReversalControl,
   slic6828Si3239IsOffhook,
   slic6828Si3239SetRingParms,
   slic6828Si3239GetRingParms,
   slic6828Si3239SetBoostedLoopCurrent,
   slic6828Si3239SetPowerSource,
   slic6828Si3239GetOverCurrentStatus,
   slic6828Si3239GetDlp,
   slic6828Si3239ProcessEvents,
   slic6828Si3239Probe,
   slic6828Si3239GetSlicParms
};

/* Private Functions */
static void OpenSlic( SLIC_6828_SI3239_DRV *pDev );
static void CloseSlic( SLIC_6828_SI3239_DRV *pDev );
static void InitSlicRegs( SLIC_6828_SI3239_DRV *pDev );
...
)
{
   ...
   memset( pDev, 0, sizeof( SLIC_6828_SI3239_DRV ) );

   /* Initialize the SLIC information for each channel */
   pDev->chan = chan;
   pDev->pDrvFuncs = &slic6828Si3239DrvFuncs;
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239IsOffhook
**
...
static XDRV_BOOL slic6828Si3239IsOffhook( XDRV_SLIC *pDrv )
{
   SLIC_6828_SI3239_DRV *pDev = (SLIC_6828_SI3239_DRV *)pDrv;
   int                   chan = pDev->chan;
   XDRV_UINT16           rc;
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239ModeControl
**
...
static void slic6828Si3239ModeControl( XDRV_SLIC *pDrv, XDRV_SLIC_MODE mode )
{
   ...
   {
      /* Apply signal when switching into STANDBY mode.
      ** Revert change upon exiting from STANDBY mode.
      */
#if 0 
      if (( mode == XDRV_SLIC_MODE_STANDBY ) || ( pDev->slicMode == XDRV_SLIC_MODE_STANDBY ))
      {
         ITPC_SHARED_APM_DMA_SIGNAL_CONFIG itpcSignalConfig;
         itpcSignalConfig.chanNum = chan;
         itpcSignalConfig.bOverwrite = ((mode == XDRV_SLIC_MODE_STANDBY)? XDRV_TRUE: XDRV_FALSE);
         xdrvItpcSendSyncCommand( pDev->pItpcDrv, ITPC_APM_DMA_OVERWRITE_CMD_ID, &itpcSignalConfig , NULL );
      }
#endif      
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239LedControl
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239PhaseReversalControl
**
...
static void slic6828Si3239PhaseReversalControl( XDRV_SLIC *pDrv, XDRV_UINT16 value )
{
   ...
   {
      slic6828Si3239ModeControl( pDrv, pDev->slicMode );
   }
}

/*
*****************************************************************************
** FUNCTION:   slic6828Si3239SetRingParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239GetRingParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239GetSlicParms
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239SetBoostedLoopCurrent
**
...
static void InitSlicRegs( SLIC_6828_SI3239_DRV *pDev )
{
   ...
   */
   ...
   MSPI_Write( chan, SI3239_REG_LINEFEED_CONFIG_3, 0x0000 );
}
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239SetPowerSource
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239GetOverCurrentStatus
**
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239SetSpecialMode
**
...
*****************************************************************************
*/
int slic6828Si3239SetSpecialMode( SLIC_6828_SI3239_DRV *pDev, SLIC_6828_SI3239_SPECIAL_MODE mode, XDRV_BOOL enable )
{
   switch( mode )
   {
      case SLIC_6828_SI3239_SPECIAL_DISABLE_RING_TRANISTION_MODE:
      {
...
/*
*****************************************************************************
** FUNCTION:   slic6828Si3239UpdateState
**
...
*****************************************************************************
*/
void slic6828Si3239UpdateState( SLIC_6828_SI3239_DRV *pDev )
{
   if( pDev->pendingSlicTime > 0 )
   {
      ...
      else
      {
         ...
         ** Revert change upon exiting from STANDBY mode.
         */
         if (( pDev->pendingSlicMode == XDRV_SLIC_MODE_STANDBY ) || ( pDev->slicMode == XDRV_SLIC_MODE_STANDBY ))
         {
            ITPC_SHARED_APM_DMA_SIGNAL_CONFIG itpcSignalConfig;
            itpcSignalConfig.chanNum = pDev->chan;
            itpcSignalConfig.bOverwrite = ((pDev->pendingSlicMode == XDRV_SLIC_MODE_STANDBY)? XDRV_TRUE: XDRV_FALSE);
            xdrvItpcSendSyncCommand( pDev->pItpcDrv, ITPC_APM_DMA_OVERWRITE_CMD_ID, &itpcSignalConfig , NULL );
         }
#endif
