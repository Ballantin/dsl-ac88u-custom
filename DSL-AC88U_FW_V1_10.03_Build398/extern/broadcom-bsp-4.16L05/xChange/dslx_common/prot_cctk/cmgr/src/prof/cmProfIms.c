...
**      related processings for the specific IMS profile.
**
***********************************************************************************/
...
static void cmProfImsMultilineUserAdd ( void *pRes );
#else
static void cmProfImsDefaultUserAdd ( void *pRes );
...
/***********************************************************************************
** FUNCTION:   cmProfImsUserAdd
**
** PURPOSE:    Configure a specific user for an IMS application.
**
...
**             completely.
***********************************************************************************/
void cmProfImsUserAdd ( void *pRes )
{
#if CMGRCFG_MULTILINE
   cmProfImsMultilineUserAdd( pRes );
#else
   cmProfImsDefaultUserAdd( pRes );
#endif /* CMGRCFG_MULTILINE */
}

#if !CMGRCFG_MULTILINE
/***********************************************************************************
** FUNCTION:   cmProfImsDefaultUserAdd
**
** PURPOSE:    Configure a specific user for an IMS application.
**
** PARAMETERS: pRes - the user resource to configure.
      ...
      if ( pUsr )
      {
         ...
         */
                                    ...
                                    CFG_ITEM_GET ) == BOS_STATUS_OK )
         {
            ...
            */
            ...
            else if ( pPcscfUsr->pcPCSCFDomain )
            {
               cctkUsr.usr.net.obProxyLst.route[0].addr.hostPort.pcAddr =
                                                          pPcscfUsr->pcPCSCFDomain;
            }
            else
            {
         ...
         */
         ...
         if ( pPcscfUsr && pPcscfUsr->bBackupRecovery )
         {
            ...
            */
            CMGRLOG_INFORMATION ((
                        "cmProfImsDefaultUserAdd: %s - usr 0x%X, uses backup-recovery",
         ...
         */
         CMGRLOG_INFORMATION ((
            "cmProfImsDefaultUserAdd: %s - usr 0x%X, no outbound - remote user support",
            pCmUsr->uUid, pCmUsr->uHdl ));
#else
         ...
         */
         ...
         {
            CMGRLOG_CRITICAL (( "cmProfImsDefaultUserAdd: %s - failed to get digit map!",
                                pCmUsr->uUid ));
         }

         CMGRLOG_INFORMATION (( "cmProfImsDefaultUserAdd: %s - usr 0x%X, net 0x%X, dm %d",
                                pCmUsr->uUid, pCmUsr->uHdl,
...
#if CMGRCFG_MULTILINE
/***********************************************************************************
** FUNCTION:   cmProfImsMultilineUserAdd
**
** PURPOSE:    Configure a specific user for a multiline enabled ims application.
**
** PARAMETERS: pRes - the user resource to configure.
**
** RETURNS:    Nothing.
...
static void cmProfImsMultilineUserAdd ( void *pRes )
{
   ...
   if ( !pCmUsr )
   {
      CMGRLOG_CRITICAL (( "cmProfImsMultilineUserAdd: pRes is NULL!" ));
      return;
   }
...
/***********************************************************************************
** FUNCTION:   cmProfImsCctkAccntAdd
**
** PURPOSE:    Configure a specific CCTK account for a multiline and IMS
**             application.
**
...
**             completely.
***********************************************************************************/
BOS_BOOL cmProfImsCctkAccntAdd ( void *pRes )
{
   CFGPROVDEVICECFG *pDevCfg = NULL;
   CFGSIPUSERCFG *pUsr = NULL;
   PROV_CFG_IDX cfgIdx;
      ...
      if ( pUsr )
      {
         ...
         ** FORBIDDEN state*/
         ...
         gCmCfgCb.cmgrProvCb ( CFGITEM( SIP, UserConcept ),
                               ...
                               (void *) pUsr,
                               CFG_ITEM_SET );
         /* Restored to original active domain, so it s deleted to avoid memory leak. */
         pUsr->pcActiveDomain = pcOrgActiveDomain;
...
#if CMGRCFG_VIDEO_MEDIA
         cmCfgVideo ( (void *) pCctkAccnt, &cctkMedVid );
#endif
         ...
         */
         ...
         if ( pPcscfUsr && pPcscfUsr->bBackupRecovery )
         {
            ...
            */
            CMGRLOG_INFORMATION ((
                        "cmProfImsCctkAccntAdd: %s - usr 0x%X, uses backup-recovery",
         ...
         */
         CMGRLOG_INFORMATION ((
            "cmProfImsCctkAccntAdd: %s - usr 0x%X, no outbound - remote user support",
            pCctkAccnt->uUid, pCctkAccnt->uHdl ));
#else
         /* Setup outbound keep alive configuration.  The outbound keep alive
         ** configuration ensures that we are always accessing the same target
                  ...
                  else if ( usrBrCfg.eMode == eCCTKBRSVCMODE_EXTLIST )
                  {
                     ...
                     for ( uIx = 0; uIx < pPcscfUsr->obProxyLst.uNum; uIx++ )
                     {
                        ...
                        */
                        ...
                        {
                           usrBrCfg.extList.ipport[usrBrCfg.extList.uNum].addr = obIpAddr;
                           usrBrCfg.extList.ipport[usrBrCfg.extList.uNum].uPort = pPcscfUsr->obProxyLst.obProxy[uIx].uPort;
                           usrBrCfg.extList.uNum++;
                        }
                        else
                        {
                           CMGRLOG_CRITICAL (( "cmProfImsCctkLineAdd: Failed to configure recovery ip list!" ));
                        }
         ...
         */
         ...
         if ( gCmCfgCb.cmgrProvCb ( CFGITEM( SIP, DigitMapConcept ),
                                    ...
                                    CFG_ITEM_GET ) == BOS_STATUS_OK )
         {
            dmStatus = cmDmCompile ( pCctkAccnt->uNet,
                                     pUsrDigMap->pABNF,
         ...
         {
            CMGRLOG_CRITICAL (( "cmProfImsCctkAccntAdd: %s - failed to get digit map!",
                                pCctkAccnt->uUid ));
         }

         CMGRLOG_INFORMATION (( "cmProfImsCctkAccntAdd: %s - usr 0x%X, net 0x%X, dm %d",
                                pCctkAccnt->uUid, pCctkAccnt->uHdl,
                                pCctkAccnt->uNet, dmStatus ));
...
#endif /* (CMGRCFG_CFG_PROFILE == CMGRCFG_CFG_PROFILE_IMS) */
