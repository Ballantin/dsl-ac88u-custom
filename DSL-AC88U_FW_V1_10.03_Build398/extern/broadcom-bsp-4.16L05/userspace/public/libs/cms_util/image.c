...
// ARCADYAN
/* Add by runsen_lao, 2014-12-19, reference by cmsImg_writeValidatedImageEx, 
 * only write the broadcom or flash type image, others are return error */
CmsRet arca_writeValidatedImageEx(char *imagePtr, UINT32 imageLen,
                                    CmsImageFormat format, UINT32 opts)
{
   CmsRet ret=CMSRET_SUCCESS;
   int noReboot = 0;
   ...
   case CMS_IMAGE_FORMAT_BROADCOM:
   case CMS_IMAGE_FORMAT_FLASH:
      ret = flashImage(imagePtr, imageLen, format, opts, &noReboot);
      break;
   default:
       cmsLog_error("Unrecognized image format=%d", format);
       ret = CMSRET_INVALID_IMAGE;
...
/* Add by runsen_lao, 2014-12-19, reference by cmsImg_validateImage, 
 * remove the code which communicate with smd, like sendConfigMsg() */
CmsImageFormat arca_validateImage(const char *imageBuf, UINT32 imageLen)
{
   ...
   if ((imageLen > sizeof(FILE_TAG)) 
        && (verifyBroadcomFileTag((FILE_TAG *) imageBuf, 1, imageLen) == CMSRET_SUCCESS))
   {
      UINT32 maxLen;
