...
#define CFERAM_SIZE             (8 * eraseSize)
...
typedef struct
{
     ...
     * cferamFound - image includes cferam (must be true).
     ...
     * cferamP - saved local copy before flashing.
     * cferomP - saved local copy before flashing.
     */
    IMG_FORMAT_PARSER_CB fmtParserCb;
    CAL_CRC32_CB calCrc32Cb;
    UINT32 imgFormat;
    char cferamFname[CFERAM_NAME_LEN];
...
#define incSeqNum(seq) ((seq > 999) ? seq = 0 : seq++)
...
#define getFlashOffsetForCferam(s) (8 * s)
...
{
    static UBOOL8 mode = FALSE;
    static UBOOL8 mode_init = FALSE;
    UINT32 flashInfo;

    if (mode_init != TRUE)
    {
        flashInfo = getFlashType();

        if ((flashInfo & FLASH_INFO_FLAG_NAND) != 0)
        {
  ...
  CAL_CRC32_CB calCrc32Cb)
{
    ...
    mtdP = get_mtd_device_nm(MTD_NAME_IMAGE, &mtdFd);
    if (mtdP == 0)
    {
        imgif_error("get_mtd_device_nm(%s) failed", MTD_NAME_IMAGE);
        return (IMGIF_HANDLE)NULL;
    }

    mtdP = get_mtd_device_nm(MTD_NAME_IMAGE_UPDATE, &mtdFd);
    if (mtdP == NULL)
    {
        imgif_error("get_mtd_device_nm(%s) failed", MTD_NAME_IMAGE_UPDATE);
        return (IMGIF_HANDLE)NULL;
    }
    else
    {
...
int imgif_get_flash_info(IMGIF_HANDLE h, imgif_flash_info_t *flashInfoP)
{
    ...
    {
        flashInfoP->flashType = ctxP->flashInfo;
...
static imgif_ret_e imgif_ctx_init(imgif_ctx_t *ctxP)
{
    ...
    {
        imgif_error("malloc(%d) failed.", sizeof(NVRAM_DATA));
        goto init_exit;
    }
    ...
    strcpy(ctxP->cferamFname, getCferamName());
    seq = initSeqNum();
    if (seq < 0)
    {
...
void dumpImgifCtx(imgif_ctx_t *ctxP)
{
    ...
    printf("\tcferam: found=%d, saved=%p, seq=%d\n",
      ctxP->cferamFound, ctxP->cferamP, ctxP->cferamSeqNum);
...
static int flashNormalBlock(imgif_ctx_t *ctxP, UINT8 *bufP, UINT32 writeLen)
{
    ...
    if (nandWriteBlk(ctxP->mtdP, ctxP->flashBlkAddr, writeLen, (char*)bufP,
...
static imgif_ret_e checkCferom(imgif_ctx_t *ctxP)
{
    ...
    cferomExist = (*(UINT16*)ctxP->startP == JFFS2_MAGIC_BITMASK) ?
      FALSE : TRUE;
    ...
    ctxP->cferomP = malloc(ctxP->eraseSize);
    ...
    memcpy(ctxP->cferomP, ctxP->startP, ctxP->eraseSize);
...
static imgif_ret_e procData(imgif_ctx_t *ctxP)
{
    ...
    if (curState == IMGIF_UPGRADE_SEARCH)
    {
        ...
        if (ctxP->usrBlks == 0)
        {
            ...
            /* If exists, cferom is on the first block. */
        ...
        if ((ctxP->usrBlks > 0) || (cferomFound == FALSE))
        {
    ...
    if (moveOn == TRUE)
    {
        procBytes = getProcessedDataLen(ctxP);
        crc = crc_getCrc32(ctxP->startP, procBytes, ctxP->crcBody);
...
static imgif_ret_e procSearchCferam(imgif_ctx_t *ctxP)
{
    ...
    /* Check cferam header. */
    if ((ctxP->cferamFound != FALSE) || (ctxP->cferamP != NULL))
    {
        imgif_error("Invalid state, found=%d, cferamP=%p",
          ctxP->cferamFound, ctxP->cferamP);
        return IMGIF_PROC_ERR;
    }

    cferamP = searchCferamSeqNum(ctxP->cferamFname, (char*)ctxP->startP,
      ctxP->eraseSize, &done, &dirP);
    if (cferamP == NULL)
    {
    ...
    else
    {
        ...
        ctxP->cferamP = malloc(ctxP->cferamSize);
        if (ctxP->cferamP != NULL)
        {
            memcpy(ctxP->cferamP, cferamP, ctxP->cferamSize);
            ctxP->cferamFound = TRUE;
            ctxP->flashBlkCferam = getFlashOffsetForCferam(ctxP->eraseSize);
            ctxP->flashBlkAddr = ctxP->flashBlkCferam + ctxP->eraseSize;
            ctxP->cferamDirOffset = (UINT32)((UINT8*)dirP - ctxP->startP);
...
static imgif_ret_e procUpdateAndFlashCferam(imgif_ctx_t *ctxP)
{
    ...
    updateCferamSeqNum((char*)(ctxP->cferamP + ctxP->cferamDirOffset),
      ctxP->cferamSeqNum);
...
static imgif_ret_e procFlashCferom(imgif_ctx_t *ctxP)
{
    ...
    ret = flashCferom((char*)ctxP->cferomP,
      (ctxP->wfiFlags & WFI_FLAG_SUPPORTS_BTRM),
      ctxP->cferomSize, &cferom_offset);
    if (ret < 0)
    {
