...
function menuAdmin(options) {
   ...
   if ( parseInt(numWl) != 0 ) {
      ...
      if(wlVisualization == '1') {
	      if (numWl == '1') {
		      wlVisualNode = insFld(nodeWireless, gFld(getMenuTitle(MENU_WL_VISUALIZATION), "wlvis/configure.asp"));
	      } else {
		      wlVisualNode = insFld(wlanMenu, gFld(getMenuTitle(MENU_WL_VISUALIZATION), "wlvis/configure.asp"));
	      }
              insDoc(wlVisualNode, gLnk('R', getMenuTitle(MENU_WL_VISUALIZATION_SITESURVEY), 'wlvis/visindex.asp'));
              insDoc(wlVisualNode, gLnk('R', getMenuTitle(MENU_WL_VISUALIZATION_CHANNELSTAT), 'wlvis/channelcapacity.asp'));
              insDoc(wlVisualNode, gLnk('R', getMenuTitle(MENU_WL_VISUALIZATION_METRICS), 'wlvis/metrics.asp'));
              insDoc(wlVisualNode, gLnk('R', getMenuTitle(MENU_WL_VISUALIZATION_CONFIGURE), 'wlvis/configure.asp'));
      }
   ...
   // Configure DPI 
   if ( dpi == '1' && websockets == '1' && jqplot == '1') {
      nodeDpi = insFld(nodeAdvancedSetup, gFld(getMenuTitle(MENU_DPI), 'dpicharts.html'));
      insDoc(nodeDpi, gLnk('R', getMenuTitle(MENU_DPI_GRAPH),'dpicharts.html'));
      insDoc(nodeDpi, gLnk('R', getMenuTitle(MENU_DPI_APP_INST_TABLE),'dpiappinst.cmd?sort=true'));
   }
...
function createBcmMenu(options) {
   ...
   if (jqplot == '1' && websockets == '1')
      insFld(nodeDeviceInfo, gFld(getMenuTitle(MENU_CPU_MEM_GRAPH),'cpumemcharts.html'));
   // usb hosts info
   if (buildUsbHosts  == '1') {
