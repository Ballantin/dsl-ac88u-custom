...
static UBOOL8 glbImgIfEnabled = FALSE;
...
static int myWrite(char *inBuf, int inBufLen)
{
	...
	if (glbImgIfEnabled == TRUE)
	{
	if (imgifHandle == NULL)
	{
		imgifHandle = imgif_open(cmsImg_parseImgHdr, NULL);
		if (imgifHandle == NULL)
		{
			bb_error_msg("imgif_open() failed.");   
			return -1;
		}
		allocSize = cmsImg_getImageFlashSize() + cmsImg_getBroadcomImageTagSize();
                allocSize = (allocSize > CMS_IMAGE_MAX_ALLOC_LEN) ? CMS_IMAGE_MAX_ALLOC_LEN : allocSize;
	}
	}
	else
	{
	...
	{
		byteCount = imgif_write(imgifHandle, (UINT8*)inBuf, inBufLen);
		if ((byteCount < 0) || (byteCount != (int)inBufLen))
		{
			bb_error_msg("imgif_write() failed, towrite=%d, ret=%d", inBufLen, byteCount);
        		return -1;
		}
	...
	{
		bb_error_msg("Image bigger than estimated size %u byte.\n", allocSize);
		return -1;
	}
	}
	else
...
static size_t brcm_get_ftp_data(int src_fd, const size_t size)
{
   ...
   RESERVE_CONFIG_BUFFER(buffer,BUFSIZ);
	...
	glbImgIfEnabled = imgif_get_enable_mode();
		...
		*/
		...
		if (brcm_get_ftp_data(fd_data, filesize) != filesize) 
		{
			if (glbImgIfEnabled == TRUE)
			{
				...
				bb_error_msg("imgif_close() abort");
			}
			exit(EXIT_FAILURE);
		}
		...
		if (glbImgIfEnabled == TRUE)
		{
			imgif_close(imgifHandle, 0);
			bb_error_msg("imgif_close() ok, reboot");
			cmsUtil_sendRequestRebootMsg(msgHandle);
		}
		else
		{
			/*
			* cmsImg_writeImage will determine the image format and write
