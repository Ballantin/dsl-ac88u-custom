 ...
 *---------------------------------------------------------------------------
 */
static inline UINT32 xtmOverhead(UINT16 bufStatus, UINT32 hdrType, UINT32 len, UINT32 rfc2684Type)
{
    ...
    if(ctType == XCT_PTM)
    {
        ...
        UINT32 totalBytes = codewords * (XTMRT_PTM_CODEWORD_LEN + XTMRT_PTM_CODEWORD_SYNC_LEN);
...
#else
#define xtmOverhead(_bufStatus, _hdrType, _len, _rfc2684Type)  0
#endif
...
int bcmxtmrt_xmit(pNBuff_t pNBuf, struct net_device *dev)
{
   ...
   rc = bcmxapi_xmit_packet(&pNBuf, &pData, &len, txdma, txdma->ulDmaIndex,
                            bufStatus, skbMark, xtmOverhead(bufStatus, hdrType, len, rfc2684Type));
   if (rc)
   {
