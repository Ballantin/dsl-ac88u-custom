...
// ARCADYAN
int kerSysGetDectRfpi( unsigned char *Rfpi )
{
    NVRAM_DATA *pNvramData;
    int i=0;
    ...
    for (i=0;i<5 ;i++)
      Rfpi [i] = pNvramData->DECTRfpi[i];

    kfree(pNvramData);
    return 0;
}

/*Arcadyan 2015.09.01 Simon Chu, Add get RXTUN func */
int kerSysGetDectRxtun( char *Rxtun )
{
    NVRAM_DATA *pNvramData;
    ...
    memcpy( Rxtun, (char *)pNvramData + ((size_t) &((NVRAM_DATA *)0)->DECTRxtun), NVRAM_ARCADYAN_DECT_RXTUN_LEN);	
...
/// ARCADYAN
    ...
    case BOARD_IOCTL_SET_GPIO:
        ...
        break;
// ARCADYAN
    //ARC GPIO operation   jingwei 2015/03
    case BOARD_IOCTL_SET_GPIO_PINMUX:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            bcm_set_pinmux(ctrlParms.strLen, ctrlParms.offset);            //printk("BOARD_IOCTL_SET_GPIO_PINMUX: gpio_num=%d pinmux=%d\n",ctrlParms.strLen,ctrlParms.offset);
            ret = 0;
        }
        else {
            ret = -EFAULT;
    ...
    case BOARD_IOCTL_GET_GPIO:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            //printk("BOARD_IOCTL_GET_GPIO: gpio_num=%d pin_value=%d\n",ctrlParms.strLen,ctrlParms.offset);
            __copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
            ret = 0;
        }
        else {
        ...
        break;
    //End of ARC GPIO operation

/* ARCADYAN #ARC_RC_SD6#, 2015/03/20, simon, add DECT IOCTL */
    case BOARD_IOCTL_WAKEUP_DECT:
        kerSysSetGpioState(BP_GPIO_8_AL, kGpioActive);         //wakeup the DECT
        udelay(1000);           //1000us strobe, minimum is 1us from datasheet, but HW suggests 1ms
        kerSysSetGpioState(BP_GPIO_8_AL, kGpioInactive);       //release the wakeup
        printk(KERN_INFO "DECT should be waken up!\n");         //
        ret = 0;
        break;

    case BOARD_IOCTL_RESET_DECT:
        if (arg == 0) {
            kerSysSetGpioState(BP_GPIO_9_AL, kGpioActive);         //reset the DECT
            printk(KERN_INFO "DECT RESET kGpioActive\n");
        } else {
            kerSysSetGpioState(BP_GPIO_9_AL, kGpioInactive);       //release the reset
            printk(KERN_INFO "DECT RESET kGpioInactive\n");
        }
        ret = 0;
        break;

/*Arcadyan 2015.09.01 Simon Chu, Modify DECT get/set paramter */ 
    case BOARD_IOCTL_DECT_GET_PARAM:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
	    if (ctrlParms.action == ACTION_RFPI )  { // RFPI
                if( (ctrlParms.result = kerSysGetDectRfpi(ucaDectRfpi)) == 0 ) {
                    __copy_to_user(ctrlParms.string, ucaDectRfpi,
                        sizeof(ucaDectRfpi));
                }
            }
            else if (ctrlParms.action == ACTION_RXTUN ) { //RXTUN
                if ( (ctrlParms.result = kerSysGetDectRxtun(ucaDectRxtun)) == 0 ) {
                    __copy_to_user(ctrlParms.string, ucaDectRxtun,
                        sizeof(ucaDectRxtun));
                }
            }
    ...
    case BOARD_IOCTL_DECT_SET_PARAM:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            NVRAM_DATA *pNvramData;
            printk("in BOARD_IOCTL_DECT_SET_RFPI\n");

            mutex_lock(&flashImageMutex);

            if (NULL != (pNvramData = readNvramData())) {
              if (ctrlParms.action == ACTION_RFPI ) { //RFPI 
                  memcpy(pNvramData->DECTRfpi, ctrlParms.string, NVRAM_ARCADYAN_DECT_RFPI_LEN);
                  writeNvramDataCrcLocked(pNvramData);
              } else if (ctrlParms.action == ACTION_RXTUN ) { //RXTUN
		  memset((char *)pNvramData + ((size_t) &((NVRAM_DATA *)0)->DECTRxtun),0, sizeof(pNvramData->DECTRxtun)-1 );
                  strncpy(pNvramData->DECTRxtun, ctrlParms.string, ctrlParms.strLen);
                  writeNvramDataCrcLocked(pNvramData);
              } else {
                ctrlParms.result= -EFAULT;
              }
...
// ARCADYAN
    if((BpGetWirelessPowerDown2Gpio(&wlanPDGpio)) == BP_SUCCESS) {
        if (wlanPDGpio != BP_NOT_DEFINED) {
            if(state == WLAN_OFF)
                kerSysSetGpioState(wlanPDGpio, kGpioActive);
            else
...
// ARCADYAN
EXPORT_SYMBOL(BpGetWirelessPowerDown2Gpio);
/// ARCADYAN
