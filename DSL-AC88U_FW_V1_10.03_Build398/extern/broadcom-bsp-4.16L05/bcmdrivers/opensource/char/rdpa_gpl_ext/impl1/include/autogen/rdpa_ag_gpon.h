...
typedef enum {
    ...
    rdpa_gpon_attr_gem_ds_cfg = 21, /* gem_ds_cfg : RWF : aggregate/9[256(object)] gem_ds_cfg(rdpa_gpon_gem_ds_cfg_t) : Downstream GEM configuration */
 ...
 * Set misc transmit.
 * \param[in]   mo_ gpon object handle or mattr transaction handle
 * \param[in]   misc_tx Attribute value
 * \return 0 or error code < 0
 * The function can be called in task context only.
 */
