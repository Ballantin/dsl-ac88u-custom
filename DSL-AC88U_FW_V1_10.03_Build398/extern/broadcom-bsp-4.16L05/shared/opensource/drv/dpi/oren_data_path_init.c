...
static void _f_initialize_dma_sdma(DRV_BBH_PORT_INDEX bbh_port)
{
    E_DMA_PERIPHERAL peripheral_id = (bbh_port == pDpiCfg->wan_bbh && pDpiCfg->wan_bbh > DRV_BBH_EMAC_4) ?
        DMA_PERIPHERAL_EMAC_AE_GPON_EPON : bbh_port;
    ...
    fi_dma_configure_memory_allocation(BB_MODULE_DMA,
            peripheral_id,
            bbh_rx_dma_data_fifo_base_address[bbh_port],
            bbh_rx_dma_chunk_descriptor_fifo_base_address[bbh_port],
            bbh_rx_dma_data_and_chunk_descriptor_fifos_size[bbh_port]);
    fi_dma_configure_memory_allocation(BB_MODULE_SDMA,
            peripheral_id,
            bbh_rx_sdma_data_fifo_base_address[bbh_port],
            bbh_rx_sdma_chunk_descriptor_fifo_base_address[bbh_port],
            bbh_rx_sdma_data_and_chunk_descriptor_fifos_size[bbh_port]);
    ...
    dma_pri_cfg.txpri = (bbh_port==DRV_BBH_EPON) ? 
...
{
    uint32_t dma_mem_sel_reg, bbh_port;

    if (!fiber)
    {
        for (bbh_port = DRV_BBH_EMAC_0; bbh_port < DRV_BBH_EMAC_5; bbh_port++)
            _f_initialize_dma_sdma(bbh_port);
    }
    else
...
DRV_BPM_USER_GROUP_0 : DRV_BPM_USER_GROUP_1)
    ...
    if (bpm_defenitions[global_thresh].dongle_thresh)
    {
        ...
        bpm_ug_configuration.ug_arr[DRV_BPM_USER_GROUP_2].exclusive_threshold = bpm_defenitions[global_thresh].dongle_thresh;
    }
