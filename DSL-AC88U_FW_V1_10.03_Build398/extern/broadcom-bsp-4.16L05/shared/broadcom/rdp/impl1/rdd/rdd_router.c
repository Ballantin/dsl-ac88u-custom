...
void _rdd_fc_context_read_from_table_entry(rdd_fc_context_t *ctx, RDD_FLOW_CACHE_CONTEXT_ENTRY_DTS *entry)
{
    ...
    ctx->trap_reason = ctx->trap_reason + rdpa_cpu_rx_reason_connection_trap_0;
                                                  ...
                                                  rdpa_traffic_dir                 xi_direction )
{
    ...
    if ( xi_add_connection->lookup_entry->dst_ip.family != bdmf_ip_family_ipv4 )
    {
        ...
        {
            if ( xi_add_connection->context_entry.actions_vector & rdpa_fc_action_tunnel )
            {
                return ( BL_LILAC_RDD_ERROR_ADD_CONTEXT_ENTRY );
            }
        }
...
BL_LILAC_RDD_ERROR_DTE rdd_clean_context_aging_get(uint32_t context_entry_index, uint8_t *xo_aging_status)
{
    RDD_FLOW_CACHE_CONTEXT_ENTRY_DTS *entry;
    unsigned long flags;
    ...
    RDD_FLOW_CACHE_CONTEXT_ENTRY_ACTIVITY_STATUS_READ(*xo_aging_status, entry);
    RDD_FLOW_CACHE_CONTEXT_ENTRY_ACTIVITY_STATUS_WRITE(LILAC_RDD_OFF, entry);
