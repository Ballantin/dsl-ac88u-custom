...

{
    ...
    *xo_rule_cfg_lookup_mode = rule_cfg_lookup_mode;
    ...
    {
        for ( new_rule_cfg_id = 1; new_rule_cfg_id < RDD_INGRESS_CLASSIFICATION_RULE_CFG_TABLE_SIZE; new_rule_cfg_id++ )
        {
    ...
    if ( new_rule_cfg_id == RDD_INGRESS_CLASSIFICATION_RULE_CFG_TABLE_SIZE )
    {
        f_rdd_unlock_irq ( &int_lock_irq, flags );
    ...
    rule_cfg_entry_ptr = &( rule_cfg_table_ptr->entry[ new_rule_cfg_id ] );
    ...
    if ( g_ingress_classification_rule_cfg_table[ xi_direction ].first_rule_cfg_id == RDD_INGRESS_CLASSIFICATION_RULE_CFG_TABLE_SIZE )
    {
    ...
    else
    {
        ...
        while ( 1 )
        {
            if ( ( next_rule_cfg_id == RDD_INGRESS_CLASSIFICATION_RULE_CFG_TABLE_SIZE ) || ( xi_rule_cfg_priority > g_ingress_classification_rule_cfg_table[ xi_direction ].rule_cfg[ next_rule_cfg_id ].priority ) )
            {
                                                                    ...
                                                                    int32_t           xi_rule_cfg_priority )
{
    ...
    while ( 1 )
    {
        if ( deleted_rule_cfg_id == RDD_INGRESS_CLASSIFICATION_RULE_CFG_TABLE_SIZE )
        {
            f_rdd_unlock_irq ( &int_lock_irq, flags );
        ...
        if ( g_ingress_classification_rule_cfg_table[ xi_direction ].rule_cfg[ deleted_rule_cfg_id ].priority == xi_rule_cfg_priority )
        {
            ...
            rule_cfg_entry_ptr = &( rule_cfg_table_ptr->entry[ deleted_rule_cfg_id ] );
            ...
            g_ingress_classification_rule_cfg_table[ xi_direction ].rule_cfg[ deleted_rule_cfg_id ].next_rule_cfg_id = RDD_INGRESS_CLASSIFICATION_RULE_CFG_TABLE_SIZE;
            g_ingress_classification_rule_cfg_table[ xi_direction ].rule_cfg[ deleted_rule_cfg_id ].next_group_id = RDD_INGRESS_CLASSIFICATION_RULE_CFG_TABLE_SIZE;
                                                                   ...
                                                                   const rdd_ingress_classification_context_t  *xi_context )
{
    ...
    if ( xi_direction == rdpa_dir_ds )
    {
        ...
        if ( xi_context->rate_shaper < 0 )
        {
            RDD_DS_INGRESS_CLASSIFICATION_CONTEXT_ENTRY_RATE_SHAPING_MODE_WRITE ( 0, ds_ingress_classification_context_entry_ptr );
            RDD_DS_INGRESS_CLASSIFICATION_CONTEXT_ENTRY_RATE_SHAPER_ID_WRITE ( 0, ds_ingress_classification_context_entry_ptr );
        }
        else
        {
            RDD_DS_INGRESS_CLASSIFICATION_CONTEXT_ENTRY_RATE_SHAPING_MODE_WRITE ( 1, ds_ingress_classification_context_entry_ptr );
            RDD_DS_INGRESS_CLASSIFICATION_CONTEXT_ENTRY_RATE_SHAPER_ID_WRITE ( xi_context->rate_shaper, ds_ingress_classification_context_entry_ptr );
        }
        ...
        if ( g_broadcom_switch_mode && ( xi_context->vlan_command_id.ds_vlan_command.multicast_vlan_command != LILAC_RDD_VLAN_COMMAND_SKIP ) )
        {
            RDD_VLAN_COMMAND_INDEX_ENTRY_ETH0_VLAN_COMMAND_ID_WRITE ( xi_context->vlan_command_id.ds_vlan_command.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
            RDD_VLAN_COMMAND_INDEX_ENTRY_ETH1_VLAN_COMMAND_ID_WRITE ( xi_context->vlan_command_id.ds_vlan_command.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
            RDD_VLAN_COMMAND_INDEX_ENTRY_ETH2_VLAN_COMMAND_ID_WRITE ( xi_context->vlan_command_id.ds_vlan_command.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
            RDD_VLAN_COMMAND_INDEX_ENTRY_ETH3_VLAN_COMMAND_ID_WRITE ( xi_context->vlan_command_id.ds_vlan_command.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
            RDD_VLAN_COMMAND_INDEX_ENTRY_ETH4_VLAN_COMMAND_ID_WRITE ( xi_context->vlan_command_id.ds_vlan_command.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
            RDD_VLAN_COMMAND_INDEX_ENTRY_PCI0_VLAN_COMMAND_ID_WRITE ( xi_context->vlan_command_id.ds_vlan_command.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
            RDD_VLAN_COMMAND_INDEX_ENTRY_MULTICAST_VLAN_COMMAND_WRITE ( LILAC_RDD_VLAN_COMMAND_SKIP, vlan_cmd_idx_entry_ptr );
        }
        else
        {
            ...
            RDD_VLAN_COMMAND_INDEX_ENTRY_MULTICAST_VLAN_COMMAND_WRITE ( xi_context->vlan_command_id.ds_vlan_command.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
        }
    }
    else
    {
                                                                ...
                                                                rdd_ingress_classification_context_t  *xo_context )
{
    ...
    if ( xi_direction == rdpa_dir_ds )
    {
        ...
        RDD_DS_INGRESS_CLASSIFICATION_CONTEXT_ENTRY_RATE_SHAPING_MODE_READ ( rs_mode, ds_ingress_classification_context_entry_ptr );

        if ( rs_mode )
        {
            RDD_DS_INGRESS_CLASSIFICATION_CONTEXT_ENTRY_RATE_SHAPER_ID_READ ( xo_context->rate_shaper, ds_ingress_classification_context_entry_ptr );
        }
        else
        {
        ...
        RDD_VLAN_COMMAND_INDEX_ENTRY_MULTICAST_VLAN_COMMAND_READ ( xo_context->vlan_command_id.ds_vlan_command.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
    }
    else
    {
        if ( xi_context_id >= RDD_US_INGRESS_CLASSIFICATION_CONTEXT_TABLE_SIZE )
...
static BL_LILAC_RDD_ERROR_DTE rdd_ingress_classification_long_lookup_entry_update ( rdpa_traffic_dir  xi_direction,
                                                                                 ...
                                                                                 int is_add)
{
    ...
    crc_init_value = get_crc_init_value ( RDD_CRC_TYPE_16 );
