...
static void p_lilac_rdd_check_lists_ds_free_pool ( bdmf_session_handle session, uint8_t xi_print_descriptors )
{
	...
	list_size = p_lilac_rdd_check_lists_get_list_size ( session, head_address, tail_address, RDD_DS_FREE_PACKET_DESCRIPTORS_POOL_SIZE, DEVICE_ADDRESS( RUNNER_PRIVATE_0_OFFSET ), xi_print_descriptors );
	...
	for ( tcont_id = RDD_WAN_CHANNEL_0; tcont_id <= RDD_WAN_CHANNEL_39; tcont_id++ )
	{
		...
		for ( rc_id = BL_LILAC_RDD_RATE_CONTROLLER_0; rc_id <= ( ( tcont_id <= RDD_WAN_CHANNEL_7 ) ? BL_LILAC_RDD_RATE_CONTROLLER_31 : BL_LILAC_RDD_RATE_CONTROLLER_3 ); rc_id++ )
		{
			...
			for ( queue_id = BL_LILAC_RDD_QUEUE_0; queue_id <= BL_LILAC_RDD_QUEUE_7; queue_id++ )
			{
				...
				list_size = p_lilac_rdd_check_lists_get_list_size ( session, head_address, tail_address, RDD_US_FREE_PACKET_DESCRIPTORS_POOL_SIZE, DEVICE_ADDRESS( RUNNER_PRIVATE_1_OFFSET ), xi_print_descriptors );
                                              ...
                                              rdpa_iptv_lookup_method  xi_classification_mode )
{
    ...
    switch ( xi_iptv_entry->l3_entry_fields.bridge_port )
    {
    case BL_LILAC_RDD_WAN0_BRIDGE_PORT: // DSL
    case BL_LILAC_RDD_WAN1_BRIDGE_PORT: // DSL
        bdmf_session_print ( session, "%-13s ", "GPON" );
        break;
    case BL_LILAC_RDD_LAN0_BRIDGE_PORT :
        bdmf_session_print ( session, "%-13s ", "ETH0" );
        break;
    ...
    case BL_LILAC_RDD_LAN4_BRIDGE_PORT :
        ...
        break;
    case BL_LILAC_RDD_WAN_ROUTER_PORT :
        bdmf_session_print ( session, "%-13s ", "GPON ROUTER" );
        break;
    case BL_LILAC_RDD_WAN_IPTV_BRIDGE_PORT :
        bdmf_session_print ( session, "%-13s ", "GPON IPTV" );
        break;
    case BL_LILAC_RDD_PCI_BRIDGE_PORT :
        bdmf_session_print ( session, "%-13s ", "PCI" );
        break;
    default :
        if ( xi_iptv_entry->l3_entry_fields.bridge_port & 0x3f0 )
        {
            bdmf_session_print ( session, "MC: 0x%-7X ", xi_iptv_entry->l3_entry_fields.bridge_port );
        }
        else
        {
            bdmf_session_print ( session, "%-13s ", "INVALID" );
    ...
    bdmf_session_print ( session, "0x%04x ", xi_iptv_entry->l3_entry_fields.ssid_vector );
    ...
    if ( layer_flag == 0 )
    {
        ...
        for ( entry_index = 0; entry_index < RDD_IPTV_DDR_LOOKUP_TABLE_SIZE; entry_index++ )
        {
            ...
            if ( rdd_error == BL_LILAC_RDD_OK )
            {
                ...
                switch ( iptv_entry.l2_entry_fields.bridge_port )
                {
                case BL_LILAC_RDD_WAN0_BRIDGE_PORT: // DSL
                case BL_LILAC_RDD_WAN1_BRIDGE_PORT: // DSL
                    bdmf_session_print ( session, "%-13s ", "GPON" );
                ...
                case BL_LILAC_RDD_PCI_BRIDGE_PORT :
                    ...
                    break;
                default :
                    if ( iptv_entry.l2_entry_fields.bridge_port & 0x3f0 )
                    {
                        bdmf_session_print ( session, "MC: 0x%-7X ", iptv_entry.l2_entry_fields.bridge_port );
                    }
                    else
                    {
                        bdmf_session_print ( session, "%-13s ", "INVALID" );
                ...
                bdmf_session_print ( session, "0x%-6x", iptv_entry.l2_entry_fields.ssid_vector );
...
{	
    RDD_FLOW_CACHE_CONTEXT_ENTRY_DTS  context;
    ...
    context_entry_ptr = ( RDD_CONTEXT_ENTRY_UNION_DTS * )( &( context_table_ptr->entry[ index ] ) );

    RDD_FLOW_CACHE_CONTEXT_ENTRY_ACTIVITY_STATUS_READ ( context.activity_status, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_VALID_PACKETS_COUNTER_READ ( context.valid_packets_counter, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_FWD_ACTION_READ ( context.fwd_action, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_CPU_REASON_READ ( context.cpu_reason, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_DSCP_VALUE_READ ( context.dscp_value, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_ECN_VALUE_READ ( context.ecn_value, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_NAT_PORT_READ ( context.nat_port, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_IP_CHECKSUM_DELTA_READ ( context.ip_checksum_delta, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_LAYER4_CHECKSUM_DELTA_READ ( context.layer4_checksum_delta, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_NAT_IP_READ ( context.nat_ip, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_EGRESS_PORT_READ ( context.egress_port, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_WIFI_SSID_READ ( context.wifi_ssid, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_IP_VERSION_READ ( context.ip_version, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_TRAFFIC_CLASS_READ ( context.traffic_class, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_WAN_FLOW_INDEX_READ ( context.wan_flow_index, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_RATE_CONTROLLER_READ ( context.rate_controller, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_DS_LITE_HEADER_INDEX_READ ( context.ds_lite_header_index, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_OUTER_VID_OFFSET_READ ( context.outer_vid_offset, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_POLICER_ID_READ ( context.policer_id, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_OVERFLOW_READ ( context.overflow, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_OUTER_PBIT_REMAP_ACTION_READ ( context.outer_pbit_remap_action, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_INNER_PBIT_REMAP_ACTION_READ ( context.inner_pbit_remap_action, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_LAYER2_HEADER_NUMBER_OF_TAGS_READ ( context.layer2_header_number_of_tags, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_L2_OFFSET_READ ( context.l2_offset, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_L2_SIZE_READ ( context.l2_size, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_ACTIONS_VECTOR_READ ( context.actions_vector, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_CONNECTION_DIRECTION_READ ( context.connection_direction, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_CONNECTION_TABLE_INDEX_READ ( context.connection_table_index, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_SERVICE_QUEUES_MODE_READ ( context.service_queues_mode, context_entry_ptr );
    RDD_FLOW_CACHE_CONTEXT_ENTRY_SERVICE_QUEUE_READ ( context.service_queue, context_entry_ptr );

    for ( i = 0; i < RDD_FLOW_CACHE_CONTEXT_ENTRY_L2_HEADER_NUMBER; i++ )
    {
        RDD_FLOW_CACHE_CONTEXT_ENTRY_L2_HEADER_READ ( context.l2_header[ i ], context_entry_ptr, i );
    }

    bdmf_session_print ( session, "Activity status:         %6d\n", ( int )context.activity_status );
    bdmf_session_print ( session, "Valid packets counter:   %6d\n", ( int )context.valid_packets_counter );
    bdmf_session_print ( session, "Forward action:          %6d\n", ( int )context.fwd_action );
    bdmf_session_print ( session, "CPU reason:              %6d\n", ( int )context.cpu_reason );
    ...
    bdmf_session_print ( session, "IP checksum delta:       %6d\n", ( int )context.ip_checksum_delta );
    bdmf_session_print ( session, "L4 checksum delta:       %6d\n", ( int )context.layer4_checksum_delta );
    bdmf_session_print ( session, "NAT IP:                  %-3d.%-3d.%-3d.%-3d\n", ( int )( ( context.nat_ip >> 24 ) & 0xFF ), ( int )( ( context.nat_ip >> 16 ) & 0xFF ), ( int )( ( context.nat_ip >> 8 ) & 0xFF), ( int )( context.nat_ip & 0xFF ) );
    ...
    if ( context.actions_vector != 0 )
    {
        ...
        if ( context.actions_vector & ( 1 << rdpa_fc_act_tunnel ) )
        {
            bdmf_session_print ( session, "| tunnel " );
        }
    ...
    return context.connection_table_index;
}
...
#else
    bdmf_session_print ( session, "Occupied SKB's: %d\n", ( int )( g_cpu_tx_abs_packet_limit - g_cpu_tx_skb_free_indexes_counter) );
#endif
...
static int p_lilac_rdd_print_ingress_classification_context ( bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms )
{
    ...
    if ( direction == rdpa_dir_ds )
    {
        ...
        vlan_cmd_idx_table_ptr = ( RDD_VLAN_COMMAND_INDEX_TABLE_DTS * )(DEVICE_ADDRESS( RUNNER_PRIVATE_0_OFFSET ) + VLAN_COMMAND_INDEX_TABLE_ADDRESS );
        ...
        RDD_DS_INGRESS_CLASSIFICATION_CONTEXT_ENTRY_RATE_SHAPER_ID_READ ( ds_context.union_field1, ds_ingress_classification_context_entry_ptr );
        ...
        RDD_VLAN_COMMAND_INDEX_ENTRY_MULTICAST_VLAN_COMMAND_READ ( vlan_entry.multicast_vlan_command, vlan_cmd_idx_entry_ptr );
    }
    else
    {
    ...
    if ( direction == rdpa_dir_ds )
    {
        ...
        bdmf_session_print ( session, "Rate shaping mode:   %6d\n", ( int )ds_context.rate_shaping_mode );
        if ( ds_context.service_queue_mode )
        {
            bdmf_session_print ( session, "Service queue:      %6d\n", ( int )ds_context.union_field1 );
        }
        else
        {
            bdmf_session_print ( session, "Rate shaper ID:      %6d\n", ( int )ds_context.union_field1 );
        }
        ...
        bdmf_session_print ( session, "multicast_vlan_command: %6d\n", ( signed char )vlan_entry.multicast_vlan_command );
    }
    else
    {  
...
static int p_lilac_rdd_print_ingress_classification_rule_cfgs ( bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms )
{
    ...
    for ( rule_cfg_id = 0; rule_cfg_id < RDD_INGRESS_CLASSIFICATION_RULE_CFG_TABLE_SIZE; rule_cfg_id++ )
    {
