...
int enableMRouter()
{
    ...
    if ( (MRouterFD  = socket(AF_INET, SOCK_RAW, IPPROTO_IGMP)) < 0 )
        my_log( LOG_ERR, errno, "[enableMRouter] IGMP socket open" );
...
void disableMRouter()
{
    if ( setsockopt( MRouterFD, IPPROTO_IP, MRT_DONE, NULL, 0 )
        ...
        MRouterFD = 0;
        my_log( LOG_ERR, errno, "[disableMRouter] MRT_DONE/close" );
    }
...
void addVIF( struct IfDesc *IfDp )
{
    ...
    if ( VifDp >= VCEP( VifDescVc ) )
        my_log( LOG_ERR, ENOMEM, "[addVIF] addVIF, out of VIF space" );
    ...
    my_log( LOG_NOTICE, 0, "[addVIF] adding VIF, Ix %d Fl 0x%x IP 0x%08x %s, Threshold: %d, Ratelimit: %d",
	...
	my_log(LOG_DEBUG, 0, "[addVIF]         Network for [%s] : %s",
	    IfDp->Name,
    ...
    if ( setsockopt( MRouterFD, IPPROTO_IP, MRT_ADD_VIF,
        ...
        my_log( LOG_ERR, errno, "[addVIF] MRT_ADD_VIF" );
...
int addMRoute( struct MRouteDesc *Dp )
{
...

    {
        ...
        my_log( LOG_NOTICE, 0, "[addMRoute] Adding MFC: %s -> %s, InpVIf: %d",
    ...
    if (rc)
        my_log( LOG_WARNING, errno, "[addMRoute] MRT_ADD_MFC" );
