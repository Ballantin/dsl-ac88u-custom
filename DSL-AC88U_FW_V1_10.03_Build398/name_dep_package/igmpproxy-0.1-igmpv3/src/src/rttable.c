...
void initRouteTable() {
    ...
    for ( Ix = 0; (Dp = getIfByIx(Ix)); Ix++ ) {
        ...
        if( Dp->InAdr.s_addr && ! (Dp->Flags & IFF_LOOPBACK) && Dp->state == IF_STATE_DOWNSTREAM) {
            ...
            my_log(LOG_DEBUG, 0, "Joining all-routers group %s on vif %s",
                         inetFmt(igmpv3_allrouters_group,s1),inetFmt(Dp->InAdr.s_addr,s2));
            joinMcGroup( getMcGroupSock(), Dp, igmpv3_allrouters_group );
#endif
        }
    }
}
...
void sendJoinLeaveUpstream(struct RouteTable* route, int join) {
    ...
    if(join) {
        ...
        if(route->vifBits > 0) {
            ...
            my_log(LOG_DEBUG, 0, "[sendJoinLeaveUpstream] No downstream listeners for group %s. No join sent.",
                inetFmt(route->group, s1));
        }
        ...
        if(route->upstrState != ROUTESTATE_NOTJOINED) {
            my_log(LOG_DEBUG, 0, "[sendJoinLeaveUpstream] Leaving group %s upstream on IF address %s",
                         inetFmt(route->group, s1),
                         inetFmt(upstrIf->InAdr.s_addr, s2));
...
int insertRoute(uint32_t group, int ifx) {
    ...
    if( ! IN_MULTICAST( ntohl(group) )) {
        my_log(LOG_WARNING, 0, "[insertRoute] The group address %s is not a valid Multicast group. Table insert failed.",
            inetFmt(group, s1));
        return 0;
    }
    ...
    if(ifx >= MAX_MC_VIFS) {
        my_log(LOG_WARNING, 0, "[insertRoute] The VIF Ix %d is out of range (0-%d). Table insert failed.",ifx,MAX_MC_VIFS);
        return 0;
    }
    ...
    if(croute==NULL) {
        ...
        my_log(LOG_DEBUG, 0, "[insertRoute] No existing route for %s. Create new.",
                     inetFmt(group, s1));
        ...
        if(routing_table == NULL) {
            ...
            my_log(LOG_DEBUG, 0, "[insertRoute] No routes in table. Insert at beginning.");
        } else {

            my_log(LOG_DEBUG, 0, "[insertRoute] Found existing routes. Find insert location.");

            // Check if the route could be inserted at the beginning...
            if(routing_table->group > group) {
                my_log(LOG_DEBUG, 0, "[insertRoute] Inserting at beginning, before route %s",inetFmt(routing_table->group,s1));
                ...
                my_log(LOG_DEBUG, 0, "[insertRoute] Inserting after route %s",inetFmt(croute->group,s1));
        ...
        // Log the cleanup in debugmode...
        my_log(LOG_INFO, 0, "[insertRoute] Inserted route table entry for %s on VIF #%d",
        ...
        // Log the cleanup in debugmode...
        my_log(LOG_INFO, 0, "[insertRoute] Updated route entry for %s on VIF #%d",
            inetFmt(croute->group, s1), ifx);
        ...
        if(croute->originAddr != 0) {
            ...
            if(!internUpdateKernelRoute(croute, 1)) {
                my_log(LOG_WARNING, 0, "[insertRoute] The insertion into Kernel failed.");
                return 0;
            }
        }
    }
    ...
    logRouteTable("[insertRoute] Insert Route");
...
int activateRoute(uint32_t group, uint32_t originAddr) {
    ...
    if(croute == NULL) {
        my_log(LOG_DEBUG, 0,
		"[activateRoute] No table entry for %s [From: %s]. Inserting route.",
    ...
    if(croute != NULL) {
        ...
        if(originAddr > 0) {
            if(croute->originAddr > 0 && croute->originAddr!=originAddr) {
                my_log(LOG_WARNING, 0, "[activateRoute] The origin for route %s changed from %s to %s",
                    inetFmt(croute->group, s1),
    ...
    logRouteTable("[activateRoute] Activate Route");
...
void ageActiveRoutes() {
    ...
    my_log(LOG_DEBUG, 0, "[ageActiveRoutes] Aging routes in table.");
    ...
    logRouteTable("[ageActiveRoutes] Age active routes");
}
...
int removeRoute(struct RouteTable*  croute) {
    ...
    // Log the cleanup in debugmode...
    my_log(LOG_DEBUG, 0, "[removeRoute] Removed route entry for %s from table.",
                 inetFmt(croute->group, s1));
    ...
    if(!internUpdateKernelRoute(croute, 0)) {
        my_log(LOG_WARNING, 0, "[removeRoute] The removal from Kernel failed.");
        result = 0;
    }
    ...
    logRouteTable("[removeRoute] Remove route");
...
int internAgeRoute(struct RouteTable*  croute) {
    ...
    if(croute->ageValue == 0) {
        ...
        if(croute->ageActivity>0) {
            ...
            my_log(LOG_DEBUG, 0, "[internAgeRoute] Updating route after aging : %s",
                         inetFmt(croute->group,s1));
            ...
            my_log(LOG_DEBUG, 0, "[internAgeRoute] Removing group %s. Died of old age.",
                         inetFmt(croute->group,s1));
...
int internUpdateKernelRoute(struct RouteTable *route, int activate) {
    ...
    if(route->originAddr>0) {
        ...
        my_log(LOG_DEBUG, 0, "[internUpdateKernelRoute] Vif bits : 0x%08x", route->vifBits);
        ...
        for ( Ix = 0; (Dp = getIfByIx(Ix)); Ix++ ) {
            if(Dp->state == IF_STATE_UPSTREAM) {
                ...
                my_log(LOG_DEBUG, 0, "[internUpdateKernelRoute] Setting TTL for Vif %d to %d", Dp->index, Dp->threshold);
        ...
        my_log(LOG_NOTICE, 0, "[internUpdateKernelRoute] Route is not active. No kernel updates done.");
    }
...
void logRouteTable(char *header) {
        ...
        my_log(LOG_DEBUG, 0, "");
        my_log(LOG_DEBUG, 0, "[logRouteTable] Current routing table (%s):", header);
        my_log(LOG_DEBUG, 0, "-----------------------------------------------------");
        if(croute==NULL) {
            my_log(LOG_DEBUG, 0, "[logRouteTable] No routes in table...");
        } else {
            do {
                /*
