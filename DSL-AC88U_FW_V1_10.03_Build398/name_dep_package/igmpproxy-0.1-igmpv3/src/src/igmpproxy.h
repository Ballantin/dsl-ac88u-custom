...
#define IGMP_UPNP_GROUP                 0xeffffffaU /* 239.255.255.250 */

#define SUPPORT_IGMP_V3

#ifdef SUPPORT_IGMP_V3

#define ROUTER_ALERT_OPTION
#define INADDR_IGMPV3_ALLRTRS_GROUP		0xe0000016U	/* 224.0.0.22 */
...
typedef struct {
	...
	byte		MRT;			// Max Response Time
	word		checksum;
	longword	address;		// group address
	byte		s_qrv;			// Resv, S, QRV
	byte		qqic;			// QQIC
	word		number_of_src;	// Number of Sources
} igmpv3Query;
...
typedef struct {
	...
	word		checksum;
	word		reserved2;
	word		number_of_grp;	// Number of Group Records
} igmpv3Report;

#define IGMP_GRPREC_MODE_IS_INCLUDE			1
#define IGMP_GRPREC_MODE_IS_EXCLUDE			2
#define IGMP_GRPREC_CHANGE_TO_INCLUDE_MODE	3
#define IGMP_GRPREC_CHANGE_TO_EXCLUDE_MODE	4
#define IGMP_GRPREC_ALLOW_NEW_SOURCES		5
#define IGMP_GRPREC_BLOCK_OLD_SOURCES		6
typedef struct {
	byte		type;
	byte		aux_data_len;
	word		number_of_src;
...
#ifdef SUPPORT_IGMP_V3
extern uint32_t igmpv3_allrouters_group;
#endif
void initIgmp(void);
void acceptIgmp(int);
...
extern void longToByteArray(unsigned char *byte_value, unsigned long long_value);
