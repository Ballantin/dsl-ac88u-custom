...
int isAdressValidForIf( struct IfDesc* intrface, uint32_t ipaddr ) {
    ...
    for(currsubnet = intrface->allowednets; currsubnet != NULL; currsubnet = currsubnet->next) {
		...
		/* linghong_tan 2014.05.30   Sync from W724. 
		 *
		 * Since netmask of ppp0 is 255.255.255.255, the source address 
		 * is impossible under the subnet of ppp0. That caused kernel drop 
		 * multicast traffic on ppp interface as lack of MFC to LAN side.
		 */
        if( (currsubnet->subnet_mask == 0xFFFFFFFF) || (ipaddr & currsubnet->subnet_mask) == currsubnet->subnet_addr) {
            return 1;
        }
    }
    return 0;
