...
int timer_setTimer(int delay, timer_f action, void *data) {
    ...
    if (node == 0) {
        my_log(LOG_WARNING, 0, "[timer_setTimer] Malloc Failed in timer_settimer\n");
        return -1;
    }
    node->func = action;
    node->data = data;
    ...
    if (!queue) {
        queue = node;
    	my_log(LOG_DEBUG, 0, "[timer_setTimer] queue is empty");
    }
    else {
        ...
        while (ptr) {
            if (delay < ptr->time) {
                ...
                ptr->time -= node->time;
		my_log(LOG_DEBUG, 0,
			"[timer_setTimer] Created timeout %d (#%d) - delay %d secs",
			node->id, i, node->time);
		debugQueue();
                return node->id;
            } else {
    ...
    my_log(LOG_DEBUG, 0, "[timer_setTimer] Created timeout %d (#%d) - delay %d secs",
            node->id, i, node->time);
    debugQueue();
