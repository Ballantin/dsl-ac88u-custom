...
int loadConfig(char *configFile) {
    ...
    if(!openConfigFile(configFile)) {
        my_log(LOG_ERR, 0, "[loadConfig] Unable to open configfile from %s", configFile);
    }

    // Get first token...
    token = nextConfigToken();
    if(token == NULL) {
        my_log(LOG_ERR, 0, "[loadConfig] Config file was empty.");
    }
    ...
    while ( token != NULL ) {
        ...
        if(strcmp("phyint", token)==0) {
            ...
            my_log(LOG_DEBUG, 0, "[loadConfig] Config: Got a phyint token.");
            tmpPtr = parsePhyintToken();
            if(tmpPtr == NULL) {
                // Unparsable token... Exit...
                closeConfigFile();
                my_log(LOG_WARNING, 0, "[loadConfig] Unknown token '%s' in configfile", token);
                return 0;
            } else {

                my_log(LOG_DEBUG, 0, "[loadConfig] IF name : %s", tmpPtr->name);
                my_log(LOG_DEBUG, 0, "[loadConfig] Next ptr : %x", tmpPtr->next);
                my_log(LOG_DEBUG, 0, "[loadConfig] Ratelimit : %d", tmpPtr->ratelimit);
                my_log(LOG_DEBUG, 0, "[loadConfig] Threshold : %d", tmpPtr->threshold);
                my_log(LOG_DEBUG, 0, "[loadConfig] State : %d", tmpPtr->state);
                my_log(LOG_DEBUG, 0, "[loadConfig] Allowednet ptr : %x", tmpPtr->allowednets);
            ...
            my_log(LOG_DEBUG, 0, "[loadConfig] Config: Quick leave mode enabled.");
...
struct vifconfig *parsePhyintToken() {
    ...
    my_log(LOG_DEBUG, 0, "[parsePhyintToken] Config: IF: Config for interface %s.", token);
    ...
    while(token != NULL) {
        if(strcmp("altnet", token)==0) {
            // Altnet...
            token = nextConfigToken();
            my_log(LOG_DEBUG, 0, "[parsePhyintToken] Config: IF: Got altnet token %s.",token);

            *anetPtr = parseSubnetAddress(token);
            if(*anetPtr == NULL) {
                parseError = 1;
                my_log(LOG_WARNING, 0, "[parsePhyintToken] Unable to parse subnet address.");
                break;
            } else {
            ...
            // Upstream
            my_log(LOG_DEBUG, 0, "[parsePhyintToken] Config: IF: Got upstream token.");
            tmpPtr->state = IF_STATE_UPSTREAM;
        }
        else if(strcmp("downstream", token)==0) {
            // Downstream
            my_log(LOG_DEBUG, 0, "[parsePhyintToken] Config: IF: Got downstream token.");
            tmpPtr->state = IF_STATE_DOWNSTREAM;
        }
        else if(strcmp("disabled", token)==0) {
            // Disabled
            my_log(LOG_DEBUG, 0, "[parsePhyintToken] Config: IF: Got disabled token.");
            tmpPtr->state = IF_STATE_DISABLED;
        }
        else if(strcmp("ratelimit", token)==0) {
            // Ratelimit
            token = nextConfigToken();
            my_log(LOG_DEBUG, 0, "[parsePhyintToken] Config: IF: Got ratelimit token '%s'.", token);
            tmpPtr->ratelimit = atoi( token );
            if(tmpPtr->ratelimit < 0) {
                my_log(LOG_WARNING, 0, "[parsePhyintToken] Ratelimit must be 0 or more.");
                parseError = 1;
                break;
            }
        }
            ...
            // Threshold
            token = nextConfigToken();
            my_log(LOG_DEBUG, 0, "[parsePhyintToken] Config: IF: Got threshold token '%s'.", token);
            tmpPtr->threshold = atoi( token );
            if(tmpPtr->threshold <= 0 || tmpPtr->threshold > 255) {
                my_log(LOG_WARNING, 0, "[parsePhyintToken] Threshold must be between 1 and 255.");
                parseError = 1;
                break;
            }
        }
...
struct SubnetList *parseSubnetAddress(char *addrstr) {
    ...
    if(tmpStr != NULL) {
        ...
        if(bitcnt <= 0 || bitcnt > 32) {
            my_log(LOG_WARNING, 0, "[parseSubnetAddress] The bits part of the address is invalid : %d.",tmpStr);
            return NULL;
        }
    ...
    if(addr == -1 || addr == 0) {
        my_log(LOG_WARNING, 0, "[parseSubnetAddress] Unable to parse address token '%s'.", addrstr);
        return NULL;
    }
    ...
    my_log(LOG_DEBUG, 0, "[parseSubnetAddress] Config: IF: Altnet: Parsed altnet to %s.",
