...
extern int iAllowAnonymous;
extern int iAccNum;
extern struct passwd *pwdlist;
...
vsf_sysutil_getpwnam(const char* p_user)
{
	...
	static char char_user[64];
	static char char_pwd[64];
	int acc_num, i, iAnonymous=0;
	int iMatched = 0;
	//ftp_dbg("%s: username %s\n", __func__, p_user);
	if (strcmp(p_user, "root") == 0){
		result = &pwd;
		bzero(result, sizeof(struct passwd));
		result->pw_name = "root";
	...
	iAnonymous = iAllowAnonymous;
	//ftp_dbg("iAllowAnonymous %d, iAnonymous %d\n", iAllowAnonymous, iAnonymous);
	if(iAnonymous==0)
	{
		...
		for(i=0;i<acc_num;i++)
		{
			...
			//ftp_dbg("name '%s'\n", pwdptr->pw_name);
			//ftp_dbg("p_user '%s'\n", p_user);
			if( strcmp(pwdptr->pw_name, p_user)==0) 
			{
				iMatched = 1;
				break;
			}
		...
		if(iMatched) {
			...
			strncpy(char_user, pwdptr->pw_name, 64);
			strncpy(char_pwd, pwdptr->pw_passwd, 64);
			result->pw_name = (char *)p_user;
			result->pw_passwd = char_pwd;
			result->pw_uid = 1;
			result->pw_gid = 1;
