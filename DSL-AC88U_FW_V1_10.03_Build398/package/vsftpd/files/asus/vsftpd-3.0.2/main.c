...
int iAllowAnonymous = 0;
int iAccNum = 0;
struct passwd *pwdlist = NULL;

int
arc_getpwnam()
{
	char cfgItem[64], cfgValue[64];
	int i;
	char user[64], pwd[64];

	bzero(cfgValue, 64);
	arcgpl_cfg_get("ARC_USB_FTP_AllowGuest", cfgValue, 64);
	if(strlen(cfgValue)>0)
		iAllowAnonymous = atoi(cfgValue);
	else
		iAllowAnonymous = 0;
	//ftp_dbg("get iAllowAnonymous %d\n", iAllowAnonymous);
	
	
	bzero(cfgValue, 64);
	arcgpl_cfg_get("ARC_USB_ACCOUNT_TotalUserCount", cfgValue, 64);
	if(strlen(cfgValue)>0) {
		iAccNum = atoi(cfgValue);
		if(iAccNum > 20)
			iAccNum = 20;
	} else {
		iAccNum = 0;
	...
	//ftp_dbg("get iAccNum %d\n", iAccNum);
	if(iAccNum >0) {
		pwdlist = malloc(iAccNum*sizeof(struct passwd));
		if(pwdlist==NULL) return -1;
		
		bzero(pwdlist, (iAccNum*sizeof(struct passwd)));
		for(i=0;i<iAccNum;i++) {
			
			snprintf(cfgItem, 64, "ARC_USB_ACCOUNT_%d_Name", i);
			arcgpl_cfg_get(cfgItem, user, 64);
			if(strlen(user)<=0)
				continue;
			snprintf(cfgItem, 64, "ARC_USB_ACCOUNT_%d_Password", i);
			arcgpl_cfg_get(cfgItem, pwd, 64);
			if(strlen(pwd)<=0)
				continue;

			pwdlist[i].pw_name = (char *)strdup(user);
			pwdlist[i].pw_passwd = (char *)strdup(pwd);
			pwdlist[i].pw_gecos = pwdlist[i].pw_name;
			//ftp_dbg("get %s\n", pwdlist[i].pw_name);
			//ftp_dbg("get %s\n", pwdlist[i].pw_passwd);
