...
int runCmd(const char *cmd, char *buf, int iBufSize)
{
	FILE *fp;
	int ret = 0;
	int iAvailableBufSize = iBufSize;
	
	if(!cmd || !buf) return ret;
	
	fp = popen(cmd, "r");
	if(fp) {
		while(!feof(fp) && (ret < iBufSize)) {
			*(buf+ret) = fgetc(fp);
			ret++;
		}
		pclose(fp);
		usb_dbg("cmd %s, result %s", cmd, buf);
	}
	return ret;
}
...
int get_account_list(int *acc_num, char ***account_list)
{
	...
	*acc_num = nvram_get_int("acc_num");
	...
	if( runCmd("abscfg get ARC_USB_ACCOUNT_TotalUserCount", szRet, 8) >0) {
		*acc_num = atoi(szRet);
	} else {
		*acc_num = 0;
	}
		
	usb_dbg("acc_num %d", *acc_num);
	return 0;
#endif
}
	...
	if (follow_info == NULL) {
		...
		if (f == NULL) {
			...
			if(!strcmp(account,"admin")) {
				usb_dbg("Get default full wr right about \"%s\" with \"%s\".\n", f? f:"Pool", account);
				return 3;
			} else
				return -1;
		} else {
