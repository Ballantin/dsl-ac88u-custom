...
void SetHomeDIR(char *szDest, char *szSrc){
	...
	logfile(LOG_DEBUG, "szSrc \"%s\"", szSrc);
...
int CheckPartition(void)
{
	...
	logfile(LOG_DEBUG, "szHomeDir \"%s\"; wd \"%s\"", szHomeDir, wd);
...
#ifdef USB_MOUNT_POINT_TMP_MDEIA
static const char mount_path[] = "/tmp/mnt";
#else
static const char mount_path[] = "/ramdisk/tmp/mnt";
#endif
static int doIsolatedFlashArea(const char *name)
{
    ...
    logfile(LOG_DEBUG, "resolved_path: %s mount_path: %s", resolved_path, mount_path);

	if( 
		strncmp(resolved_path, mount_path, strlen(mount_path))==0) {
		iRet = 0;
	}
...
void domkd(char *name)
{
	...
	if(isRealDisk() != 1){
		logfile(LOG_DEBUG, "you should do mkdir in real disk");
...
void dostor(char *name, const int append, const int autorename)
{
	...
	//check the upload position, it must be real disk!
	if(isRealDisk() != 1){
		logfile(LOG_DEBUG, "you should do store in real disk!!!!!!!!!!!");
		addreply_noformat(553, MSG_CREATE_FAILURE);
		goto end;
	}
#endif // ARC_RAM_READ_ONLY
