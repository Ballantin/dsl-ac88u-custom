...
{
	return dns_validIPv6_query(ipAddress);
}
...
#endif
{
	...
	if (dns_type >= DNS_SRV)
	{
		printf("\n#[%s] %d, dns_cache_query_srv domain %s , server %s\n", __FUNCTION__, __LINE__, domain,server);
		strcpy(dns_server_list, server);
	#ifdef HAVE_IPv6	
		ans_srv = dns_cache_query_srv(dns_server_list, service, proto == UDNS_UDP ? "udp" : "tcp", domain, my_ip, my_port, timeout, local_ip_have_v6, allowIPv6);
	#else
		ans_srv = dns_cache_query_srv(dns_server_list, service, proto == UDNS_UDP ? "udp" : "tcp", domain, my_ip, my_port, timeout);
	#endif	
	}
	...
	{
		ipArr_t *tmpA = NULL;
		dns_a4_ans_t *ans_a4 = NULL;
		
#ifdef HAVE_IPv6
		dns_a6_ans_t *ans_a6=NULL;
		//query IPv6 address first , if not then query IPv4 addrress. 
		
		printf("\n#[%s] %d,  domain %s , server %s , ip %s\n", __FUNCTION__, __LINE__, domain,server,my_ip);
		strcpy(dns_server_list, server);
		if (local_ip_have_v6 && allowIPv6)
		{
			ans_a6 = dns_cache_query_a6(dns_server_list, domain, my_ip, my_port, timeout);
		}
		if (ans_a6 != NULL)
		{
			tmpA = ans_a6->ipArrHead;			
		}
		else
		{
#endif	
		...
		while (tmpA != NULL)
		{
			...
			tmpIP->proto_type = proto;
#ifdef HAVE_IPv6
			tmpIP->ttl = ans_a6?ans_a6->ttl:ans_a4->ttl;
#else
			tmpIP->ttl = ans_a4->ttl;
#endif
			tmpIP->next = NULL;
	...
	if(head_ip){
		if (dns_validIPv6(head_ip->ip)){
			head_ip->type = UDNS_AF_INET6;
		}else{
			head_ip->type = UDNS_AF_INET;
		}
	}
	...
	if (srv_ans != NULL)
	{
		...
		while (pointer_s != NULL)
		{
			...
			{			
				...
				printf("\n#[%s] %d, dns_query_a6 server %s ,domain %s , ip = %s\n", __FUNCTION__, __LINE__, server,pointer_s->domainName,my_ip);
				if (local_ip_have_v6 && allowIPv6)
				{
					ans_a6 = dns_query_a6(server, pointer_s->domainName, my_ip, my_port, timeout);
				}
				if (ans_a6 != NULL)
				{
					//printf("ans_a6->ipArrHead-> ip  = %s\n",ans_a6->ipArrHead->ip);
					pointer_s->ipArrHead = ans_a6->ipArrHead;
					free(ans_a6->domainName);
					free(ans_a6);
...
dns_a6_ans_t *dns_cache_query_a6(char *server, char *domain, char *my_ip, unsigned short my_port, unsigned int timeout)
{
	...
	while (pointer != NULL)
	{
		if (tt >= pointer->ttl)	// timeout
		{
			dns_a6_ans_t *tmp = pointer;

			if (privious == pointer)
			{
				dns_a6_ans_list = pointer->next;
				privious = pointer->next;
			}
			else
			{
